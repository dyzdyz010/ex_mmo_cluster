...
早期服务端 / UE 文档中的 `MicroPerMacro=4` 会让 `1 micro = 25 cm`。这适合粗粒度方块，
但会限制曲面、薄墙、楼梯和局部破坏。当前 canonical 服务端权威设计已与浏览器端统一为
`8x8x8` refined payload，并在协议中显式携带 `micro_resolution`；UE `test1` 的 4 只保留为历史参考。
