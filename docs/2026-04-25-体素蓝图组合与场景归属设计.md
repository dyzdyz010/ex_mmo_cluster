...
- `docs/2026-04-29-server-authoritative-voxel-data-protocol-design.md`：服务端权威 voxel 第一版数据结构、协议、地块建设与同步原则。
...
4. `VoxelIntentResult`：返回 action seq、结果码、拒绝原因和 authoritative cell/object。
...
第一版服务端权威设计与浏览器端统一为 `8x8x8` refined payload，并在协议中显式携带
`micro_resolution`。`1 micro = 12.5 cm`，足够表达曲面、薄墙、楼梯和局部破坏。
...
5. 服务端拒绝或判定 intent 过期时能返回 authoritative object/cell；客户端不把该 intent 写进 confirmed geometry。
