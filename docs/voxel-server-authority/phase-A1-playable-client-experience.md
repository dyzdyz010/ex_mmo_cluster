...
- 2026-05-09:决策稿起草。用户已授权直接开干(不再单独 review),边做边纠偏。决策稿入仓 commit `edbfbda`。
- 2026-05-09:A1-1 落地(commit `0275899`)。BlueprintCatalog v2(sphere/cylinder/stairs micro mask),PrefabRaster 产生 micro intents,gate dispatch 改 `:put_micro_block`,客户端 catalog + hotbar 跟齐。**已知性能问题:sphere ~248 micro intents 在当前 Storage.put_micro_block 路径下 O(N²),实测 1.5-2s 延迟,prefab 测试 timeout 调到 5s**;追加 backlog:**A1-1b** Storage.put_micro_blocks/4 batch API,把 O(N²) 降到 O(N)。
- 2026-05-09(待开):**A1-1b** 性能优化(独立 step,demo 可玩性已满足下可推后)/ **A1-2** prefab 防覆盖(chunk_process build_intents_storage 现状 occupied silent skip,改 reject) / **A1-3** prefab 线框预览 / **A1-4** 跳跃同步 / **A1-5** 破坏技能。
