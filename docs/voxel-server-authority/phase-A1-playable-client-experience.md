...
**起草日期**:2026-05-09。**状态**:已完成(2026-05-09)。
...
- 2026-05-09:决策稿起草(commit `edbfbda`)。
- 2026-05-09:**A1-1** prefab 形状 micro 化(commit `0275899`)。BlueprintCatalog v1 macro list → v2 micro mask,sphere/cylinder/stairs 跟客户端 prefab/definitions.ts 几何函数对齐。PrefabRaster 产生 micro `:put_micro_block` intents。gate ws/tcp build_prefab_intents_by_chunk 改 micro 字段。客户端 onlinePrefabCatalog + worldEditController hotbar 跟齐。已知性能 1.5-2s(O(N²) Storage.put_micro_block),测试 timeout 调到 5s,记 **A1-1b** backlog。
- 2026-05-09:**A1-1 e2e smoke**(commit `a4616e9`)。stdio observe + WsConnection driven sphere placement,验证 storage occupancy_words 跟 BlueprintCatalog 像素级对齐(280/280 slots),persisted Postgres round-trip 也保 mask intact。
- 2026-05-09:**A1-2** prefab 防覆盖(commit `14c90a9`)。chunk_process.prepare_transaction_in_state 加 validate_batch_occupancy/2 + in-batch claimed-set。reject 在 prepare 阶段(fence 未写入 Postgres,zero-cost cleanup)。Wire reason unwrap `{:prepare_failed, _coord, inner}` → 裸 atom(`:micro_slot_already_occupied`)。客户端 hudView 订阅 `world:voxel-prefab-result` reject flash。e2e smoke 验证 reject 仅 20ms,chunk_version 不变。
- 2026-05-09:**A1-3** prefab 线框预览(commit `b2fe630`)。prefabPreviewGeometry.ts 早已实现 micro mask 描线框,A1-1 切 hotbar 后 cell.occupancyWord 真实存在,自动正确。加 chunkRenderer regression test 把"builtin prefab 产生 200+ micro slots wire,不退化成单 macro 方框"立成契约。
- 2026-05-09:**A1-4** movement ack ground_z 端到端流通(commit `b692ab1`)。诊断发现 jump 同步链路其它层都 IMPLEMENTED,唯一 gap 是 server ack 不带 ground_z + client localPlayer hack 绕过 server truth。修法:Ack struct 加 :ground_z,gate codec frame 末尾追加 float-64 BE(96 → 104 字节),client decoder offset 96 解码,reconcile + localPlayer 直接用 ack.groundY 不再 fallback。**A1-4 jump arc smoke**(commit `133bb85`)验证 ground_z 在 airborne arc 中 0 漂移,apex 96cm(对齐 A2 决策稿 ~1m 设计)。
- 2026-05-09:**A1-5** 破坏技能 voxel damage 路由(commit `7932fe2`)。新模块 SceneServer.Combat.VoxelDamageRouter:从 world_micro_coord lookup ChunkSnapshotStore → Storage.lookup_owner_at → ObjectRegistry.accumulate_damage。PlayerCharacter resolve_skill_cast 后并行 dispatch voxel damage(双轨,actor 路径不变)。4 个 unit test 覆盖 :no_voxel / :applied / :cascade(part_destroyed)路径。

## 已知 backlog

- **A1-1b Storage batch micro_block API**(性能):新增 `Storage.put_micro_blocks(storage, macro, [{slot, layer_attrs}], opts)` 一次性构造 refined cell + 一次 `normalize!`。chunk_process 在 commit 路径 detect 同 macro 的 `:put_micro_block` 列表 → 调 batch API,把 sphere prefab 1.5-2s 降到 < 200ms。Phase 5 ChunkDelta 优化时也会用到。
- **transaction snapshot 跨 test 残留**(测试隔离):scene_server / gate_server 在前一个测试 prefab 卡住时,会留下 :prepared transaction 在 voxel_transaction_coordinator_snapshots 表;下次 mix test 启动时 coordinator reload 触发 cascade。test_helper 加 setup 阶段 truncate 几张 voxel 表即可,但 not in A1-1 scope。
