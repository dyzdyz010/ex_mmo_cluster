...
- **Voxia LOD 分层与各层技术选型设计（2026-07-06，v2.2 待拍板)**:[`docs/voxel-server-authority/2026-07-06-voxia-lod-layering-and-technology-design.md`](./2026-07-06-voxia-lod-layering-and-technology-design.md) —— 07-05 路线的下游细化：L0-L4+天空分带（7/14/28/56m 环 + 3.5m collar）、每层技术选型与理由、source page payload=7m occupancy+material mip（any-solid/众数规约算子）、分带失效契约（含流式重派生定位与换轨触发条件）、seam/垂直/tier 契约、三列里程碑（A 客户端渲染正确 → B 接口冻结+fixture 消费 → C 服务端接入）。经 1 轮对抗评审修订。
