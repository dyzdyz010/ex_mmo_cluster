...
| VLOD-A1 | 体素 LOD 里程碑 A 步 1:切默认分级 + 显式 tier 契约 + L2.5 第三环(四环 7/14/28/56m,实测 quads 3.70M→1.39M −62%) | 实现完成、核心验收全绿(#9 FPS/#10 截图被环境级 GPU device-removal 阻塞,归 A2) | [`phase-vlod-a1-explicit-tiering.md`](./phase-vlod-a1-explicit-tiering.md) |
| VLOD-A2 | 体素 LOD 里程碑 A 步 2:远景渲染后端分组件 UDynamicMeshComponent + StaticDraw + 组件级视锥剔除(消除 A1 device-removal 与 RuntimeMesh O(N²)、不动几何) | 决策稿就绪、未开工 | [`phase-vlod-a2-partitioned-staticdraw.md`](./phase-vlod-a2-partitioned-staticdraw.md) |
