...
为准：当前推进 Phase 7.E 材料属性扩展；完成后再进入 `ConductionPathKernel` 与电属性使用。Phase 8 物理现象系统已落设计目标稿，但必须等
