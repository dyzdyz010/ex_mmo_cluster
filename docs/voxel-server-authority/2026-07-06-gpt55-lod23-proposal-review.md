...
> **2026-07-06 后记（终态化拍板）**:本节裁决当日已由用户升格定案——投影路线(pages/snapshot 下发)即**终态**,"客户端 worldgen 采样"从"终态带宽优化"降格为**特定负载画像下的定向优化选项**(仅限"处女地基底本地生成"的定向加法,被修改区仍走投影)。下方五条件语义不变,但其重启对象相应收窄。裁决全文与五维度依据见 [`2026-07-06-projection-route-final-decision.md`](./2026-07-06-projection-route-final-decision.md)。
