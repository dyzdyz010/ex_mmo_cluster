...
| **S6g（第五片已落地）** | runtime SVDAG resource 数据面：SVO builder 输出 CPU 侧 root table + 去重 node buffer，pack 为固定 64B node / 48B root word payload，由 `FVoxiaSvoRuntimeGpuBuffers` 上传为 RHI ByteAddress buffer；`FVoxiaSvoRaymarchCS` 参数绑定面、独立 `VoxiaShaders` PostConfigInit 模块和最小 dispatch probe 已接入。完整 GPU raymarch renderer / screen pass 尚未接入 | `Voxia.Voxel.SvoPreview` 红绿；8km SVO CLI smoke 为 ready=true / runtime payload bytes；WorldActor 日志 `rhi_ready=1` / `rhi_bytes=...` / `raymarch_dispatched=0`（null RHI）；真实 RHI automation dispatch report 为 shader_available=1 / parameters_ready=1 / dispatched=1 / groups=[1,1,1] | 中 |
| S6+（下一步） | 8km 生产级权威 3D 源全量调度、持久化 artifact、完整 launcher/update 包下载/安装 UI 与 diff-chain 可视化流程、完整 GPU raymarch renderer / screen pass；VHI 冻结为过渡 baseline。当前用户要求先不碰服务端，后续仅继续客户端侧渲染生产化 | 分阶段 CLI + real RHI + 服务端 materialization 验收 | 高 |
...
- SVO 持久化 artifact、完整 launcher/update 包下载/安装 UI、完整 GPU raymarch renderer / screen pass；客户端本地 baseline pack release-manifest shard 校验和 `ConnectGate` / `EnterScene` entry gate 只覆盖已下载 `.vxpack` 与入场前硬拒，不等于完成包分发链路。runtime SVDAG resource 已创建第一片 RHI ByteAddress buffer，并具备 shader 参数绑定面和最小 dispatch probe，但尚未被完整 raymarch renderer / screen pass 消费；upload-level section 复用目前只是在 ProceduralMesh patch section 层跳过未变化 mesh，尚未替换为 runtime mesh / HISM / Nanite-ready artifact。
