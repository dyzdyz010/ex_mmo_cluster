...
| **S6g（第二片已落地）** | runtime SVDAG resource 数据面：SVO builder 输出 CPU 侧 root table + 去重 node buffer，并 pack 为固定 64B node / 48B root word payload；RHI buffer / shader / raymarch 尚未接入 | `Voxia.Voxel.SvoPreview` 红绿；8km SVO CLI smoke 为 ready=true / 21016 roots / 3627 nodes / 58032 node words / 252192 root words / 1240896 bytes | 中 |
