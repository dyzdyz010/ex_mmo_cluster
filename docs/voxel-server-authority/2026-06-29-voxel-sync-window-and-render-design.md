...
> - [`docs/original/2026-06-27-架构设计指导思想-系统正交.md`](../original/2026-06-27-架构设计指导思想-系统正交.md) —— 正交分解 / 自维护不变量（含时间性不变量）/ 显式契约。
> - [`docs/current_status/design/voxel/README.md`](../current_status/design/voxel/README.md) —— 体素真值、三阶段边界、tile 预算口径、baseline 硬校验铁律。
>
...
>
> **v3 整合**：整合用户提供的 [`docs/design/voxel_mmo_sliding_window_architecture.md`](../design/voxel_mmo_sliding_window_architecture.md)，补客户端运行时工程层（§6.7：L0/L2 双层无 L1、collision 资源解耦、Hot/Warm/Cold tile 生命周期、预测加载、mesh page、失败处理；W-14~16）。该文档与本稿三态一致，其架构正确性短板（远景编辑可见性无 owner / 无 content_version 运维 / 无浮点 bit-exact / 无垂直维度 / 无动态层）由本稿 W-8~W-13 补。
>
...
> 整合用户提供的 [`docs/design/voxel_mmo_sliding_window_architecture.md`](../design/voxel_mmo_sliding_window_architecture.md)，补本稿在客户端运行时工程上的空白（尤其 **collision**——v1/v2 完全遗漏）。该文档「L0 真实体素窗口 + L2 远景代理、v1 无 L1」与本稿三态一致；其架构正确性短板（远景编辑可见性无 owner、无 content_version 运维、无浮点 bit-exact、无垂直维度、无动态层）由本稿 W-8~W-13 补齐。
...
- [`docs/voxel-server-authority/2026-06-29-voxel-baseline-streaming-boundary.md`](./2026-06-29-voxel-baseline-streaming-boundary.md) — baseline 形态/三边界/H 凭证（D-1~D-14）
- [`docs/current_status/design/voxel/README.md`](../current_status/design/voxel/README.md) — 体素真值/三阶段边界/tile 预算
- [`docs/current_status/design/client/streaming-lod.md`](../current_status/design/client/streaming-lod.md) — 近/远渲染、heightmap LOD
...
- [`docs/design/voxel_mmo_sliding_window_architecture.md`](../design/voxel_mmo_sliding_window_architecture.md) — 用户提供的滑动窗口最小可行设计（L0/L2 双层、Hot/Warm/Cold、预测加载、mesh page、collision 分层、失败处理）；客户端工程层整合进 §6.7
