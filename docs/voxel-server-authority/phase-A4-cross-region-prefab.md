...
- 2026-05-10:**A4-bis-3 落地**。`SceneServer.Voxel.RegionRuntime.apply_lease/2` handler 在 put state.leases 之后调 `RegionRouting.register_local_region(lease.region_id)`;失败(BeaconServer cluster split-brain / Horde join 失败)只 emit `voxel_lease_region_routing_register_failed` 不阻塞 lease apply(lease-local state 是 source of truth)。同 region_id 重复 apply(lease epoch bump)幂等(BeaconServer.register 把 `:already_registered` 当 `:ok`)。**lease 释放走 process exit + Horde 自动 reap**(显式 `release_lease/2` API 决策稿写"接 unregister"但当前没有 World-driven migration / expiry caller,留到真有 caller 时再加 — YAGNI;decision stub 在 RegionRuntime 顶端 docstring "Cluster region routing" 段标清)。**Horde.Registry startup 移到 `apps/scene_server/test/test_helper.exs`** 而非 per-test setup,因 Horde CRDT keys ETS 表 init 需要时间,setup-per-test 会让首个 test 的 register 撞 `:ets table does not exist`;test_helper 一次启动给 ExUnit boot 过程足够初始化窗口。新增测试 3 个(`region_runtime_test` "cluster region routing (Phase A4-bis-3)" describe 块):apply_lease → BeaconServer 可见、同 region 重复 apply 幂等、RegionRuntime exit → entry reaped。所有"register → resolve" 断言走 `await_resolve_to/3` polling helper,因 Horde CRDT 是 eventually consistent(单节点 propagation 窗口虽小但非零)。生产路径 `RegionRuntime.apply_lease` 还无 caller(A4-bis-4 World 端 lease 派发接进来才在生产触发 register),本步是 hook 就位。回归:scene 1 doctest + 410 tests(+3 cluster routing),0 fail;其他 app 无变动。**未 push**。
- 2026-05-10:**A4-bis-4 段 1 落地**(把 A4-bis-4 切成两段以提供干净检查点)。新模块 `WorldServer.Voxel.SceneNodeRegistry`(`apps/world_server/lib/world_server/voxel/scene_node_registry.ex`)World 侧持 `join_order :: [node()]` + `region_assignments :: %{region_id => node()}` + `round_robin_cursor`,实现 D8.B 分配策略:`register_scene_node/2` append 到 join_order 幂等、`unregister_scene_node/2` 从 join_order 移除但**不 reassign 已分配 region**(MVP no failover,Phase 6 HA 留)、`assign_region/2` round-robin pick 下一个 join_order node,**已分配 region 调用幂等返回原 assignment 不重新分配**(避免 runtime 转手),`{:error, :no_scene_nodes}` 当 join_order 空且无既往 assignment、`lookup_assignment/2` 只读不 assign-on-miss、`snapshot/1` debug。**段 1 不挂 supervision tree、不接 BeaconServer subscriber、不动 MapLedger / world_sup**(零 production-path 改动 = 检查点干净)。新增 15 单测覆盖。回归:world 1 doctest + 94 tests(+15),0 new fail。**未 push**。
- 2026-05-10:**A4-bis-4 段 2 落地**(共 4 个 sub-commit)。
  - **段 2a**(`e317736`):新模块 `WorldServer.Voxel.SceneNodeMonitor`(GenServer 监听 `:net_kernel.monitor_nodes(true)` → `:nodedown` 自动调 `SceneNodeRegistry.unregister_scene_node`,**不**处理 `:nodeup` 因为不能区分 scene_node vs 其他 BEAM)。WorldSup children 加 SceneNodeRegistry + SceneNodeMonitor。Edge case 修:`:net_kernel.monitor_nodes(true)` 在 `:nonode@nohost`(mix test 默认)返回 `:ignored` 而不是 `:ok`,原 init `:ok = monitor_nodes(...)` 模式匹配会 crash → WorldSup 起不来;改 case 分支宽容处理。新增 6 单测(scene_node_monitor_test)。
  - **段 2b**(`30a7a7c`):`SceneServer.Interface.handle_continue(:setup)` 注册 `:scene_server` 之后,await(:world_server) → `:rpc.call(world_node, SceneNodeRegistry, :register_scene_node, [...])` 主动通知 World 加入。架构约束:scene_server 在 mix.exs 不依赖 world_server,RPC 模块名用 atom literal `:"Elixir.WorldServer.Voxel.SceneNodeRegistry"` 绕开编译期检查。失败(World unavailable / RPC 抖动)只 Logger.warning,scene_server 启动**不阻塞**(degraded mode:legacy 单 region 路径仍可用)。`announce_to_world/1` 提成公开函数 + opts 注入(`:registry_module` / `:registry_name` / `:await_timeout_ms` / `:rpc_timeout_ms`),便于单测。新增 2 单测(interface_announce_test)。**关键 bug 修**:第一版混淆"RPC 模块名"和"GenServer name"(production 默认值碰巧两者相同,test 用不同 GenServer name 时炸 :undef);拆成两个独立 opt。
  - **段 2c**(`de8b4b7`):`MapLedger.put_region` 集成 `SceneNodeRegistry.assign_region/2`(D8.B round-robin)。`RegionAssignment.defstruct` 加 `assigned_scene_node :: node() | nil` 字段。MapLedger init 接 `:scene_node_registry` opt(default nil = legacy 行为)。WorldSup 现在传 `WorldServer.Voxel.SceneNodeRegistry` opt,WorldSup children 顺序调整(SceneNodeRegistry / SceneNodeMonitor 现在挂在 MapLedger **之前**)。私有 `maybe_assign_scene_node/2` 处理三种 case:registry = nil → 跳过(legacy);assigned_scene_node 已被 caller 显式填(seed pin)→ 不动;否则调 SceneNodeRegistry.assign_region(`:no_scene_nodes` 时 emit `voxel_region_put_no_scene_nodes` observe + 仍 allow put_region 成功 with assigned = nil)。新增 6 单测(map_ledger_scene_node_assign_test)。
  - **段 2d**(`345337f`):`world_sup.default_scene_opts_resolver` 改造按 participant 解析。新 API `MapLedger.lookup_region_scene_node/2`(读 `RegionAssignment.assigned_scene_node`)。新 API `WorldSup.default_scene_opts_resolver/2(participants, opts)`(原 1-arity 版变 wrapper 调 /2 with default `:ledger` = `WorldServer.Voxel.MapLedger`)。Resolver 新行为:全部解出 → 完整 opts;部分解出 → 部分 opts(executor 后续 prepare 阶段对未解出 participant 报 per-participant fail);全部解不出 → `:scene_unavailable`(watcher park transaction)。Open trade-off:partial resolution 让 executor 区分"world 路由失败"vs"per-participant prepare 失败",但部分成功的 transaction 会半 commit;Phase 6 HA 改成"全 abort 重试"。新增 5 单测(world_sup_resolver_test)。
  - **A4-bis-4 段 2 总回归**:world 1 doctest + 110 tests(+16 across 段 2a-2d),1 fail(已知 Windows path baseline);scene 412/0;gate 193/1(已知 cross_region fixture flake,无关本步)。**生产路径真改变了**(段 2d 让 World 按 region 路由 vs 之前固定单 scene_server lookup),单 scene_node 部署应等价(scene 上线 → RPC announce → SceneNodeRegistry 加 → MapLedger.put_region 分配 → resolver 解出这条新链路)。**用户验证检查点**:重启 server / 摆 prefab / 破坏方块,应跟段 1 之前等价(零行为差异);如有差异说明这条新链路某环出问题。**未 push**。
