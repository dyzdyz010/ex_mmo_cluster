...
- 2026-05-10:**A4-bis-4 段 1 落地**(把 A4-bis-4 切成两段以提供干净检查点)。新模块 `WorldServer.Voxel.SceneNodeRegistry`(`apps/world_server/lib/world_server/voxel/scene_node_registry.ex`)World 侧持 `join_order :: [node()]` + `region_assignments :: %{region_id => node()}` + `round_robin_cursor`,实现 D8.B 分配策略:`register_scene_node/2` append 到 join_order 幂等、`unregister_scene_node/2` 从 join_order 移除但**不 reassign 已分配 region**(MVP no failover,Phase 6 HA 留)、`assign_region/2` round-robin pick 下一个 join_order node,**已分配 region 调用幂等返回原 assignment 不重新分配**(避免 runtime 转手),`{:error, :no_scene_nodes}` 当 join_order 空且无既往 assignment、`lookup_assignment/2` 只读不 assign-on-miss、`snapshot/1` debug。**段 1 不挂 supervision tree、不接 BeaconServer subscriber、不动 MapLedger / world_sup**(零 production-path 改动 = 检查点干净)。段 2 范围(下个 commit):接到 `WorldServer.WorldSup` + 订阅 BeaconServer.DistributedRegistry `:scene_server` 事件 + `MapLedger.put_region` 调 `assign_region/2` 持久化 assigned_scene_node + `MapLedger.region_for_chunk/2` 反向索引 API + `world_sup.default_scene_opts_resolver/1` 改按 participant `lookup_assignment` 解析 scene_node。新增 15 单测(`scene_node_registry_test`):join_order tracking 5 个(empty start、append order、re-register 幂等、unregister 删、unregister 未知 no-op)+ assign_region D8.B 5 个(no-nodes :error、single-node、multi-node round-robin wrap、re-assign 幂等不动 cursor、新加节点不偷已有 region 这条 D8.B 核心 invariant)+ lookup_assignment 3 个(miss :error、hit、read-only)+ unregister + 既存 region 2 个(no-failover 后既存 assignments 仍指向 down node、所有 node down 不丢已分配 region)。回归:world 1 doctest + 94 tests(+15)、1 fail(已知 Windows path baseline `authority_observe_test`,跟本步无关);其他 app 无变动。**未 push**。**用户检查点:可以验证 SceneNodeRegistry 单测 + 设计;段 2 之前给用户机会 review D8.B 实现是否符合预期**。
