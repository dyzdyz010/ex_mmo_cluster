...
**Last updated**:2026-07-03,**Voxia SVO composite depth occlusion checkpoint**。旧 A4-bis 接力记录保留在下方；当前会话接手 Voxia 近场窗口、VHI 卡顿和 SVO 远景路线时，先看本节与 `docs/current_status/design/client/streaming-lod.md`。
...
- 下一步优先级：当前用户要求先不碰服务端，因此后续只推进客户端侧 gap。`-VoxiaSvoRaymarchComposite` 的 root/node hash debug tint 已替换为真实 SVO payload traversal / depth-aware blend，并补入 debug scene-depth occlusion 第一片；FarField 也已补 backend-neutral render artifact contract。下一项若继续客户端优先，应在真实 RuntimeMesh / HISM / Nanite 后端接入和 production camera-correct world-space SVO renderer 之间择一推进；完整 launcher/update 包下载/安装 UI 与 diff-chain 可视化流程留待服务端/打包链路恢复后继续。
