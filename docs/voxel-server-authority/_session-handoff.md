...
**Last updated**:2026-07-03,**Voxia SVO raymarch composite-hook checkpoint**。旧 A4-bis 接力记录保留在下方；当前会话接手 Voxia 近场窗口、VHI 卡顿和 SVO 远景路线时，先看本节与 `docs/current_status/design/client/streaming-lod.md`。

## 2026-06-30 Voxia near-window / VHI / SVO checkpoint

- Voxia 客户端提交：`9896917 feat(voxia): integrate near-window VHI SVO streaming`；raymarch composite hook 最新提交为 `d67f747 feat(voxia): add SVO raymarch composite hook`。
...
- 下一步优先级：当前用户要求先不碰服务端，因此后续只推进客户端侧 gap：完整 launcher/update 包下载/安装 UI 与 diff-chain 可视化流程留待服务端/打包链路恢复后继续；客户端侧下一项是把 `-VoxiaSvoRaymarchComposite` 从 pass-through view extension 推进到真正写入 SceneColor 的 composited GPU raymarch pass，或继续把 section fingerprint 复用推进到更稳定的 runtime mesh / HISM / Nanite-ready artifact。
