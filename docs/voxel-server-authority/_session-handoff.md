...
**Last updated**:2026-07-03,**Voxia SVO raymarch traversal-composite checkpoint**。旧 A4-bis 接力记录保留在下方；当前会话接手 Voxia 近场窗口、VHI 卡顿和 SVO 远景路线时，先看本节与 `docs/current_status/design/client/streaming-lod.md`。
...
- 下一步优先级：当前用户要求先不碰服务端，因此后续只推进客户端侧 gap。`-VoxiaSvoRaymarchComposite` 的 root/node hash debug tint 已替换为真实 SVO payload traversal / depth-aware blend；下一项优先把 section fingerprint / ProcMesh patch 路线推进到更稳定的 runtime mesh / HISM / Nanite-ready artifact。完整 launcher/update 包下载/安装 UI 与 diff-chain 可视化流程留待服务端/打包链路恢复后继续；production camera-correct / scene-depth occluded SVO renderer 也仍是后续增强，而非当前 debug composite 的完成条件。
