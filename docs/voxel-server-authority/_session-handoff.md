...
- 当前路线真值见 `docs/voxel-server-authority/2026-07-05-voxia-voxel-lod-production-route.md`：L0 近景继续服务端权威 3x3x3 tiles；L1-L3 远景走 SVO / SVDAG source pages + leaf-surface mesh artifact + FarField patch 上传；raymarch-only 只保留为 L4 或 A/B profile；VHI 冻结为 2.5D baseline。
