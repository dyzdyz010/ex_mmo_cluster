...
- 下一步优先级：当前用户要求先不碰服务端，因此后续只推进客户端侧 gap：完整 launcher/update H gate 留待服务端/打包链路恢复后继续；客户端侧下一项是把 CPU SVDAG artifact 推到 GPU raymarch renderer / runtime resource，或继续把 section fingerprint 复用推进到更稳定的 runtime mesh / HISM / Nanite-ready artifact。
