...
> 日期：2026-07-06　状态：长稳参考 v2.13（2026-07-13 同步 A10 本地 provider、shared/parallel/async 性能切片与预算化回收；修订记录见 §11）
> 定位：[`2026-07-05-voxia-voxel-lod-production-route.md`](2026-07-05-voxia-voxel-lod-production-route.md) 的现行下游参考——把 L0-L3、source pages 与 mesh renderer 落成边界和里程碑。2026-07-12 的 v2.8 覆盖旧稿中“raymarch 保温”“A 只含原 A1-A5”以及“A10 先接磁盘 provider”的相反表述。
> 证据源：GPT-5.5 两份方案的对抗评审（[`2026-07-06-gpt55-lod23-proposal-review.md`](../../20-archive/voxel-far-field/2026-07-06-gpt55-lod23-proposal-review.md)）、codex 三份代码调研、UE 5.8 源码实地核查（`D:\UE\UE_5.8`）、8km 实测锚点（`_session-handoff.md:32-37`）。
> 术语口径：base / delta / truth / snapshot 的统一定义见 [`glossary.md`](../protocol/glossary.md)（2026-07-06 拍板）——本稿的 source page 即"远区 7m snapshot"。
> 数据源终态：**投影路线已拍板为终态**（[`2026-07-06-projection-route-final-decision.md`](../contracts/2026-07-06-projection-route-final-decision.md)）——§3.2b 的"pages 为唯一生产远景源"不再是待拍板项；同构路线（客户端 WorldGen ⊕ overlay）降格为定向优化选项。
> **2026-07-13 实现状态覆盖**：§2-8 的四环距离、服务端 snapshot-only 方向和 mesh renderer 仍是 B 的设计输入；其中 `SVO macro cell`、按 Y 占据层清单与二维 near-skip 等文字描述的是默认兼容路径或旧生产提案，**不是已经冻结的生产 artifact 契约**。扩展 A 已建立唯一 `production_all_features` 根，把成熟 near 滑窗/数据泵与 Pure3D far 同场运行。Pure3D far 已实跑 WorldGen/H-gated local request page diff/residency、cooperative cancellation、source-bound shared artifact cache、parallel resolved surface、worker coverage plan、预算化 lease release 与 absolute XYZ stable-patch transaction；相邻 Real-RHI worker 约 `0.91-0.95s`。local 只接 far，near/far 尚未共享 residency/generation/transaction，反向依赖/full oracle、离群帧与完整 route 仍未收口。B 才决定固定 7m 投影 payload 与规约，不能从旧 SVO 编码反推。
...
| **L4 超远景**（**需求触发 defer**，见 §3.3） | d 73-96 | 8.2-10.75km | 未选型 | 未排期；触发后重新评估 coverage 与预算 | 未选型；raymarch 已退出当前路线 | 剪影 |
...
**T-2 已拍板采纳四环（2026-07-06）**——派生环全链 2× 跳变、每个入带角尺寸 ≤20px（17/12/15px）、代价 +18% quads。回退钩子保留：若完整三维多层在扩展 A 的实跑预算中冲破上限，降级序列第二档即撤 L2.5 回 7/14/56（见 §5），其余设计不受影响——回退成本只是一行 tier 配置加一次预算复核。
...
- 两者表达力等价，但 SVO 白送同质空间自动坍缩与单一结构覆盖全部 L1-L3 环；历史 SVDAG 子树去重数据只保留为诊断证据，不再驱动 L4 路线。
...
- **为什么 page 装 7m mip 而不是原始 chunk / SVO 节点 / mesh**：原始 chunk 是 71TB 问题；SVO 节点会把树编码固化成线协议；mesh artifact 又依赖 coverage/boundary 位形，不能作为 canonical source。7m material/occupancy page 是 renderer-neutral 的客户端派生输入，压缩后约 ~1-2KB/cell；当前生产 renderer 只走 mesh，raymarch 已退出路线。
...
- **为什么不是 Nanite/HLOD/RVT/impostor**：Nanite 运行时构建是 UE5.8 API 级不可行（NaniteBuilder editor-only，运行时 `BuildFromMeshDescriptions` 强制 fast-build）；WP/HLOD 对零静态 actor 的纯运行时世界输入为空集；RVT/impostor 都不能作为完整 3D 洞穴、崖壁与浮空体的默认表达。离线 bake 仍只在独立 A/B 数据证明收益后立项。
- **更新频率分带（P-4 的渲染侧）**：L1 跨 tile 即时增量；L2 跨 tile 批量合并；L2.5/L3 低频队列（秒级 coalesce），远环 dirty 永远排在近环之后。
- **材质/光照口径**：兼容路径远景默认 **Lit**，与近景共用顶点色和 `FVoxiaTerrainUv` 的有界相位契约；互补 dither 与 fog/TSR 专项已收口。纯 3D 新路径使用 world-aligned 采样，不读取绝对 mesh UV；opaque 已通过，dither/透明/发光材质族仍属 A10。历史决策与实证见 [`phase-vlod-a4-seam-fade-collar.md`](../../20-archive/voxel-far-field/phase-vlod-a4-seam-fade-collar.md)。

### 3.3 L4：第五环扩圈（**仅保留需求触发器，不绑定 raymarch**）

2026-07-10 的真实 RHI 复核在 compute dispatch/readback 成功后再次触发 D3D12 3D/Compute 队列超时；当前拍板是**严格不用 raymarch**。历史 SVDAG/compute 代码只作诊断遗留，不是 B backlog、A/B profile 或 renderer 备选。

是否需要超过 8km 的第五环仍可独立评估，但必须另立阶段重新选 renderer/数据预算。触发条件仅保留：

1. 世界内容出现必须在 >8km 可见的标志性结构；
2. 美术定调清澈大气且 8km 收尾在截图中不可接受；
3. 高空俯瞰的世界边界在真实巡航中被判不可接受。

未命中触发条件时不扩圈；即使命中，也不得自动恢复 raymarch。缺 source coverage 时必须显式禁用，禁止 fallback。
...
## 9. 阶段路线（三列里程碑；v2.7 以完整 3D 与流送扩展 A）

> 排序原则：**A 先把客户端完整 3D near/far LOD、流送数据流与 presentation 做正确且可持续，零服务端依赖；B 在 A 退出后才冻结生产投影契约并形成 fixture oracle；C 才改服务端。** 通用 codec、source-neutral builder 或 H-gated 本地 batch 属于 A 的客户端地基，不等于 B 已开工。

### 里程碑 A：客户端完整 3D LOD 与流送正确性（当前进行中）

| 步 | 内容 | 状态 |
| --- | --- | --- |
| A1 | 显式 tier + 7/14/28/56m 四环 | 已完成 |
| A2 | 分组件 `UDynamicMeshComponent + StaticDraw`、剔除与 bounded submit | 已完成 |
| A3 | per-cell masked greedy merge；raymarch 竞态诊断并最终禁用 | 已完成 |
| A4 | 跨 depth coverage、互补 fade、3.5m collar 与 8km 长巡航 | 已完成 |
| A5 | 紧凑顶点、cache key/LRU/容量/孤儿卫生 | 已完成 |
| A6 | near 连续 generate/apply、compact store、per-chunk renderer、bounded observe/pump | 已完成 |
| A7 | near/far XYZ ownership、retirement lease、垂直活性、快速折返与联合性能 | 已完成 |
| A8 | XYZ cube-shell、canonical pages、六向 material mip、coverage-resolved exact surface | dev 单代内核已完成 |
| A9 | source-neutral scene stage、generation barrier、真实 fence/scene host、三维 dev Real-RHI 手动整代切代 | 单代内核已完成；不代表完整滑窗世界 |
| A10 | 唯一生产组合根、WorldGen 正常入场、同源 near/far、自动 XYZ 滑窗、请求式 page residency、可取消增量 DAG、stable patch 分块呈现、H-gated 本地 provider、三轴长巡航、完整材质族 | 实施中：唯一根与 Pure3D far S2/S3/S2L 首轮完成，S4/S5 已有 artifact/stable patch 实跑；near/far 统一 transaction、延迟预算、S6 route 与 S7 收口待实施。[作战任务](../../10-active/voxel-far-field/2026-07-12-a10-cancellable-incremental-voxel-shell-streaming.md) |

**A 退出门槛**：

- 正常 WorldGen 可玩场景无需 CLI 即完成 loading→playable，near/far/ownership/fence 同代 live，真实 pawn 默认驱动 XYZ auto-follow；
- pure-3D actor 只依赖显式 provider + source-neutral builder；A10 已实跑请求式 WorldGen、scripted 与 H-gated local provider。local 只消费开发包并只接 far；服务器/HTTP/在线 authority provider 尚未实现并后置；
- adjacent recenter 结构化证明 overlap page/artifact 复用、dirty patch 构建与 stale worker 取消，不再全量重建约 3.3 万 pages；
- X/Y/Z、对角、快速折返、传送和长巡航全部满足 `gap=0 / overlap=0 / stale_commit=0`，并报告 provider/worker/GT 分位数；
- opaque/dither/透明/发光 world-aligned material family 与 near/far 同点 audit 完成；
- 用户操作、automation、CLI/observe 三入口闭环。A 不修改服务端，也不冻结生产 7m page 契约。

当前上位 active 计划见 [`2026-07-12-pure-3d-voxel-shell-migration.md`](../../10-active/voxel-far-field/2026-07-12-pure-3d-voxel-shell-migration.md)，A10 当前执行稿见 [`WorldGen 驱动的完整客户端 3D 滑动世界`](../../10-active/voxel-far-field/2026-07-12-a10-cancellable-incremental-voxel-shell-streaming.md)。

### 里程碑 B：生产投影契约冻结 + 客户端 fixture oracle（未开始）

| 步 | 内容 | 备注 |
| --- | --- | --- |
| B1 | 冻结 T-4 固定 7m page/直接整数规约、T-11 失效/HTTP 语义、T-12 required-set/shard manifest 与稳定失败分类 | 只写客户端/契约，不改服务端 |
| B2 | fixture 生成器产真 7m projection；14/28/56m 全部直接从同一 7m base 规约 | 通用 v2 codec 只作过渡/测试适配层 |
| B3 | 分拆 1m near exact 与 7m far source provider/batch/source fingerprint，scene builder 真消费双投影 | 不引入 raymarch/SVDAG renderer 任务 |
| B4 | fixture invalidation、page 级 dirty/rebuild、旧 live stale 与新 generation 时序 | 失败不得 partial publish |
| B5 | XYZ sparse shard/coverage fixture、磁盘 H gate、正负向 X/Y/Z Real-RHI oracle | 客户端成为 C 的验收靶子 |

**B 验收**：固定契约 fixture 包驱动 page→规约→surface/mesh→generation→scene host 全管线，正负向 gate、失效和三轴移动全绿。此时客户端才成为服务端接入 oracle。
...
自研 SceneProxy（触发：分组件路径仍有可测 hitch/显存瓶颈）；按环差异化容器（触发：远环管理成本成为可测瓶颈）；**L4 第五环扩圈**（仅在 §3.3 三条件命中后另立阶段，renderer 重新选型）；Nanite bake farm（触发：editor 手工 A/B 数据显著）；Event Overlay；同构路线定向加法；隐秘区域的 pages/pack 准入规则。**raymarch 不在 defer 清单内。**
...
- v2.3（2026-07-06）：落术语表 [`glossary.md`](../protocol/glossary.md)（base / delta / truth / snapshot 四词口径 + 客户端 snapshot-only 推论 + 远区修改回流回路），文首加指针；无技术内容变更。
- v2.4（2026-07-06）：①数据源终态拍板落稿（[`2026-07-06-projection-route-final-decision.md`](../contracts/2026-07-06-projection-route-final-decision.md)）——投影路线为终态，§3.2b 数据源选型不再待拍板；②L4 从"可选 profile"改为 **defer**（§3.3 重写：第五环与 raymarch 保温两决策拆开、三条触发条件写死、启用形态=低一致性档）；③B1 增加"coverage radius 为 manifest 一等字段"硬要求；④B3 SVDAG payload 构建标注为 raymarch AB 保温用途；⑤defer 清单补 L4 扩圈与同构路线定向加法两条。
...
- v2.7（2026-07-12）：根据已落地的 near/far 流送优化、XYZ cube-shell、canonical pages、resolved surface、source-neutral scene builder、generation fence/host 与高空 Real-RHI，扩展里程碑 A 为 A1-A10；A10 当前进行中，B/C 均未开始。垂直三维 coverage 从旧 B5 前移为 A8；B 改为生产 1m/7m 投影契约与 fixture oracle。真实 RHI 再次复现 compute 队列超时后，raymarch 从 B/defer/AB profile 全部移除，L4 只保留需求触发器且 renderer 重新选型；同时明确 §2-8 的 SVO/占据层/二维 near-skip 文字仅为兼容路径或旧提案输入，不是已冻结的生产 artifact 契约。随后为 A10 增补独立执行稿，明确 tile 差集、cooperative cancellation、verified residency、依赖感知 DAG 与 stable patch transaction，不改变 A/B/C 边界。
- v2.8（2026-07-12）：用户实机确认 pure-3D 入口仍是静态式 WorldGen 整壳，旧客户端已有的完整场景、near/far 滑窗和数据泵没有迁入新组合根。A10 因此重排为 WorldGen 客户端闭环优先：先完成正常入场 lifecycle、同源 near/far、真实 pawn 自动 XYZ 滑窗、请求式 page residency，再完成取消、依赖 DAG、stable patch 与三轴长巡航。现有 H-gated 原子磁盘 batch loader 保留；尚未实现的磁盘 request/live provider 与服务器 provider 退出 A10 关键路径。以后接入只能替换 provider，不改变 A/B/C authority 边界。
- v2.9（2026-07-12）：落地运行时“唯一根事实”。普通 WorldGen 场景只启动 `AVoxiaUnifiedVoxelWorldActor` / `production_all_features`，把成熟 near 滑窗/数据泵与 Pure3D far 纳入同一顶层 root；standalone legacy/VHI/SVO/Pure3D 均降为显式 probe/compatibility。根级 CLI 要求 near settled、far live 与 XYZ center aligned，并通过地面/高空 Real-RHI。当前模块组合仍未共享 provider/residency/generation，full build 成本仍为 12-19s，因此 A10 状态改为 S1a 完成、S1b/S2-S7 进行中，而非宣称增量流送完成。
- v2.10（2026-07-12）：纠正 H-gated 磁盘能力的实现状态。已完成项只有 `FVoxiaCanonicalVoxelPages::LoadExpectedBatch` 原子磁盘 batch loader；磁盘 request/live provider、网络/HTTP provider 与服务器 provider 均尚未实现。A10 只冻结 provider-neutral 客户端边界，A 退出后才按 B 冻结的 page/manifest/失效契约实现磁盘 adapter。
- v2.11（2026-07-12）：同步 A10 S2-S5 实现事实。Pure3D far 已接 WorldGen/scripted request provider、required/keep/enter/exit planner、immutable residency/lease/LRU、cooperative cancellation、dependency-keyed material/surface cache 与 absolute XYZ `32³` stable-patch real-fence transaction。default 相邻 +X 只请求 `1517/33752` pages，保留 `175/216` far patch，Null/Real-RHI 均 `53/53` geometry components visible。near/far 仍未共享 transaction，artifact 延迟仍为 `6-9s`，故 A10 与 B/C 边界不变。
- v2.12（2026-07-13）：用户要求只后置服务器相关工作，并先完成本地加载 provider。A10 新增并落地外部 H + expected identity 的 immutable manifest open gate、H-gated `local_disk` request provider、provider-neutral Build 绑定、唯一根显式选择与本地 pack 生成入口。default 本地包冷窗读取 `33752` 页，相邻 +X 只读 `1517` 页并复用 `32235` residency keep；错误 H 根级 source authorization 硬失败、零发布且无 WorldGen fallback。dense wire 的统一材质页解码后恢复 compact storage，驻留回到约 `19MB`。成熟 near 仍用 WorldGen，故 mixed source 与 near/far transaction 缺口保持可见；B/C 边界不变。
- v2.13（2026-07-13）：同步 A10 S4 性能首轮。material/surface cache 改为 source-bound immutable shared refs，resolved surface 使用后台优先级 `ParallelFor`；actor 用 `TFuture::Consume()` 移交 generation，coverage diff 在 worker 执行，旧 coverage 在 worker 析构，residency v2 以 generation reverse index + 每 tick `1024` 页预算释放 lease。相邻 Real-RHI provider/artifact/mesh/total 约 `150-174/553-563/195-211/910-949ms`，far GameThread prepare/finalize/publish 分段约 `4.5-7.5ms`。快速折返只提交 latest generation；仍缺 near/far 同一 transaction、反向依赖/full oracle、离群帧上界和完整 route，B/C 边界不变。
