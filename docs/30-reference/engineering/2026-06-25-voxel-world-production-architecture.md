...
- **tile**:生产流式预算口径中,一个 tile 定义为 `7×7×7` 个 chunk,即 `343`
...
- 正常游戏内热路径不应承载"27 个大 tile 全 cell 变更"这种全量更新。大体素包、
  世界基线、大范围重写和资源补丁属于启动器/更新阶段或入场前阶段。
- 进入场景前必须校验本地 world pack / region manifest / chunk baseline / diff
  chain。校验失败视为客户端数据不可被信任,必须拒绝进入场景;禁止用运行时
  snapshot/resync 兜底绕过。
- 进入场景后只流送已校验基线之上的小规模 runtime diff、语义 diff、prefab/object/event
  diff。
- 数据量偏大的优化项先作为后续风险记录;等实际遇到带宽/CPU 瓶颈时,必须用
  observe/CLI 统计 `tiles_changed`、`chunks_changed`、`ops`、`bytes`、`encode_ms`、
  `send_queue_bytes` 后再针对性设计,不提前把它认定为当前可操作区域不刷新或编辑无效的主因。

```mermaid
flowchart LR
  Launcher["启动器 / 更新阶段<br/>下载 world pack / 大范围补丁"]
  PreScene["入场前阶段<br/>校验 manifest / baseline / diff chain"]
  Runtime["场景运行时<br/>只流送 runtime diff / semantic diff"]
  Reject["拒绝进入场景<br/>可诊断错误,不做 snapshot 兜底"]
...
- **active-window 差集订阅(消灭 A1/A4)**:gate 侧由 per-connection `SubscriptionWorker` 持有订阅集唯一真相。每次 subscribe/reconcile 表示“当前完整可编辑近场窗口”,worker 先退订 `old_window ∖ new_window`,再只对 `new_window ∖ current_subscriptions` 做 route+subscribe。客户端 `subscribe_voxel_around` 同样只对真正缺失/新增 chunk 发起;known[] 只携带新进框内摘要而非整个 session store;`ChunkProcess.subscribe` known 命中时不再无条件 `encode_snapshot_payload`(chunk_process.ex:1086)。
...
**最终设计:控制面小状态行级 ledger + chunk 稀疏表示 + 显式 materialization/import + 客户端内存/磁盘分层。**(主导=Implicit-Partition 独有的小状态行级 upsert + 稀疏 chunk；2026-06-28 后不再把运行时 WorldGen 作为生产兜底。)
...
- **显式 materialization/import(消灭 S1)**:chunk 进程首次访问且 DB 无行时不再生成生产基线,而是返回可诊断的 missing authoritative snapshot 错误。world pack、真实地图导入、一次性 migration 或开发期 `DevSeed` 必须在运行时订阅/编辑前把 canonical snapshot 写入 store,并由 `ChunkSnapshotStore.put_snapshot` 同事务维护 LOD projection。`WorldGen` 只保留为显式 dev/migration helper,不能作为生产 runtime fallback。
...
- step 1.5:`DevSeed` 退化为"spawn 周围显式 materialization 少量 region 内容"的开发/demo 工具,不再定义边界；其内部可以调用 WorldGen helper,但必须在运行时订阅/编辑前写 canonical snapshots。
- **消灭**:W1、W2、W7。
...
### 阶段 3 — 显式 materialization/import + chunk 生命周期

- step 3.1:world-pack/import/dev seed 在入场或启动前写入 canonical chunk snapshots；chunk 首触达且 DB 无行时显式失败,不做运行时生成。
- step 3.2:chunk 进程 LRU/idle 驱逐 + lease 撤销 quiesce。
- step 3.3:`ChunkDirectory` monitor chunk pid,崩溃 emit + ChunkInvalidate 广播。
- **消灭**:S1、S2、S3。
- **风险**:中(materialization/import 调度、baseline 校验与加载版本语义须统一)。
- **验收**:走到已 materialized 的全新区域能订阅/编辑；未 materialized 或 baseline 校验失败时返回可诊断错误并拒绝进入/订阅；万级 chunk 内存有界；杀掉 chunk 进程后订阅者收到 invalidate 并重订阅。
...
- `apps/world_server/lib/world_server/voxel/dev_seed.ex` + `default_region_bootstrapper.ex`:退化为开发/demo 的显式 materialization 桥,启动时写 starter chunk snapshots 并维护租约/LOD projection；长期由 launcher/world-pack/import pipeline 接替。
...
### 场景 chunk(Scene)
- `apps/scene_server/lib/scene_server/voxel/chunk_process.ex`:启动只接受 canonical snapshot / provided storage；缺权威 snapshot 显式失败并 emit `voxel_chunk_materialization_failed`；WorldGen/empty 仅显式 dev/test opt-in；所有写路径产 delta；snapshot 持久化同步走 `ChunkSnapshotStore.put_snapshot`，并在同一事务 upsert LOD projection。
- `apps/scene_server/lib/scene_server/voxel/chunk_directory.ex`:monitor pid + ChunkInvalidate；`prewarm_handoff` 先读 canonical snapshot，缺 snapshot 显式失败并 emit `voxel_migration_prewarm_missing_snapshot`；`subscribe_many` 批量接口。
...
- step1.5 `DevSeed`(commit `5497705`,2026-06-28 语义修正):退化为 grid 上的显式 materialization,不再定义盒子(消除盒边与
  grid region 重叠 bug);footprint 跨 4 个 grid region,每 chunk 用其 region lease 经 Scene 写 canonical terrain snapshots。
...
阶段3 显式 materialization/import + chunk 生命周期 → 阶段4 AOI 增量 + 非阻塞 → 阶段5 服务端权威编辑硬化 + 反作弊(无客户端预测)→
...
### ⚠️ 阶段3 step3.1 + 阶段7-bis(2026-06-28 修正)—运行时 WorldGen 已降级为显式 dev/migration materialization
**阶段3 step3.1 的旧“运行时 WorldGen”结论已废止**:
- `SceneServer.Voxel.WorldGen` 仍保留分形 value-noise 地形生成能力,用于开发期、demo、一次性 world-seed migration 或未来真实地图导入前的占位 materialization。
- `ChunkProcess` 生产默认只接受 canonical snapshot / provided storage。DB 无行、snapshot 损坏或 store 不可用时,启动失败并 emit `voxel_chunk_materialization_failed`；测试/dev 可以显式 opt-in 到 empty/worldgen policy,但不得静默混入生产 runtime。
- 游戏内不再采用“传种子 + 只存改动”的兜底模型。客户端本地 world pack / region manifest / chunk baseline / diff chain 必须在入场前校验；校验失败拒绝进入场景,禁止用运行时 snapshot/resync 自愈绕过。
- UE 客户端已接入入场前本地包加载:manifest 版本校验通过后分页读取 `/ingame/voxel/world_diff`,持久化审计 JSONL,并把每个 chunk snapshot 规范化为本地随机访问 `chunk_snapshot_frame_0x62_v1` 文件；进场订阅前先把当前可编辑窗口加载进 `VoxelStore`,缺本地 chunk 或解码失败即阻断订阅并输出诊断。
- 服务端已补真实 WorldGen world-pack 生成入口:`WorldServer.Voxel.WorldPackBootstrapper` 在 `VOXEL_WORLD_PACK_GENERATE=1` 时按 `VOXEL_WORLD_PACK_CHUNK_MIN/MAX` 配置的范围批量调用 `WorldPackMaterializer` → `SceneServer.Voxel.WorldGenMaterializer`,写 canonical chunk snapshots 与 LOD projection,全部成功后才把 manifest 发布为 ready。默认不开启,避免误把完整 32km 生成塞进本地启动；完整 32km 包仍需要部署阶段显式配置范围、预算和离线生成窗口。
**阶段7-bis 服务端启动 / demo materialization**:
- `DefaultRegionBootstrapper` 的开发/demo 默认改为 `seed_terrain?: true` 且 `rebuild_lod_projection?: true`：服务端启动后通过 `DevSeed` 路由出生点 footprint、续发租约、经 Scene 批量 intent 写 starter chunk snapshots,并显式 rebuild/backfill LOD projection。
- 这是一条开发/demo 的显式 materialization 桥,不是生产进入场景后的全量同步。正式产品里大体素包、全量 tile 更新、真实地图导入和 baseline 校验归 launcher / 入场前阶段。
- 预热链:BeaconServer join_cluster 固定 sleep(1000) → 轮询 peer(50ms)+ give-up 250ms(早返回)。
  历史实测 `voxel-ready 4.6s→2.78s(≈gate-ready)` 仍可作为预热链优化证据,但不再支持“运行时缺块可由 WorldGen 兜底”的结论。
- 本轮数据量口径按 `1 tile = 7×7×7 chunks`、`27 tiles = 3×3×3 tiles` 记录。数据量大的问题后期实际碰到再通过 observe/CLI 量化,当前不作为 streaming/editability 缺陷的默认解释或阻塞项。

### ✅ 阶段3 step3.2/3.3 — chunk 生命周期(大世界内存有界)
- step3.2 idle 驱逐:ChunkProcess 无订阅者 + 无活跃 field region + 无 pending fence,连续 idle 达
  阈值即自停(:normal)。已编辑 chunk 编辑路径同步落库；再访问必须从 canonical snapshot 重载,缺 snapshot 显式失败,不由 WorldGen 重生成。默认禁用(单测),dev/prod config opt-in
...
- **阶段3 部分落地**(严格 snapshot materialization + chunk 生命周期);idle 驱逐 3/0、chunk_directory 11/0、
...
- **`GateServer.Voxel.SubscriptionWorker`(新,step4.3 非阻塞核心)**:**per-connection GenServer**,
  ...
  热路径零改动);路由失败由 worker 发回 connection 编 `0x68` error 帧(`reason: inspect(reason)`,
  保留失败测试字节)。worker 持 `RouteCache`(region 级缓存即 step4.4 批路由收益:一个 slab 命中同
  region 只打一次控制面)。
- **active-window 差集(step4.2 当前版)**:connection 不再保存权威 `voxel_subscriptions`/`voxel_pending`。
  worker 的 `subscriptions` 是当前 active/editable window 的唯一状态。新 reconcile promote 时先按最新
  `center+radius` 裁剪同 scene 下窗口外旧订阅(`old_window ∖ new_window`),再订阅窗口内缺失 chunk;
  already-subscribed 且非强制 known 的 chunk 不重打 ChunkDirectory。

### 9.3 关键取舍与契约保全

- **快照即 ACK**:订阅成功路径**不发**结果帧(现状同),快照由 worker 订阅触发经 fan-out 异步到达;
  失败走 worker→connection 的 `0x68`。`assert_receive` 对异步到达天然容忍。
- **introspection 不破**:connection 只持 `voxel_worker` pid,debug/stdout/test 统一调用
  `GateServer.Voxel.SubscriptionWorker.subscriptions/1` 读取当前订阅集。**消息序保证**:
  ChunkProcess 在 `subscribe` 调用内先 `send` 快照给 connection,worker 随后在自身状态记录订阅;
  测试收到 `0x62` 后再同步读 worker，必见已落库的订阅。
...
- **step4.3 非阻塞核心**:`GateServer.Voxel.SubscriptionWorker`(新,per-connection GenServer,与
  ...
  订阅,快照/delta 仍直达连接→socket(fan-out 热路径零改动);失败经
  `{:voxel_subscribe_failed,...}` 回连接发 `0x68`。阶段2-bis 的 per-region route 缓存随之移入 worker。
- **step4.2 active-window 差集**:worker 持 `subscriptions` 唯一真相。订阅框 `diff = 新窗口 ∖ 当前订阅集`,
  只把新 chunk 投 ChunkDirectory;promote 最新窗口时先退订同 scene 下窗口外旧 chunk,使服务端可操作区
  跟随玩家当前 center,不再依赖客户端显式尾框退订。
...
- **修复 subscribe/unsubscribe 乱序竞态**(异步必然引入):订阅状态和 Scene 订阅/退订同属 worker 单进程,
  同 chunk 的 subscribe/unsubscribe 严格定序;窗口裁剪、显式 unsubscribe、invalidate 都通过 worker
  发起 Scene 侧幂等退订。
...
### ⚠️→✅ 阶段 4 对抗式评审(16-agent)修正:订阅状态单一所有者(2026-06-25)
**首版(commit ba06994)把订阅集留在连接、Scene I/O 在 worker**——16-agent 对抗评审确证 **6 个真实并发
bug**,同源于「`voxel_pending` 无代际 MapSet + 连接与 worker 双发起 Scene 操作」:退订/重订在途交错
(worker route 慢路径窗口宽达数秒)下,worker 回报 `{:voxel_subscribed}` 错配 pending、`scene_unsubscribe`
误删 pid-keyed 单成员订阅 → **静默死订阅**(连接自认已订阅、差集永不重投,但 ChunkProcess 已 0
subscriber → chunk 客户端永久黑块)。另:ChunkInvalidate 不清连接订阅 → 差集吞重订(评审 F5);流式
首失败语义漂移(F6)。

**修正(重写 SubscriptionWorker)**:**worker 成为订阅集唯一所有者 + Scene 订阅/退订唯一发起者**。
单进程串行处理 reconcile/unsubscribe → 同 chunk 的 subscribe/unsubscribe 严格定序、订阅集与 Scene
注册始终一致,**整类乱序竞态结构性消失**(无需代际 tag)。连接退化为纯转发(只持 worker pid),
introspection/debug/rebind 走 worker 同步 `call`;rebind/`subscriptions` 移入 worker;ChunkInvalidate
解码 chunk 后 `invalidate_chunk` 丢订阅(修 F5);reconcile best-effort 不 halt + 首失败带 chunk_coord
(修 F6)。worker `Process.monitor` 连接(`:normal` 退出不穿 link)防 orphan。
- **验证**:核心订阅用例(失败路径/路由/差集 no-op/快照→delta/退订)隔离 **7/0 × 3 seed** 稳定;
  gate 全套 **229/0**。rebind/prefab/edit 的顺序-时序 flaky 经 stash 对比确认 **ba06994 同样存在**
  (异步时序差异只改变触发 seed),非本次引入。
- **教训**:异步化订阅必须让「订阅状态 + Scene 操作」同处一个串行进程(单一所有者),否则双发起者
  在 pid-keyed 幂等订阅上必有退订/重订乱序 → 静默死订阅。这类 bug 测试难复现(窗口靠慢 I/O),
  对抗式评审 + 交错推演是发现手段。

### ✅ 阶段 4 第二轮复审(13-agent)修(2026-06-25)
对 single-owner 版再审,确证 5 个真实点(1 high + 1 med + 3 low),已全修:
- **F1(high)`invalidate_chunk` 跨进程乱序**:ChunkInvalidate 从 Scene 端发出,与 worker 自己的
  subscribe **跨进程**,二者在 ChunkProcess vs worker 上可乱序提交 → 若 worker 收 invalidate cast 前
  刚(因 AOI)重订阅该 chunk(map 有、Scene 有),仅删 map 会留 Scene 侧静默死订阅。**修**:
  invalidate_chunk **主动对 Scene 幂等退订**再删 map,保「map==Scene 注册集」;客户端按 0x69 重订重建。
- **F2(med)subscribe >15s 超时泄漏**:ChunkProcess 在快照编码前就 put_subscriber,gate call 超时
  返回 error → worker 没记录 → 日后退订漏退 Scene 泄漏。**修**:`Routing.subscribe` 区分 `:timeout`,
  worker 超时按「可能已订阅」**记录订阅**(日后幂等退订无害),不发 0x68(快照已异步直达)。
- **F3(low)WS 非正常 close 记 crash**:浏览器断线(`:remote`/`:timeout`/`{:error,_}`)非 :normal 退出
  → crash 日志噪声 + worker 靠 monitor 自停的通道被 link 信号抢先。**修**:`normalize_close_reason`
  全归一 :normal(对齐 TCP)。
- **F4(low)debug rebind 漏清 route 缓存**:public cast 路径清、debug 探针路径漏。**修**:清缓存移进
  `do_rebind`,两路径一致。
- **F5(low)stdio_interface 读已删 `voxel_subscriptions` 字段恒空**:**修**:改向 worker 取(`:exit` 兜底)。
- 验证:gate 全套 229/0;核心订阅 7/0。

### ✅ 2026-06-28 运行时窗口跟随修正:latest-window prune + LOD 投影热路径

用户现场复现“绿色可操作区域永远停在出生点,且在绿色边缘点击仍无效”后,确认服务端还有一个
active-window 归属缺口:latest-wins 只替换 pending tail,但已经成功订阅的旧窗口 chunk 会继续留在
worker `subscriptions` 内。这样 debug/introspection 会把历史订阅累积集误看成当前可操作窗口。

**修正**:

- `SubscriptionWorker.reconcile/2` 语义明确为“当前完整 active/editable chunk window”。
- promote 最新 reconcile 时,worker 先按最新 `center+radius` 计算 desired set,对同 logical scene 下
  不在 desired set 的旧 subscription 执行 Scene 幂等 `unsubscribe`,再继续订阅新窗口缺失 chunk。
- 新增 `voxel_reconcile_pruned` observe,字段包含 `connection_pid`、`request_id`、`logical_scene_id`、
  `center_chunk`、`radius`、`desired_count`、`pruned_count`、`subscription_count`、`pruned_sample`。
- `sync_reconcile_for_debug/2` 使用同一裁剪语义,避免 stdio/debug 探针和真实运行路径分叉。

**同时修正的性能根因**:

- `SceneServer.Voxel.LodProjection` 原来为一个 chunk 的各 stride cell 反复调用
  `AuthoritativeHeightmap.heightmap_region(..., source: :snapshot_store, ...)`,等价于对同一 authoritative
  snapshot 反复 decode/聚合。snapshot forwarding/edit ACK 会被这段重复工作拖慢。
- 当前实现先一次读取该 chunk 的 `16x16` fine heightmap,再在内存中聚合 `2/4/8/16` stride cell。失败仍显式返回
  `{:lod_projection_height_failed, reason}`,没有添加 runtime snapshot/resync 兜底。

**验证**:

- `cd apps/gate_server && mix test test/gate_server/ws_connection_voxel_test.exs:343` -> 1/0,覆盖
  “新 center radius 0 会退订旧 center,旧 ChunkProcess subscriber_count=0,新 chunk subscriber_count=1,
  observe 含 voxel_reconcile_pruned”。
- `cd apps/gate_server && mix test test/gate_server/ws_connection_voxel_test.exs` -> 40/0。
- `cd apps/scene_server && mix test test/scene_server/voxel/lod_projection_test.exs test/scene_server/voxel/lod_projection_rebuilder_test.exs` -> 5/0。
