...
- 2026-08-06（晚）：用户在 1920×1080 可见生产根窗口试玩并确认"手感目前看起来不错"，§10 完成条件第 7 项闭合，**本计划全部完成**。封闭地下口袋 presentation 停滞已记入 `known_gaps.md` 作为下一会话候选任务；吸附 CPU 缓存为可选优化。客户端分支与外层 master 已推送。
