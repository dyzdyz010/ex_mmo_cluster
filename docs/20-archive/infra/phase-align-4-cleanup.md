...
> 上层索引:[`2026-06-14-architecture-triage-and-alignment.md`](../voxel-authority/2026-06-14-architecture-triage-and-alignment.md)
...
**判定(2026-06-14 具体评估后):单 facade 受 Rustler 0.37.3 硬限制无法做;workspace 部分步风险/收益失衡,
原则性推迟,不动现状。** `native/` 现为 5 个独立 crate(`movement_core` rlib edition2021 + `scene_ops`/
`octree`/`field_kernel`/`movement_engine` 4 cdylib edition2018,各自 `rustler::init!` 一个 Elixir 模块),
无顶层 workspace。

**评估结论(读全 5 个 Cargo.toml 后)**:
1. **MOD-3 核心目标"单 NIF facade"被 Rustler 0.37.3 硬挡**:`init!` 一次仅一个 Elixir 模块,合并需自定义
   unsafe FFI 或 Rustler 大版本升级——超出对齐迁移范围。
2. **workspace 部分步(`native/Cargo.toml [workspace]` 协调 member/Cargo.lock/target)风险大于收益**:
   - **panic 安全 profile 隐患**:cargo 在 workspace 下**只认根 `[profile.*]`,忽略非根 member 的 profile
     并告警**;现 4 个 cdylib 各自的 `[profile.release] panic="unwind"`(NIF-11/15 守卫)若不正确上移到根,
     **会被静默忽略 → panic abort 杀 BEAM scheduler 的承重安全契约悄然回退**。
   - **Windows 重建风险**:改 cargo 结构强制 NIF 全量重编;本机 NIF 编译需 VS Dev Cmd(`cl`/`link` 在 PATH),
     当前 shell 未必激活 MSVC → 重建可能因环境失败(与 workspace 正确性无关),且留下损坏构建。
   - **收益仅 Cargo.lock/target 去重**(纯依赖协调),非任何承重契约。
3. **承重已满足**:BND-1 数据入 Rust(2.7c ResourceArc<CellSim>)/ NIF-1 节点级 SimRuntime / NIF-11/15
   各 crate panic=unwind 已配 / NIF-12 已删 coordinate_system;MOD-4 [v2.0.2] 亦允许"stage 调度在 Elixir、
   Rust 仅算力单元"合规变体。

**结论:不引入 workspace(避免为 cosmetic 去重承担静默破坏 panic 安全 + Windows 重建风险)。单 facade 显式
记为 Rustler 0.37.3 版本受限的 future——待 Rustler 大版本升级支持多模块 init 时再做。**

## 进度日志(时间倒序)

- 2026-06-14:**4.2 / 4.3 评估定案**。4.2 data_init:**保留**——已是非 live 迁移工具(无 supervision tree、
  编译期依赖、`mix migrate_to_pg` 需 `TableDef` 读旧 Mnesia),ANTI-8/§16(live 路径无 Mnesia)已满足;
  生产 Mnesia 数据退役前删除会破坏迁移 task。4.3 cargo workspace:**不引入**——MOD-3 单 facade 受 Rustler
  0.37.3 硬限;workspace 部分步会静默忽略非根 panic profile(NIF-11/15 隐患)+ Windows NIF 全量重建风险,
  收益仅 cosmetic 去重,承重已满足。详见上方各节评估。**至此梯队 4 收尾清理告一段落:可删的(agent_*/
  data_store/data_contact/coordinate_system/WriteTokenStore 垫片)已删,余 2 项为外因阻塞的原则性保留/推迟。**
