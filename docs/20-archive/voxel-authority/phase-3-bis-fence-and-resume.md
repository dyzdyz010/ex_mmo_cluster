...
| 3-bis-6 | docs 同步:`apps/scene_server/lib/scene_server/voxel/README.md`、`apps/world_server/lib/world_server/voxel/README.md`、`docs/10-active/cross-cutting/voxel-server-authority-phase-overview.md`、协议设计文档 §11(若涉及新表)。决策稿进度日志补每 step 实施期 RFC | 文档与代码一致;`docs/10-active/cross-cutting/voxel-server-authority-phase-overview.md` 阶段表 Phase 3-bis = 已完成 |
...
- 2026-05-08: 决策稿入仓(commit `5e3b1e7`)。D1–D8 推荐值用户已确认。
- 2026-05-08: **Phase 3-bis 全程落地**(fence persistence + auto-resume commit)。
  - **Step 3-bis-1**(commit `5cadbdf`):新建 `voxel_chunk_pending_transactions` schema +
    migration + `DataService.Schema.VoxelChunkPendingTransaction` Ecto schema +
    `DataService.Voxel.ChunkPendingTransactionStore`(stateless module:`put_fence/2`、
    `get_fence/3`、`delete_fence/3`、`snapshot/1`)。`unique_constraint` 触发
    `:fence_already_present`,`:safe binary_to_term` 反序列化 intents。data_service:
    41 → 53 tests。
  - **Step 3-bis-2**(commit `f6602b0`):`ChunkProcess.prepare_transaction/3` → `/4`
    新增 `:decision_version` opt 透传(`BuildTransactionApplier.prepare` opts +
    `ChunkDirectory.prepare_transaction` attrs 一并扩展)。prepare 同步 INSERT、
    commit/abort 同步 DELETE、init load + lease 一致性校验(`owner_*` 4 字段对齐),
    不匹配视为孤儿 DELETE + emit `voxel_chunk_pending_transaction_orphaned`。
    新建 `chunk_process_persistence_test`(9 case)。scene_server: 268 → 277 tests。
  - **Step 3-bis-3**(commit `d767c29`):`BuildTransaction` struct 加
    `intents_by_participant` 字段,`TransactionCoordinator.begin_transaction` attrs 接收
    `:intents_by_participant`(非 map 拒为 `:invalid_intents_by_participant`),
    `begin_fingerprint` 不算 intents(replay 不 burn intents 内容)。字段随 transactions
    map 一并写入既有 coordinator 单行 snapshot,reload 完整保留。world_server: 48 → 53 tests。
  - **Step 3-bis-4**(commit `9db8c1d`):`TransactionExecutor.execute/4` 加 `:prepared`
    fast-path,`derive_prepare_results_from_prepared_state/1` 从 transaction.participants
    推导 prepare_results(`:prepared` → `{:ok, %{resumed?: true}}`,`:failed` →
    `{:error, :prepare_failed_before_resume}`),emit
    `voxel_transaction_executor_resume_started`。world_server: 53 → 55 tests。
  - **Step 3-bis-5**(commit `d01b3d6`):`TransactionRecoveryWatcher` 对 `:prepared`
    通过 `:scene_opts_resolver`(0-arity fn 返回 `{:ok, executor_opts}`)调
    `TransactionExecutor.execute/4` 重发 commit dispatch。新增 sweep summary keys
    `:resumed_commit` / `:resume_partial` / `:resume_failed`(原 4 keys → 7 keys)。
    `WorldSup.default_scene_opts_resolver` 走 `BeaconServer.Client.lookup(:scene_server)`
    构造 `[scene_opts: [chunk_directory: {ChunkDirectory, scene_node}]]`,未就绪时退化
    为 `:pending_commit` + emit `voxel_transaction_recovery_scene_opts_unavailable`。
    partial commit failures emit `voxel_transaction_recovery_resume_partial`。
    world_server: 55 → 60 tests(预存失败 `authority_observe_test.exs:35` Windows path
    大小写不算回归)。
  - **RFC 备注 / Step 3-bis-5 实施期发现**:决策稿 D6 原文写 resolver 返回
    `{:ok, scene_opts}`(单层 keyword list),实施时改为返回完整 executor_opts
    `{:ok, [scene_caller: ..., scene_opts: ...]}`,直接透传给
    `TransactionExecutor.execute/4`。语义等价(production 默认 resolver 不带
    `:scene_caller` 时 executor 用默认 BuildTransactionApplier),但测试可注入 stub
    scene_caller。已同步更新 Watcher moduledoc。
  - **Step 3-bis-6**(本 commit):docs 同步:`apps/scene_server/lib/scene_server/voxel/README.md`、
    `apps/world_server/lib/world_server/voxel/README.md`、`docs/10-active/cross-cutting/voxel-server-authority-phase-overview.md`
    阶段表 Phase 3-bis 状态置 `已完成`,决策稿进度日志补全每 step RFC + commit hash。
