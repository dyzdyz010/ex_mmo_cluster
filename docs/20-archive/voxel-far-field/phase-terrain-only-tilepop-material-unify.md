...
> ✅ **本文已归档**：T1/T2/T3 与对应 Real-RHI 验收已完成。文中的列源 `Y=0` 只记录迁移期修复，不能作为现役空间契约；当前契约为完整 XYZ，后续实现状态以 A10 / S1b-1 为准。本文的 WorldGen preview 仍属于客户端开发源，不代表在线 authority cutover 已完成；当前纯 3D 路线见 [`2026-07-12-pure-3d-voxel-shell-migration.md`](../../10-active/voxel-far-field/2026-07-12-pure-3d-voxel-shell-migration.md)。
