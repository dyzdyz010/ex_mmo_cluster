...
> 上游：[`2026-07-06-voxia-lod-layering-and-technology-design.md`](../../30-reference/overview/2026-07-06-voxia-lod-layering-and-technology-design.md)（v2.6）§3.2d（T-6 渲染后端选型 + F-1 修订）、§9 里程碑 A 步 A2。
...
- **验收全绿后**：新锚点（FPS/内存/剔除率）按 snapshot 纪律直接改写进 `docs/00-current-truth/design/client/streaming-lod.md`；本文件状态改"已完成"并记进度日志；`README.md` 阶段表同步；A1 §4.1 的 #9/#10 ⚠️ 项在此闭环后回填"已由 A2 解除"。
