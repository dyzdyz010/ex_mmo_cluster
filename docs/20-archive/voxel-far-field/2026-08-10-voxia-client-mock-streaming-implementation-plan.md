...
- [x] **Step 4: 补负坐标、AABB 内部、相同距离、不同世代严格弱序测试并跑绿**
...
- [x] **Step 3: 将 Near index 改为 priority-key 索引**
...
- [x] **Step 4: 让 Near assembly、chunk mesh 与 index 使用同一冻结锚点**
...
- [x] **Step 1: 写 RED，证明 Near 未完成时 Far 不可发布**
...
- [x] **Step 4: 分离 build complete 与 visible publication**
...
- [x] **Step 1: 写 RED，缺任一 Near 事实都不能 Playable**
...
- [x] **Step 2: 跑 runtime contract 测试确认 RED**
...
- [x] **Step 3: 实现唯一 readiness predicate**
...
- [x] **Step 1: 写 RED，expected gap 仍允许 delta**
...
- [x] **Step 2: 跑 coverage tests 确认 RED**
...
- [x] **Step 3: 实现 structural state 与 delta eligibility**
...
- [x] **Step 4: 将 fallback 改为一次性显式 rebase**
...
完成证据（2026-08-10）：客户端提交 `8d8bac7`；RendererCoverage、SceneHostLedger、
UnifiedWorldRuntimePresenter 与 TransactionPresentationRoot 自动化均通过。完整 Null-RHI Mock
跨 tile smoke 通过，首个 Playable 证据为 Near `27/216/9261/9261`、Required Far 完整、
coverage parity=true，增量 eligible/apply=`245/245`、fallback=`0`。
...
- [x] **Step 1: 写 RED，预校验失败不得推进 confirmed revision**
...
- [x] **Step 2: 跑 authority test 确认 RED**
...
- [x] **Step 4: WorldIntent 在副本完成所有校验后统一发布**
...
- [x] **Step 5: 补 reorder、duplicate、session drift 和 token replay 测试并跑绿**
...
完成证据（2026-08-10）：客户端提交 `e26f266`。RED 稳定复现旧路径在 journal correlation
失败后把 confirmed revision `0→2` 且清空 reorder buffer；修复后原子用例 `1/1`、完整
`Voxia.Authority` `21/21`、`Voxia.WorldModel` `15/15`，均为 0 warning / 0 failure。
