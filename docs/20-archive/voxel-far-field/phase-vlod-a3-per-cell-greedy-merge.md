...
> 状态：未开始（决策稿）。
> 上游：[`2026-07-06-voxia-lod-layering-and-technology-design.md`](../../30-reference/overview/2026-07-06-voxia-lod-layering-and-technology-design.md)（v2.6）§3.2c（T-5：per-cell masked greedy merge，作用域限 cell 内）、§4 预算表（I-4 按带系数）、§9 里程碑 A 步 A3。
...
## 9. 结项裁决（Closure，2026-07-07 拍板）

经对本决策稿验收矩阵逐条独立核查（文档 + 代码 grep 双证），**A3 按原口径不算"做完"，但不是"欠 merge 的债"，而是核心前提在 A3.0 诊断中被证伪、须正式改判**。裁决如下：

- **A3.0 诊断线：圆满结项（done，价值兑现）。** 是本 phase 唯一真正落地的产出——不仅完成阈值扫描，还证伪了 A2「overdraw=device-removal」归因、定位并**修复**了真崩溃、经阴/阳性对照 + 代码路径二分 + DRED-lite 把因果钉死。唯一未闭合项=**精确 faulting 指令不可观测**（Heisenbug + 禁用 `-gpucrashdebugging` 双重物理封死），**定性封存、非债务**，勿当"诊断没做完"重开。

- **§5.B 渲染层（device-removal 消除 / FPS）：判 done-by-other-means。** far-mesh-go-live 崩溃**已解决**——由 `clients/Voxia@1fc93d2`（`ShouldEnableSvoRaymarch()` 默认关）解决，**不是 merge**。merge 与该崩溃被实测证正交（r16=0.49M 与 r72=1.39M 同样崩，减 quad 修不了）。**正式采纳 `1fc93d2` raymarch 默认关为生产终态。** 风险锁死：强开仍活是脆弱偶然（布局相关潜伏 UB / graphics×async-compute 跨队列时序竞态），将来重启 L4 超远景 raymarch 前必须先根因修跨队列同步（barrier/fence/避 async-compute 重叠）或用 `-VoxiaSvoRaymarchDeferInitialProbe` 跨帧解耦，不可依赖偶然掩盖。

- **A3.1 merge 本体（承重墙）+ §5.A 几何层：完成度 0（从未开工），但 merge 保留、迁出、另立门户。** 用户 2026-07-07 拍板 **merge 仍要做**（其几何/带宽/内存收益成立），但**从 A3 的「device-removal 承重墙/必达」叙事中迁出**——前提已换（不再挂靠崩溃根治），须重写目标与验收矩阵。承接 phase = **[`phase-vlod-a3b-per-cell-greedy-merge.md`](phase-vlod-a3b-per-cell-greedy-merge.md)**（目标=纯几何/带宽优化）。原 §3.1-3.4 scope、§5.A 几何验收矩阵、决策项 **D1/D2/D4/D5 整体迁入 A3b**；**D3 作废**（其「merge 不足则重挂 A5/雾/Lumen」框架随前提一并失效）。

- **一句话**：A3 诊断线结项、崩溃已由 raymarch-off 修复并采纳为生产终态；merge 不丢弃，作为**独立几何/带宽优化**转入 A3b 正式推进。A3 本 phase 到此关闭，后续 merge 工作不再复用本稿的 device-removal 叙事。
