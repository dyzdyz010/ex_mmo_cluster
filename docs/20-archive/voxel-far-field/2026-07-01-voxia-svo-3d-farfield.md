...
- 2026-07-01：**Step C + D 委派 Fable 模型 agent 实现**（用户指定）。C=跨 tile 增量流送（镜像 VHI `BuildWorldGenTileUpdate`+`FVoxiaVhiReuseContext`+`VhiPatchSection`/`FreeVhiSections` 增量上传，`PlanIncremental`，per-macro-cell 缓存，`cache_hit_rate`/`upload_queue` 接真值）；D=真 seam check（dedup/missing/一致性三项，替空壳）+ 近/远 skirt。
- 2026-07-02：**Step E 距离自适应 LOD 环**（Voxia `aa6ae78`，已推）。用户实跑反馈「窗口外全是巨大方块」——根因=深度全场统一（8km 配置 depth=1 → 56m 叶），窗口边第一圈与 1m 近场直接相撞；samples=4 全场等比变细无感。修=启用 SVO 分层细节：cell 深度=基线+距离环加深（近环≤8 tile +2 → ~14m、中环≤24 tile +1 → ~28m、天边基线 ~56m，上限 3）；复用校验升级「邻接签名+LOD 深度」双校验（中心移动时换环 cell 重建，增量==全量保持）；`-VoxiaSvoNearLodRing/-VoxiaSvoMidLodRing`（0/0 关闭）；snapshot 加环字段。automation 23/23（含 8km 关环/开环双口径断言）。SVDAG 三层能力口径：数据复用（子树去重/增量缓存）已用、分层 LOD 本步启用、GPU raymarch 逐像素自适应（DAG 复用全兑现）为并行线已铺前置的下一步。
- 2026-07-02：**与并行 SVDAG/GPU-raymarch/store 源线架构级合并并推送**（Voxia merge `39c0d16`，已推 origin/master；本地线完整历史另存 `svo-3d-farfield` 分支）。推送时发现远端 master 有 6 个并行提交（SVDAG 去重 + GPU runtime payload/RHI buffers/raymarch shader 前置 + **ConfirmedVoxelStore 源**（D-5 preview 版）+ `FVoxiaFarFieldPatchUploader` 指纹级增量上传组件 + baseline entry gate），与本地 SA-SD 同文件大冲突。合并策略=以远端为基座、本地独有价值重注入：SA（WorldGen 3D）无冲突并入且远端 27 点 3D 采样自动吃到岛；SB 重打（root Y 上探岛顶 + -Y 底面）；SD 重打（边界 skirt + 真 seam check，missing 检测限 WorldGen 源）；SC 采远端实现、补**邻接位签名复用校验**（保证增量==全量等价）+ upload_queue 真值；本地 SC 手写上传让位远端组件。验证：automation **23/23 全绿**；跨 tile smoke reused=20706/built=310、cache_hit_rate=0.985、真 seam sample=348037 全 0 pass。
- 2026-07-02：**Step C + D 完成**（Voxia `a260a02`）。Fable agent 完成 C 构建侧（`BuildWorldGenTileUpdate`+`FVoxiaSvoReuseContext`+邻接位签名+稳定聚合序）与 transport 接线后因网络错误中断；主线补完 WorldActor 增量上传（`SvoPatchSection`+`FreeSvoSections`，只清离开 patch/只传 dirty patch，bulk-hide 仅冷启动）、`ReportSvoUploadQueueDepth` 真值上报、Step D 全部（真 seam check 三计数 + near-skip 边界 skirt 24 macro）与增量==全量等价单测。**关键修复**：复用门槛去掉 `SourceVoxelRevision` 比较——SVO 几何是 WorldGen 纯函数（不读 VoxelStore），preview 跨 tile 必 bump revision → 拿它当门槛复用永不命中（实测 reused=0 全量重建）；D-5 切权威 store 源时须恢复 revision 门控（代码+测试注释已标）。**验证**：Voxia.Voxel 15/15 绿；跨 tile smoke teleport +2 tile → `reused=20706/built=310/removed=296, cache_hit_rate=0.985, upload_queue 299→0`（增量真实生效，远景不再整场重刷）。U-2 交付达成。
