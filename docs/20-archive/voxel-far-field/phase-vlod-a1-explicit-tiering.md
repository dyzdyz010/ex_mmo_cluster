...
> 上游：[`2026-07-06-voxia-lod-layering-and-technology-design.md`](../../30-reference/overview/2026-07-06-voxia-lod-layering-and-technology-design.md)（v2.6）§2 分带总表（T-1/T-2 已拍板）、§8 显式 tier 契约（T-10）、§9 里程碑 A 步 A1。
...
| 9 性能底线 | ⚠️ 环境阻塞（A2）| 默认 Lumen/TSR profile 渲染 8km ProcMesh 大代理（单组件 0% 剔除 / 2.78M 三角）稳定 `DXGI_ERROR_DEVICE_REMOVED`；**旧三环 `7@24,28@72`（3.69M quads）逐字复现** → 与 A1/分级无关，A1 反而 −62% 减负；崩溃 breadcrumb 在 Lumen ScreenProbeGather / TSR / TranslucencyLighting 稳态。FPS 锚点归 A2「分组件 StaticDraw + 组件剔除」重构后重测（本项设计上为非门槛底线）。**【A2 部分解除，2026-07-07】** A2 诊断先行 + 8 次真实 RHI 实测 airtight 归因：device-removal 是 **GPU TDR（崩溃现场显存 38%、非 OOM）**，真凶=远景几何量 overdraw（`Lumen+近景=100 FPS` vs `Lumen+8km 远景=崩`）；A2 渲染侧全部杠杆（分组件/StaticDraw/剔除/Unlit/bulk-hide）兑现剔除（look-away 250 FPS）/上传期存活/Lumen-关存活/消尖峰，但默认全 Lumen 8km facing 的 FPS 门槛受 overdraw 物理约束、按 D6(a) 重挂 A3 merge（quad 减半）后复测。硬件实为 4060 Laptop 8GB。 |
| 10 视觉审计 | ⚠️ 部分（结构化替代）| 截图机制在存活/降级 profile 下未落盘（同 #9 环境）；"无洞"由 `seam_check.status=pass`（全量扫 1.39M quad 查缝隙/重复面）+ per-ring 覆盖精确 + upload_complete 覆盖；入带角尺寸目视机位随 A2 修复渲染后补。**【A2 部分解除，2026-07-07】** A2 渲染侧落地后 look-away/Lumen-关 视角可正常渲染取图，但默认全 Lumen 8km facing 截图仍受 #9 同源 device-removal 阻塞（overdraw）；三机位入带角尺寸目视截图归 A3 merge 降 overdraw、渲染稳定后补齐。 |
...
- 验收全绿后：新锚点（quad/per-ring/FPS/内存）按 snapshot 纪律**直接改写**进 `docs/00-current-truth/design/client/streaming-lod.md`（不加日期条目）；本文件状态改"已完成"并记进度日志;`docs/10-active/cross-cutting/voxel-server-authority-phase-overview.md` 阶段表同步。
...
- 2026-07-06：**snapshot 文档与状态收口**。`docs/00-current-truth/design/client/streaming-lod.md` 按 snapshot 纪律直接改写四环实测锚点（quad 1.39M、per-ring、runtime payload 20.5MB、`svo` 字段名 samples→lod_config/lod_rings、legacy flag 移除说明）并新增「8km 环境级 GPU device-removal（A2 待排查）」条目；本文件状态改「实现完成、核心验收全绿」并补 §4.1 验收结果表。`README.md` 阶段表待同步。
