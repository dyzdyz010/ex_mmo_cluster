...
status: obsolete
subsystem: client
superseded_by: AGENTS.md §1 客户端口径（Voxia 焦点）+ 00-current-truth/design/client/streaming-lod.md
obsoleted_on: 2026-07-08
---

> ⚠️ **本文已失效** — 决策稿核心结论是"bevy_client 转主线；web_client 降为参考 oracle"，但据当前仓库 AGENTS.md 客户端口径（Voxia 焦点/web parity/bevy 参考），bevy_client 已在其后（约 2026-06-25 之后）被 Voxia(UE5.8) 取代为客户端焦点、bevy 降级为参考实现，本决策稿的方向被再次反转。 客户端口径已再次反转（Voxia 成焦点、bevy 降参考）；现行口径见 `AGENTS.md §1` 与 [`streaming-lod.md`](../../00-current-truth/design/client/streaming-lod.md)。
