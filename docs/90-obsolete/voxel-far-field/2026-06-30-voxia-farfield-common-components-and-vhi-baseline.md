...
| **S6g（第六片已落地）** | runtime SVDAG resource 数据面：SVO builder 输出 CPU 侧 root table + 去重 node buffer，pack 为固定 64B node / 48B root word payload，由 `FVoxiaSvoRuntimeGpuBuffers` 上传为 RHI ByteAddress buffer；`FVoxiaSvoRaymarchCS` 参数绑定面、独立 `VoxiaShaders` PostConfigInit 模块、最小 dispatch probe 和 preview grid GPU readback 已接入。完整 screen-space GPU raymarch renderer 尚未接入 | `Voxia.Voxel.SvoPreview` 红绿；8km SVO CLI smoke 为 ready=true / runtime payload bytes；WorldActor 日志 `rhi_ready=1` / `rhi_bytes=...` / `raymarch_dispatched=0`（null RHI）；真实 RHI automation dispatch report 为 shader_available=1 / parameters_ready=1 / dispatched=1 / groups=[1,1,1]，preview grid report 为 readback=1 / grid=[4,4] / samples=16 / readback_words=64 | 中 |
| S6+（下一步） | 8km 生产级权威 3D 源全量调度、持久化 artifact、完整 launcher/update 包下载/安装 UI 与 diff-chain 可视化流程、完整 screen-space GPU raymarch renderer；VHI 冻结为过渡 baseline。当前用户要求先不碰服务端，后续仅继续客户端侧渲染生产化 | 分阶段 CLI + real RHI + 服务端 materialization 验收 | 高 |
...
- SVO 持久化 artifact、完整 launcher/update 包下载/安装 UI、完整 screen-space GPU raymarch renderer；客户端本地 baseline pack release-manifest shard 校验和 `ConnectGate` / `EnterScene` entry gate 只覆盖已下载 `.vxpack` 与入场前硬拒，不等于完成包分发链路。runtime SVDAG resource 已创建第一片 RHI ByteAddress buffer，并具备 shader 参数绑定面、最小 dispatch probe 和 preview grid readback，但尚未被完整 screen-space renderer 消费；upload-level section 复用目前只是在 ProceduralMesh patch section 层跳过未变化 mesh，尚未替换为 runtime mesh / HISM / Nanite-ready artifact。
...
- 2026-07-01：**runtime SVDAG payload 第二片落地**。`FVoxiaSvoRuntimeGpuPayload` 将 runtime nodes/roots pack 为固定 stride `uint32` word buffer：node 16 words / 64B，root 12 words / 48B；`SvoSnapshot()` 新增 `runtime_node_word_count` / `runtime_root_word_count` / `runtime_payload_bytes`。验证：先写 payload stride/byte automation 并确认缺类型/方法编译红灯；实现后 `Build.bat VoxiaEditor ... -NoLiveCoding` 退出 0；`Automation RunTests Voxia.Voxel.SvoPreview` 退出 0；`Automation RunTests Voxia.Voxel` 退出 0，17 个测试全 Success；8km null RHI SVO smoke 为 `runtime_node_word_count=58032` / `runtime_root_word_count=252192` / `runtime_payload_bytes=1240896`，与 `runtime_gpu_bytes=1240896` 一致。仍未落地：RHI buffer 创建、global shader 参数绑定、GPU raymarch pass、SVO 持久化 artifact、runtime mesh / HISM / Nanite-ready artifact。
- 2026-07-01：**runtime SVDAG RHI buffer 第三片落地**。新增 `FVoxiaSvoRuntimeGpuBuffers`，把固定 stride node/root word payload 初始化为 render-thread ByteAddress buffer / SRV；`AVoxiaWorldActor` 在 SVO mesh update 路径创建、释放并记录 `rhi_ready` / `rhi_bytes`。验证：先写 `Voxia.Voxel.SvoPreview` RHI buffer automation 并确认缺类型编译红灯；实现后 `Build.bat VoxiaEditor ... -NoLiveCoding` 退出 0；`Automation RunTests Voxia.Voxel.SvoPreview` 退出 0；`Automation RunTests Voxia.Voxel` 退出 0，17 个测试全 Success；8km SVO CLI smoke 为 `runtime_payload_bytes=1240896` / `seam_check.status=pass`，WorldActor queued/streamed 日志为 `rhi_ready=1` / `rhi_bytes=1240896`。仍未落地：global shader 参数绑定、GPU raymarch pass、SVO 持久化 artifact、runtime mesh / HISM / Nanite-ready artifact。
- 2026-07-01：**runtime SVDAG shader 参数绑定第四片落地**。新增 `FVoxiaSvoRaymarchCS` global shader 参数结构、项目 shader source mapping、`Shaders/VoxiaSvoRaymarch.usf` 最小 compute shader stub，并让 `Voxia.Voxel.SvoPreview` automation 从 `FVoxiaSvoRuntimeGpuBuffers` 生成 shader 参数。验证：先写 automation 并确认缺 `VoxiaSvoRaymarchShader.h` 编译红灯；实现中曾尝试用 `TShaderMapRef` 强取 permutation，发现当前 commandlet 下未进入 global shader map，收敛为本片真实范围的参数绑定验证；最终 `Build.bat VoxiaEditor ... -NoLiveCoding` 退出 0；`Automation RunTests Voxia.Voxel.SvoPreview` 退出 0；`Automation RunTests Voxia.Voxel` 退出 0，17 个测试全 Success；8km SVO CLI smoke 仍为 `runtime_payload_bytes=1240896` / `seam_check.status=pass`，WorldActor queued/streamed 日志为 `rhi_ready=1` / `rhi_bytes=1240896`。仍未落地：实际 shader dispatch、GPU raymarch pass、SVO 持久化 artifact、runtime mesh / HISM / Nanite-ready artifact。
- 2026-07-02：**runtime SVDAG shader dispatch probe 第五片落地**。把 `FVoxiaSvoRaymarchCS` 移入独立 `VoxiaShaders` Runtime 模块，`LoadingPhase=PostConfigInit`，用 `/VoxiaShaders` shader source mapping 避免主 game module 过晚注册；`Shaders/VoxiaSvoRaymarch.usf` 写入最小 `RWByteAddressBuffer DispatchOutput`，`FVoxiaSvoRaymarchCS::DispatchProbe` 在真实 RHI 下提交 1×1×1 compute dispatch，`AVoxiaWorldActor` queued 日志新增 `raymarch_dispatched` / `raymarch_groups` / `raymarch_nodes` / `raymarch_roots`。验证：`Build.bat VoxiaEditor ... -NoLiveCoding` 退出 0；`Automation RunTests Voxia.Voxel` 在 `-nullrhi` 下 17 tests 全 Success，`SvoPreview` 明确跳过 dispatch 断言；真实 RHI `Automation RunTests Voxia.Voxel.SvoPreview` 退出 0，dispatch report 为 `shader_available=1` / `parameters_ready=1` / `dispatched=1` / `groups=[1,1,1]` / `output_bytes=16`；8km null RHI SVO CLI smoke 为 `runtime_resource_ready=true` / `runtime_payload_bytes=3260480` / `quad_count=1166362` / `seam_check.status=pass`，WorldActor queued/streamed 日志为 `rhi_ready=1` / `rhi_bytes=3260480` / `raymarch_dispatched=0`（null RHI）。仍未落地：完整 screen-space GPU raymarch renderer、SVO 持久化 artifact、runtime mesh / HISM / Nanite-ready artifact。
- 2026-07-02：**runtime SVDAG preview grid readback 第六片落地**。`FVoxiaSvoRaymarchCS` 的 compute shader 改为 8×8 thread group，按 `OutputWidth` / `OutputHeight` 写入每采样 4 个 `uint32` 的 debug grid；`DispatchPreviewGridProbe` 提交 compute dispatch 后在 render thread 完成 `FRHIGPUBufferReadback`，并返回 `OutputSize` / `SampleCount` / `ReadbackWords` / `bReadbackReady`。`AVoxiaWorldActor` 默认仍走低成本 1×1 probe，显式加 `-VoxiaSvoRaymarchPreviewGrid=N`（clamp 到 16）才启用有界读回；queued 日志新增 `raymarch_grid` / `raymarch_samples` / `raymarch_readback`。验证：先写 `Voxia.Voxel.SvoPreview` 4×4 preview grid 断言并确认缺 `DispatchPreviewGridProbe` / readback 字段编译红灯；实现后 `Build.bat VoxiaEditor ... -NoLiveCoding` 退出 0；真实 RHI `Automation RunTests Voxia.Voxel.SvoPreview` 退出 0，grid report 为 `dispatched=1` / `readback=1` / `groups=[1,1,1]` / `grid=[4,4]` / `samples=16` / `readback_words=64`；`-nullrhi` `Automation RunTests Voxia.Voxel` 退出 0，17 tests 全 Success，并显式跳过 dispatch/readback 断言。仍未落地：完整 screen-space GPU raymarch renderer / screen pass、SVO 持久化 artifact、runtime mesh / HISM / Nanite-ready artifact。
