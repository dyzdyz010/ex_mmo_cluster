...
- `docs/docs/10-active/field-emergence/2026-05-14-phase7-field-kernel-architecture.md`
- `docs/docs/10-active/field-emergence/2026-05-16-phase8-physical-phenomenon-system-architecture.md`
- `docs/docs/10-active/field-emergence/2026-05-19-prefab-field-participant-projection.md`
- `docs/2026-05-13-体素局部场最小目标-索引.md`
- `docs/10-active/cross-cutting/voxel-server-authority-phase-overview.md`

本文档是 Phase 7 后续推进的执行基准。已有 Phase 7 文档仍保留底层 kernel 设计、协议约束和实现细节；后续排期、优先级和验收以本文档为准。

> **全文覆盖口径（2026-07-14）**：`clients/Voxia` 是唯一现役客户端。本文所有 Web / `web_client` / Web CLI / HUD / 浏览器 GUI、smoke 与 overlay 内容只保留为归档历史实现和当时验证证据，不再属于默认任务或验收门禁；这不改变本文记录的服务端 `FieldKernel`、`FieldRuntime`、HTTP、`ChunkProcess` 等实现事实。若这些客户端能力仍有产品需要，必须按 Voxia 当前路线另行排期；不得以归档 Web 实现替代 Voxia 完成，也不得自动纳入当前 A10 范围。本覆盖口径优先于下文任何未限定客户端地位的历史表述。
...
7. prefab/object 尚未通过统一 field participant projection 进入所有局部场；后续不应让每个 kernel 分别特判 prefab。推进基准见
   `docs/docs/10-active/field-emergence/2026-05-19-prefab-field-participant-projection.md`，电场只作为首条验证路径。
...
设计文档：`docs/docs/10-active/field-emergence/2026-05-16-phase8-physical-phenomenon-system-architecture.md`
...
2. `docs/10-active/cross-cutting/voxel-server-authority-phase-overview.md` 只维护高层索引；细节以本文档为准。
3. `docs/docs/10-active/field-emergence/2026-05-14-phase7-field-kernel-architecture.md` 继续作为 kernel 设计和协议背景，不再作为后续优先级来源。
