...
## 9. 12-tile 完整长程结果（2026-08-19，24 段 / 39,944 帧 / 约 5 分钟持续移动）

**流畅性总体结论：无累积劣化，尖峰有界，稳态被抬高约 1.5ms。**

- `frame p50` 全部 24 段恒为 **7.692ms（130fps 满帧率）**；frame p99 最差 8.154ms
- **趋势通过**：后半程 GT p99 均值 5.24ms，限度 8.41ms；`back_12` 与 `out_01` 同水平，
  流水线资源段末全部归零（`far_release_pending=0`），24 段 parity 全部 verified
- GT max 每段 17–25ms（修复前 26–30ms）

门禁判定 15/24 段失败，全部两类，语义不同：

1. **hitch ratio 0.0010–0.0014（12 段）**：每段恰 1–2 帧 >16.67ms，门限（0.001×~1660 帧
   ≈1.7 帧）只容 1 个。来源已归因：Near 组提交（≤64 子项单帧 Plan+Begin，13–16ms）
   叠加日常工作。§8 的分帧决策项是正解。
2. **GT p95 ≈4.9–5.9ms 超 3.5ms 门限（6 段）**：这不是尖峰——是持续移动下每帧
   流式底载（prefetch/调度/呈现推进）把稳态从静止场景的 ~3.3ms 抬到 ~5ms。
   3.5ms 门限是按**静止/短程**场景标定的；对"长程持续移动"这一新场景，
   门限是否应当分场景标定属产品判断，**不由本稿单方面放宽**。
...
- 2026-08-19：12 tile 首跑在段末单次 parity 撞 pending 挂掉；改用重试 helper（`b8b973d`）。
- 2026-08-19：12 tile 重跑 24/24 段完整执行与判定，结论定型（§9）。
