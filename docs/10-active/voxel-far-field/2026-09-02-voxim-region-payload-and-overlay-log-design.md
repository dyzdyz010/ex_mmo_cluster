...
2026-09-07 实测：16 km Demo 世界 L4 768 region 47.7 MB + L5 192 region 11.4 MB（每列 [mixed ry − 1, +1] 三行，含常量 region）；32 km 见 `Voxim/Docs/R6.md` §S4 第六切片。原稿的 "L3+ < 150 MB" 对应的是最右一列（cells）。服务端 L3+ 整世界常驻载荷缓存 = 1.0–1.9 GB 内存，可接受。
...
2026-09-07 实测（S4 第六切片，v4 线格式 + L4/L5 资产，Demo 16 km 世界，方法与记录见 `Voxim/Docs/R6.md` §S4 第六切片）：

| 场景 | 实测 | 模型 |
| --- | --- | --- |
| 冷客户端一次到齐（607 region） | 14.31 MiB，其中 L0 0.40 / L1 2.15 / L2 5.12 / L3 6.14 = 13.81 MiB；L4/L5 148/176 来自资产 | 17.5 MB |
| 传送 3 km（资产已在） | 12.84 MiB，9.1 s 到齐无洞 | 17.5 MB |
| 走路 6 m/s（726 m） | 53.4 KB/s = 8.9 MB/km；GT p95 9.8 ms | 75 KB/s |
| 冲刺 24 m/s（1458 m） | 236.5 KB/s = 9.9 MB/km（L0 1.62 / L1 4.21 / L2 5.77 / L3 2.41 MiB）；GT p95 15.4 ms | 300 KB/s |
| 单 region（mixed 均值，zlib） | L0 4.7 / L1 33.9 / L2 116.7 / L3 96.9 / L4 196.1 / L5 166.2 KB；均匀 0.65 KB | §8.1 精简列 |
...
### 2026-09-07：用户批准 R6 有条件收口（当前状态）

用户获知 S1 严格同条件性能对照缺口、冷加载 backlog GT p95 16.392 ms 与 8.33 ms 未达后，确认「可以，就这样吧，收口，提交推送」。**R6 有条件收口，S1 对照延期**；这是用户批准的验收调整，不将未签收指标改写为实测通过。120 FPS 与启动/传送等剩余尖峰保留为非阻塞性能欠账，不为旧基准重建退役 provider。

收口基于 Voxim `b654795` / 本仓记录 `afda86e1`：正常构建、UE 227/227、工具 6/6、独立审查，以及下列受限功能与实测证据。当前状态和性能欠账统一见 `Voxim/Docs/R6.md` 顶部收口决定；以下日志保留当时的未签收判断。接续 **M0 / M1 / M2a → R7-A**，本次只同步文档，不实施移动、Prefab 或更改服务运行状态与开发数据。
...
UE fixture → Rust `generate_body` 200/200 → 服务端 `GeneratedStoreTest` `:t2` 用 `GeneratedStore.read`（就绪门文件 / 合成常量 / 在线 L0，即实际会发的字节）解压后 body 逐字节相同 + 逐格 200/200 → Python 第三方逐字节 200/200 → UE `ImportRandomSample` 正式 codec 逐格 200/200。
benchmark 按 §8.3 口径复测（结果注在 §8.3 表下）：冷传送 14.31 MiB（L0–L3 13.81，对照 17.5 MB）、传送 3 km 12.84 MiB、走路 6 m/s 53.4 KB/s（对照 75）、冲刺 24 m/s 236.5 KB/s（对照 300）；GT p95（CSV 逐帧）走路 9.8 ms、加载 / 传送 / 冲刺 13.6–15.4 ms，8.33 未达（R5.14 口径）。
资产：16 km 59.1 MB；32 km 实测见 Voxim 记录。R6 验收表（T-1…T-9 → 测试名、S1 → S4 数字）写在 `Voxim/Docs/R6.md` 顶部，缺口 10 条：GT p95、T-9 无自动化、DataService 日志（第七切片）、材质 catalog / content_version（第八切片）、资产安装流程、
服务端 T-1 依赖过期 WorldBake v3、T-7 重连无自动化、分服 id、ry 盒外 / 被编辑 L4+ 走网络、T-2 只覆盖 Demo 配置。**R6 验收未宣称完成**。没有产品代码改动；服务端只加测试。
