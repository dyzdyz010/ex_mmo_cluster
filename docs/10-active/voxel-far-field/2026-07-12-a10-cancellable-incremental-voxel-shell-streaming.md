...
- **状态**：A10 provider/residency/DAG 与跨 LOD 外露表面材质语义保持完成；Patch
  ...
  Real-RHI 用户可见竖墙复验及更多发布硬件待刷新；后继阶段 3 已完成 30 分钟持续流送。
  Online authority/provider 不在本任务范围；复验前仍不得把真实墙面视觉写成已验收
- **归属**：扩展后的里程碑 A / A10
...
renderer asset/UV/AO 与 presentation contract closeout 后，全关闭 Lighting/Fog/PostProcessing 的
同相机证据曾证明 live LOD material id 仍不一致：中心点降采样会漏掉默认 4m 表层。该项现已在
source-neutral canonical reducer 根修复；VXP5 从精确 source surface coverage 归约外露面材质，
LOD0–4 的 live witness、全量 Automation、Null-RHI 与 Real-RHI 均通过，因此 A10 可标记
client-complete。阶段 3 本轮未启动。
...
- VXP5 wire 继续保持 dense 确定性并携带 surface coverage semantic；解码后的统一空气/统一材质页恢复为 compact canonical storage，内容 fingerprint 与 dense/compact 表示无关，避免本地包把驻留和 mip 工作量放大；VXP2/VXP3/VXP4 与旧 schema 显式拒绝；
...
- **2026-07-23 / A10 material semantic parity 诊断与根修复**：固定相机关闭 Lighting/Fog/
  ...
  closeout 继续有效；随后先补 live owner/ring/LOD/material histogram 与 thin-stratum RED 测试，
  再以 surface-aware canonical reducer 从精确 source coverage 归约外露面材质，并升级
  schema/fingerprint/cache gate。最终 UE `152/152`、Node `82/82`、Phase 1/2 Null-RHI 与可见
  Real-RHI 均通过，25 个 LOD0–4 witness 全部为 `exact=LOD=final=1`。阶段 3 在修复期间未启动，
  本轮仍未开始；详见 [`专项修复稿`](2026-07-23-far-lod-surface-material-semantic-repair.md)。
