...
因此，本表描述跨阶段最终世界渲染目标；阶段 1 只收口独立 PRD 明确列出的只读 Mock world、生命周期、材质、完整 XYZ 与安全视图能力。表内编辑与 prefab 缺口分别由阶段 2、阶段 3 归零。无论在哪个阶段，单独模块、probe、截图或旧路径性能都不能替代正式唯一根的真实操作、automation 与 CLI / 日志门禁。
