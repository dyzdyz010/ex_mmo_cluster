...
### 2026-07-13 9 tile 整体替换回归与修复检查点
...
- 代码根因三：呈现 fingerprint 混入全局 voxel/field revision，使局部写入、相同内容重放或 field tick 都可能把整窗误判为变化；它表达的是全局运行时时钟，不是某个 chunk 实际提交内容的身份。
- 代码根因四：退出侧仍在 matching far live 后执行 `binary_after_far_visible`，本次可见实跑依次同帧释放 `296/293/266/168` 个 retained chunks。若玩家观察身后 near→far，会直接看到整片退场。
...
- 当前修复保留 retained published ledger，把 full validation 与 visible replacement 分开；fingerprint 改为复用 confirmed render content identity，不再让全局 revision 污染整窗 decision；首次创建、已有 mesh 替换和清空三类变化共用 visible-mutation 预算。
- 最终 review 又关闭三个边缘入口：visible-mutation 预算现覆盖所有非冷启动来源，不再只认 WorldGen 特殊 revision；相同 field snapshot / missing destroy 不推进全局 FieldRevision；退休块重入必须匹配冻结时自身、六邻居与自身 field 的完整 presentation fingerprint，不匹配就走 replacement。
- 交接时序按 chunk 对称化：进入侧 N 帧提交 near、N+1 mask far；退出侧 N 帧只 unmask 对应 far，N+1 才释放 lease 并清 near。release item 携带 lease token/generation，折返、重绑或新 generation 会 fencing 掉旧 item，禁止旧队列清理新所有权。
- `near_mesh` 继续暴露 build kind、visible mutations、ledger reset 与 retirement release 进度。相关定向 automation 与完整 near+far Real-RHI A→B→A→B 已通过：三次移动只启动 `1029/1029/964` chunks 的 streaming catch-up；`max_paced_visible_mutations=1`、违例 `0`、`full_window_ledger_resets_total=1`（仅 cold start）；折返以 `live_active_diverged` 取消旧 release，revision 4 最终 pending=`0`、累计完成 `776`。独立 `near_prefetch B → staged ready → move B` 又在实景命中 1 次原 `ready_reused`，其后只排正常 `1029` chunks 而非异常 `3087`。全程 backend=`partitioned_dynamic_mesh`、raymarch=`none`，无 CLI 失败、断言、崩溃或 publish failure。代码与本机可见 RHI 状态已收口，仍待用户窗口及另一台硬件做主观像素确认。
- 本轮可复现证据保存在 `clients/Voxia/Saved/Logs/.demo/observe/near_far_9tile_granular_abab_final.log` 与 `near_far_ready_reused_granular_final.log`；最终 review 后已再次完整编译，并复跑同目录的 `worldactor_9tile_final.log`、`presentation_9tile_final.log`、`tile_window_9tile_final.log`、`near_retirement_9tile_final.log`、`voxel_store_9tile_final.log`、`field_store_9tile_final.log`、`field_inbound_9tile_final.log`、`far_dither_9tile_final.log`，全部退出 `0`、无 automation failure。

### 下一步

1. 保持完整 near+far 程序打开供用户窗口确认逐块接管效果，并在另一台硬件复跑同一 A→B→A→B CLI；本机状态机、预算、N/N+1 release 与折返 fencing 已通过，不再重复实现层排查。
...
**Last updated**：2026-07-13（同块双显已关闭；9 tile 回归的内容身份、visible-mutation、N/N+1 与 token fencing 修复已通过定向自动化及本机完整 Real-RHI，待用户窗口/异机主观确认；3D LOD 仍仅设计暂停）。
