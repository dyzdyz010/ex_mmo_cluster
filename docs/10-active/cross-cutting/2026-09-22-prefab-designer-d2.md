...
2026-09-22 后续纠正：不能把失败主要归因为模型能力。父脑缺身体信息，设计会话缺主动环境查询和记忆访问，
现已按[统一输入与记忆契约](2026-09-22-npc-context-memory-contract.md)补齐并通过相关集成测试。
最新真实两层屋会话已自主调用 look，但第4次请求超时，未发布/放置；完整证据与状态见该契约的进度记录。
