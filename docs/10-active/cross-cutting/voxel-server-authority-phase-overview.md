...
> lifecycle/ownership 与阶段 2 已完成；A8/A10 的跨 LOD 外露材质语义因粗采样漏掉薄表层重新打开。
> Online confirmed provider 尚未接入。
...
| Voxia 扩展里程碑 A | 客户端完整 XYZ near/far LOD、provider-neutral 数据流、原子 presentation 与流送性能 | **A8/A10 材质语义修复待实施**：27 Tile XYZ window、唯一根、source identity、request/residency/cancel/DAG/stable patch、三轴巡航和 confirmed presentation 已通过；粗 LOD 中心采样漏掉 4m 表层，live external surface material parity 未通过。阶段 2 不受影响，Online/B/C 后置 | [`上位计划`](../voxel-far-field/2026-07-12-pure-3d-voxel-shell-migration.md) / [`材质语义修复`](../voxel-far-field/2026-07-23-far-lod-surface-material-semantic-repair.md) |
