...
- **体素生成 / 流送 / Voxia 加载渲染实施计划（2026-06-30)**:[`docs/10-active/voxel-authority/2026-06-30-voxel-generation-streaming-client-plan.md`](../voxel-authority/2026-06-30-voxel-generation-streaming-client-plan.md) —— 把 baseline 边界决策与同步窗口设计拆成 Phase 0-8 执行序列，补 H、H gate、canonical、D/P、checkpoint、LOD projection 等名词解释与验收矩阵。
- **WorldGen v1 确定性地形生成设计（2026-06-30，历史输入)**:[`docs/10-active/voxel-authority/2026-06-30-worldgen-v1-deterministic-terrain-design.md`](../voxel-authority/2026-06-30-worldgen-v1-deterministic-terrain-design.md) —— 旧 2.5D 算法稿只保留为迁移证据，不再是目标契约；当前目标是 `chunk_xyz -> canonical 3D chunk`，地表只是三维密度算子的一种内容结果。
- **纯 3D 体素立方壳迁移阶段（2026-07-12)**:[`2026-07-12-pure-3d-voxel-shell-migration.md`](../voxel-far-field/2026-07-12-pure-3d-voxel-shell-migration.md) —— 当前主线：WorldGen 与下游解耦、XYZ cube-shell/source identity、通用 occupancy/material LOD 和 GPU 原子 presentation。P0/P1 已完成，P2 进行中；新链仍为隐藏 staging。
...
- **Voxia 远景公共组件抽取 + VHI 2.5D baseline 定位（2026-06-30)**:[`docs/10-active/voxel-far-field/2026-06-30-voxia-farfield-common-components-and-vhi-baseline.md`](../voxel-far-field/2026-06-30-voxia-farfield-common-components-and-vhi-baseline.md) —— 把 VHI/SVO 逐行重复的编排（coverage 规划 / 异步生命周期 / 分帧上传）抽成 `FarField/` 三个正交公共组件（3D-ready），明确 VHI=廉价 2.5D 地表 baseline、3D（浮空岛/洞穴）归 SVO，3D 源切换作下个里程碑。抽编排不抽算法。
- **Voxia 体素管理管线生产路线（2026-07-05)**:[`docs/30-reference/overview/2026-07-05-voxia-voxel-lod-production-route.md`](../../30-reference/overview/2026-07-05-voxia-voxel-lod-production-route.md) —— 在 SVO 数据管理与滑动窗口加载策略已确定的前提下，拍板 L0 近景 `3x3x3 tiles`、L1-L3 SVO leaf-surface mesh artifact 默认生产渲染、L4 raymarch-only 可选超远景 profile；VHI 冻结为 2.5D 过渡 baseline。
...
| Baseline/streaming delta migration | 体素生成、流送、Voxia 加载渲染从全量物化过渡到确定性 WorldGen + D/P + H gate | 实施计划就位 | [`2026-06-30-voxel-generation-streaming-client-plan.md`](../voxel-authority/2026-06-30-voxel-generation-streaming-client-plan.md) |
| 纯 3D 体素立方壳迁移 | WorldGen 无关 canonical source、XYZ cube-shell、通用 3D LOD、原子 near/far presentation、退役 heightmap/column | **进行中：P0/P1 完成；P2 已落 v2 XYZ pages、六向 material mip、exact surface、原子 hidden staging 与 ±8km 世界对齐 Real-RHI debug proof，尚未切 live 或删除旧材质/UV 分支** | [`2026-07-12-pure-3d-voxel-shell-migration.md`](../voxel-far-field/2026-07-12-pure-3d-voxel-shell-migration.md) |
| WorldGen v1 旧稿 | 旧 2.5D 高度场算法输入 | **已被纯 3D canonical chunk 契约取代；仅保留历史输入** | [`2026-06-30-worldgen-v1-deterministic-terrain-design.md`](../voxel-authority/2026-06-30-worldgen-v1-deterministic-terrain-design.md) |
| Voxia VHI preview | 新关卡试验 Voxel Hierarchical Impostor；旧 WorldGen preview / heightmap LOD 保留 | 实验实现中 | [`2026-06-30-voxia-vhi-experiment-plan.md`](../../90-obsolete/voxel-far-field/2026-06-30-voxia-vhi-experiment-plan.md) |
...
| VLOD-A4 | 体素 LOD **里程碑 A 收尾**:跨 depth 覆盖性 seam + 换环 fade + L1 collar(3.5m) ＋ A5 顶点瘦身/cache LRU ＋ A 验收长巡航 | **收尾中(2026-07-08)**:Step0(目视基线,高空"破洞"=机位假象)、Step1(跨 depth 覆盖性 seam 断言)、Step2(L1 3.5m collar,opt-in)、backlog A(远场逐面贴壁真材质)**已完成并推送**;**下一步=A(光照)**——查远景竖直侧面偏暗(真因转向光照/LOD 非材质)。**未开工**:Step3 换环 fade、Step4-5 A5 顶点瘦身/cache LRU、Step6 8km 长巡航 A 验收(收官)。战略约束 **FPS=像素-bound**(A4/A5 不动 FPS,Lumen 才是杠杆) | [`phase-vlod-a4-seam-fade-collar.md`](../voxel-far-field/phase-vlod-a4-seam-fade-collar.md) |
