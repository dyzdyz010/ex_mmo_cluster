...
6. **溯源：已做（2026-09-21）**。World 记下“谁花材料放下了这一格”（`placed_by`：macro 格 → cid），经 `material_snapshot/3` 与 NPC 的 `look` 可见；
   作者入口写的格、天然地形、被任何别的编辑改过的格无主，挖掉即清。随事务元数据（kind-3 行）走，与余额同一条路：重启、压实后仍在，
