...
- 截至 2026-06-29 15:03 的刷新验证，真实 full32km canonical 权威数据仍未落齐：coverage probe 报告 canonical scene chunk 为 `0 / 444,596,224`；对应完整 release pack 也不存在，verifier 报告 `0 / 16,384` shard verified。当前能确认的是缺失会显式非零失败，不会被局部包、空 DB 或 runtime fallback 伪装成 ready。
...
- 32km 级世界不能直接盲跑旧 full cuboid job：真实 pressure 证明 DB per-chunk snapshot + 每 chunk `.vcsnap` + 旧逐 chunk LOD projection 写入布局会落到数十 TB payload、数亿小文件和数百天级生成时间。当前已切出 deferred LOD 导入入口，但还没有完成真实 32km canonical snapshot 全量跑批、分区 LOD rebuild、以及最终 launcher pack 下载验证。
...
1. 先完成真实 32km canonical snapshot 物化/导入，再用 `scripts/world_pack_authority_coverage.exs` 证明 canonical `total_scene_chunk_count=444,596,224` 且 sampled windows/shards 都 ready；然后用 `scripts/world_pack_release_build.exs --pack-root <pack_root>` 基于 `payload_shard_grid/1` 逐 shard 跑批生成 `16,384` 个 `.vxpack`。当前 authority coverage probe 已证明 test canonical store 为 0/444,596,224。
2. 补齐 launcher/update 层：下载完整权威 pack、校验 manifest/hash/index/range，再让 Voxia 使用本地 pack 进场；只有本地 pack/index 完整才允许进入 Gate/Scene。
3. 将 full-pack materialization 默认切到 deferred LOD：先用 `materializer_opts: [lod_projection?: false]` 落齐 canonical chunks，再按 column/tile/shard 分区运行 `LodProjection.Rebuilder` 或后续批量 rebuilder，避免回到 `37,790,679,040` 级 inline upsert attempt。
4. 在新 pack/index 上补 launcher 验证闭环：启动器更新完整权威包 -> 校验 manifest/hash/index -> Voxia 进场前确认 baseline 完整 -> 进场后只流 runtime diff。
5. 保留 `scripts/world_pack_pressure_probe.exs` 作为回归工具；每次改 pack/materializer 后重跑 `single,cube64,vertical100,plane256,tile343`，并继续记录 observe JSON。
