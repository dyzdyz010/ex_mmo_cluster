...
- **默认分级错配**：缺显式 tier 契约，当前默认 `samples=4` 应切到 2 并加入第三环 L2.5（四环 7/14/28/56m，2026-07-06 拍板）；缺该契约会让 Transport、脚本和 cache key 继续依赖隐式推导；实施载体为里程碑 A1。
- **RuntimeMesh 单组件全量重建风险**：缺分组件 `UDynamicMeshComponent` + `StaticDraw` + 分频更新；现有单组件路径按代码推演存在流式期间 O(N²) 总功与瞬态尖峰风险；实施载体为里程碑 A2。
- **8km 实测尖峰归因**：缺 `min_fps=42.790` / `max_frame_ms=23.370` 的真实来源分析；该数据来自 ProcMesh 后端且上传完成后采样，不能归因到 RuntimeMesh 单组件风险；实施载体为里程碑 A2 立案，证据路径 `clients/Voxia/Saved/svo_8km_perf_smoke.log`。
- **per-cell greedy merge**：缺 cell 内 masked greedy merge；当前逐 leaf 面输出会放大 quad 预算，且 collar 与垂直多层预算依赖 merge 收益；实施载体为里程碑 A3。
- **跨 depth seam / fade / collar**：缺覆盖性 seam 断言、换环 fade 和 L1 3.5m collar（含 depth clamp 放宽）；现有 `seam_check` 不能证明 7/14/28/56m 跨 ring 过渡无洞或无双显；实施载体为里程碑 A4。
- **顶点格式与 cache 容量**：缺远景顶点格式瘦身、persistent cache LRU 和容量预算；没有它，quad 数下降后仍可能被 CPU/RAM 中间态和缓存增长拖垮；实施载体为里程碑 A5。
...
- **长巡航与生产门槛验证**：缺跨至少 8 tiles 的 8km 长巡航、per-ring quad/内存预算断言、截图审计与长期帧时间分布；短 smoke 不能证明生产稳定性；实施载体为里程碑 A 验收。
...
- **raymarch 升格与 Nanite bake**：均为 defer，不是生产缺口。生产远景 renderer 已拍板为 L1-L3 SVO leaf-surface mesh + 分组件 DynamicMesh StaticDraw；raymarch 仅保温为 d≤72 AB profile / L4 候选（触发条件与门槛见 [`2026-07-05` 路线 §4.2](../../voxel-server-authority/2026-07-05-voxia-voxel-lod-production-route.md) 与 [`2026-07-06` 设计 §3.3](../../voxel-server-authority/2026-07-06-voxia-lod-layering-and-technology-design.md)）；Nanite 渲染/烘焙后端 defer（运行时构建 UE5.8 API 级不可行，离线 bake 触发条件为 editor 手工 A/B 数据显著）。
...
- **当前事实文档维护**：缺随代码变更同步更新 `docs/current_status/**` 的稳定流程；缺口会让 snapshot 再次退化成改动记录或过期状态；实施载体为 PR / checkpoint 文档纪律。
