...
- `clients/Voxia/scripts/voxia_stdio_cli.js`：启动 headless / real client 并发送命令。
  ...
  - `svo`：仅在 `-VoxiaWorldGenPreview -VoxiaSvoPreview` 下返回 SVO 状态，含 `enabled`、`revision`、`build_in_flight`、`has_pending_build`、`radius_tiles`、`samples_per_tile_axis`、`max_depth`、node/leaf/empty/solid/mixed 计数、quad 计数、`build_ms` 和 seam check。
