...
- **Online authority 尚未开始。** 阶段 1 的 `confirmed_revision=0` 只是接口占位；WorldGen/local
  pack 都不是服务端 confirmed truth。后续只能替换 bootstrap/provider，不得新建第二条客户端主干。
...
- 初始会话先绑定 snapshot，再 deferred spawn 唯一根；near/far/snapshot/ownership/fence 一致后才
  进入 playable。
- candidate coverage 未提交时，相机保持 last-safe view；guard 不修改 pawn、control rotation、
  velocity 或几何。
- 短时 hold 先记录，再显示非阻塞同步提示；达到硬阈值进入恢复加载。当前 generation 后续提交时
  可自动恢复，也可主动 retry 或返回菜单。
...
| `Automation RunTests Voxia` | `68/68` success，0 warning / failed / not-run |
| Null-RHI 全路线 | 25 条路线 pass |
...
| 门禁 | 结果 | 产物 |
|---|---|---|
| Development build | pass | UE 5.8 UBT，exit 0 |
| `Automation RunTests Voxia` | `69/69` pass，exit 0 | `.demo/observe/voxia_phase1_review_fixes_20260718_0052/automation_all_voxia_final.log` |
| Null-RHI 全路线 | 25/25 pass，clean exit，最终 release=`11/11/0` | `.demo/observe/voxia_phase1_2026-07-17T17-58-12-947Z_null_rhi_1280x720/` |
| 1280×720 Real-RHI 全路线 | 24 条功能路线完成后首个性能窗严格失败：GT max=`16.98ms`、`>16.67ms=1` | `.demo/observe/voxia_phase1_2026-07-17T17-10-09-719Z_real_rhi_1280x720/` |
| 1280×720 performance-only | 两轮独立运行一红一绿；失败轮回程 GT max=`424.536ms`，通过轮 GT `>16.67ms=0/0` | `.demo/observe/voxia_phase1_2026-07-17T17-17-41-539Z_real_rhi_1280x720/`、`.demo/observe/voxia_phase1_2026-07-17T17-19-10-704Z_real_rhi_1280x720/` |
| 1600×900 Real-RHI soak | 30 分钟，120 条路线、95 样本，`growing_keys=[]`；queued/completed=`14→390`、pending=`0`，最终 `391/391/0` | `.demo/observe/voxia_phase1_2026-07-17T17-20-55-320Z_real_rhi_1600x900/` |

硬化候选尚未升级为新的“最终验收点”。本机 RTX 4080 SUPER / NVIDIA 591.86 / 向日葵
虚拟显示环境中，最新两次独立 D3D12 performance-only 为一红一绿；两轮都在 RHI 初始化约
58–60 秒后出现 400ms raw frame，失败轮将 `424.536ms` 计入 GameThread，通过轮则主要落在
render/RHI、GT max=`12.464ms`。先前定向 UE hitch 树将 `423.086ms` 完整归因于
`STAT_D3DUpdateVideoMemoryStats` 中的 DXGI `QueryVideoMemoryInfo`；同帧 Voxia world tick 约
`1.4ms`，far build 未恢复，release=`4/4/0`。引擎源码确认 Development D3D12 每帧同步
查询该统计且没有禁用/降频 CVar。验收脚本仍严格报错，未过滤或豁免该帧；
