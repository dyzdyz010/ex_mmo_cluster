...
- **离线 Mock 阶段 1 lifecycle、near/far handoff 与跨 LOD 表面材质语义均已收口。**
...
- **材质族载体、renderer 外观输入、有界发布与 live material reduction 已收口。**
  ...
  `exact source → canonical LOD → final artifact` witness。VXP2/VXP3/VXP4 与旧材质 schema 明确拒绝。
...
| `Automation RunTests Voxia` | `152/152` success，0 failed，exit 0 |
| Node runner/governance | `82/82` pass |
| Phase 1 Null-RHI 联合闭环 | `passed=true`，25 条 route record；X/Y/Z、对角、A-B-A、纵向往返、teleport、失败重试、新会话与性能往返通过；clean exit、far release=`11/11/0` |
...
| 2026-07-23 修复后固定相机验收 | D3D12 Development `PCD3D_SM6`，1600×900；LOD0–4 共 25 个 witness 均为 `1→1→1`；控制图 near/far RGB 最大均值差 `0.094966/255`；clean exit 0 |
...
- `.demo/observe/voxia_phase1_2026-07-23T12-52-18-563Z_null_rhi_1280x720/`
- `.demo/observe/voxia_phase2_2026-07-23T12-59-15-678Z_null_rhi_1280x720/`
- `.worktrees/voxia-phase2-macro-interaction/Saved/Logs/build_surface_semantic_review_closeout_20260723.stdout.log`
- `.worktrees/voxia-phase2-macro-interaction/Saved/AutomationReport_SurfaceSemanticReviewFinal3_20260723/index.json`
...
`voxel_surface_material_state`、`frame_perf` 与 `render_perf`。
...
该诊断随后由 source-neutral 精确表面覆盖 reducer 根修复；修复后 live LOD0–4 的 25 个同点 witness
全部为 `exact=LOD=final=material 1`，控制图 near/far RGB 最大均值差为 `0.094966/255`。详见
