...
- **近远景同块双显的已知根因与状态契约回归已关闭（2026-07-13）**：旧实现等待新 SVO result 才启用进入侧 mask，且边界像素用裸 world-position floor 会落到空气侧 chunk。现已改为 active-near proactive ownership + entity-side normal inset，并让 Prepared/Pinned 生命周期按完整 footprint 自维护；边界红测可稳定复现旧缺陷，修复后 Presentation/材质/WorldActor/TileWindow automation 与真实 RHI A→B(pinned)→C `until_near_far_exclusive` 均通过，B 先成为 live 时也没有释放 C 的 mask。这里关闭的是已知持续双显根因与可观测状态契约，不等于上一条 generation 级原子提交缺口已完成，也不替代另一台硬件上的最终像素/主观窗口复核。
- **逐 chunk 可见替换契约的代码、自动化与本机完整 Real-RHI 已闭环，仍待用户窗口与异机主观确认（2026-07-13）**：用户窗口确认同块双显已解决后，`ready_reused` 曾把完整 `3087 = 9×7³` chunks 排入 settled revalidation，造成 9 个水平 tiles 共同变化。该问题不是由 near/far 网格细分不同造成；根因是 settled/reused 重置或重判 published ledger、预算只覆盖 create、全局 voxel/field revision 污染整窗呈现身份，以及退出侧整集二值释放。当前实现改为按 confirmed render content identity 复用 decision，settled 校验保留 retained ledger；create/replace/clear 统一计入 visible-mutation 预算；退出侧使用 N 帧逐 chunk unmask far、N+1 释放 near，并以 lease token/generation fencing 丢弃折返或重绑后的过期 release。定向 automation 与完整 near+far Real-RHI A→B→A→B 均通过：三次移动为 `1029/1029/964` catch-up chunks，paced visible mutation 最大值 `1`、违例 `0`、ledger reset 仅冷启动 `1` 次；折返陈旧 release 被取消，revision 4 最终 pending=`0`、累计逐 chunk 完成 `776`。独立 `near_prefetch → staged ready → move` 实景又明确命中原 `ready_reused` 分支，修复后只排 `1029` 而非 `3087` chunks，预算与 reset 指标不变；两轮均无 CLI 失败、断言、崩溃或 publish failure。剩余缺口只保留用户可见窗口和另一台硬件的像素/主观复核；它不再是本机状态机或节流实现缺口。
