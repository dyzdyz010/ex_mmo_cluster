...
- **长期性能分布**：A4 Step6 的跨 tile 8km 长巡航、per-ring 预算与截图审计已完成；2026-07-11 可见独立 1600x900 uncapped 运行的收敛样本为 `116.4-129.9 FPS`（`7.70-8.59ms`），证明 PIE 的精确 60 FPS 不是性能上限。但首次 near 开发冷生成曾降至约 `12-15 FPS`，跨 tile 82-patch 提交附近出现 `20.4-53.4 FPS` 低谷；仍缺固定路线、更长时段、更多硬件档位的 p50/p95/p99 帧时间分布。远景 FPS 当前实测为**像素-bound 非三角形-bound**，Lumen 全屏 GI（关掉 +36~40FPS）仍是最大单一稳态 FPS 杠杆，流送尖峰则优先归属 GameThread patch/DynamicMesh 与 near 生成预算。
- **远景时序稳定与无缝流送（Phase 0/1 + Phase 2 前三切片 + Phase 3 near 预取第一切片已落地，整体未收口）**：换环 fade 已去除 `DitherTemporalAA` 的逐帧噪声并加入 `svo_visual_stability`；patch-native cache、dirty-boundary seam、默认取消 aggregate mesh / 无用 runtime SVDAG，以及 dirty macro-cell 与受影响 patch 聚合的并行任务均已落地。8km 跨 tile SVO build 从原始 `135948.497ms` 降到 `2015.912ms`（约 `-98.5%`）；第三切片同构 patch update 从 `113.545ms` 降到 `72.751ms`（约 `-35.9%`），82-task 聚合段为 `43.941ms`。WorldGen preview 已按速度预取相邻 3087-chunk slab，旧 active 持续到目标整窗 ready，跨界后才 activation 并清理旧 slab。剩余实施项是稳态 77% TSR shimmer 定量归因、patch/DynamicMesh 真正离开 GameThread、SVO outer coverage hysteresis 与 validated sharded artifact pack。阶段真值见 [`phase-far-temporal-stability-and-seamless-streaming.md`](../../10-active/voxel-far-field/phase-far-temporal-stability-and-seamless-streaming.md)。
