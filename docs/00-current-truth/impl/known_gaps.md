...
- **逐 chunk 可见替换契约在整窗重校验与退出侧仍有缺口（2026-07-13 重新打开）**：用户窗口确认同块双显已解决，但 `ready_reused` 激活可把完整 `3087 = 9×7³` chunks 排入 settled revalidation；旧 published ledger 被清空，而已有 renderable component 的替换/清空不计入每帧 `max_new_renderable=1`，因此 9 个水平 tiles 可在短窗口内共同变化。退出侧仍记录 `binary_after_far_visible`，一次释放 `296/293/266/168` 个 retained chunks。这与 near/far 网格细分不同无关；进入条带的 `1029 = 3×7³` chunks 及 ownership submitted/masked 递增仍证明首次进入路径在逐 chunk 工作。实施载体是统一 visible-mutation 预算、保留 retained published ledger、显式 revalidation 队列，以及退出侧渐进释放或有界互补 fade；验收必须分别覆盖 E、retained revalidation、X 和 cold start。
