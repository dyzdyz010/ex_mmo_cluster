...
6. **Voxia 已切到唯一 Near/Far Patch-diff 架构**：`-VoxiaWorldGenPreview` 只启动 `AVoxiaUnifiedVoxelWorldActor` / `production_all_features` 根。Root 分开维护 requested/live `FVoxiaPatchTargetKey`、可见发布优先级与派生 readiness；Near 固定 `4³ chunks` Patch，Far 固定 `8³ tiles` Patch；`UVoxiaVoxelPresentationSceneHost` ledger 是 live Patch、exact ownership、canonical boundary slot、seam、component 与 fence 的唯一事实。一个 Near Patch ready 不等待完整 Tile，一个 Far Patch ready 不等待完整 BuildFuture；Far、固定 26-slot 外壳、按实际逐 Tile owner/LOD 推导的 Near/Far 与 Far/Far LOD `LayerFace`、以及该 live Far 自己的精确 coverage/层间墙凭证原子提交。相邻换区先保留旧 live target 和旧 Far，完整候选 manifest 到达后才发布新目标；新 Near 完整后只补旧 Near 退场必需 Far，Far 随后让出共享提交入口，旧 Near 全退且根级画面证明推进后才恢复普通 Far 渐进发布。同编号 Near Patch 的窗口边缘范围不同时，先发布新旧 chunks 并集；精确 Far 已真实接管后再原子收窄到新窗口范围，最终仍严格为 9261 chunks。每个仍可见 Far 的 coverage/层间墙凭证只随该 Patch replace/remove，不因目标 manifest 轮换而消失。`playable`、`handoff_complete` 与全资源 `settled` 是三个独立事实。玩家可沿完整 XYZ 离开最后完整 Near 最多 3 chunks；只阻止继续向外进入第 4 个 chunk，返回和沿边界移动始终允许。旧 Tile handoff/chunk transaction、动态 atlas 扩容、full fallback、no-far sink、generation relabel、deterministic retry 与 legacy far runtime/probe 已从 production 删除。核心代码、Development build、完整 Automation `165/165`、Node `98/98`、原水平 Null-RHI 复现往返、独立移动安全门、竖直地面—全空气—下降路线及最新连续目标 Null-RHI 路线已通过，逐帧 gap/overlap/orphan 为 `0`；发布级广路线仍有独立 canonical 外露材质覆盖失败，Patch 发布的严格性能门也尚未关闭，完整全方向至少 10 Tile、Relocate、5 分钟以上资源平台、长稳和更多硬件仍待刷新。Far LOD VXP5 exact-surface 材质与合法 ownership MID 父链的既有完成事实不变；Online authority、阶段 3 prefab 与 B/C 尚未开始。
   **当前可见性纠正**：2026-07-27 用户实跑曾确认 Near/Far 朝内竖墙缺失。架构现已从固定 Patch 外框改为真实 `LayerFace`；同时修复跨 Far Patch 分界被跳过、退场旧 Near 污染新目标分界，以及连续目标轮换导致仍可见旧 Far 丢失墙依据三个问题。跨 Patch 的 Near/Far 固定由 Far 一侧发布，Far/Far LOD 固定由负方向一侧发布。自动化和 Null-RHI 结构化证据通过，但尚未完成修复后的 Real-RHI 用户可见复验，因此该视觉项仍保持重开，不能仅凭 slot、组件、fence 或 `gap=0` 宣布关闭。
