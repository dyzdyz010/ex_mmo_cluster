...
6. **Voxia 阶段 1 lifecycle/near-far ownership 与阶段 2 宏格 authority/receipt 已收口，但 Far LOD 表面材质语义待修**：`-VoxiaWorldGenPreview` 只启动 `AVoxiaUnifiedVoxelWorldActor` / `production_all_features` 根。完整 XYZ Tile、真实 fence、target latch、有界 near queue、shared `M_VoxelWorldAligned` 与 canonical AO/sky 的门禁继续有效；最终 live/staged/retiring/renderer=`27/0/0/27`，gap/overlap/seam/orphan 与 queue failure 均为 0。后续固定相机证据证明粗 LOD 中心采样会漏掉默认 4m 表层，使 live surface material 从 1 走样为 2；该 A8/A10 内容语义项尚未修复。阶段 2 普通宏格 place/break 与 confirmed receipt 不受影响；阶段 3 prefab 在修复前暂停，Online authority 与里程碑 B/C 仍未开始。
7. **Voxia 是唯一现役客户端，Web / Bevy 已逻辑归档**：默认客户端设计、实现、协议消费验证、联调、CI 与进度判断只看 Voxia；归档目录只保留历史证据，只有用户显式点名时才临时纳入。阶段 2 普通宏格交互已经 closeout；当前客户端先修 Far LOD 外露材质归约，再执行已设计的阶段 3 prefab runtime。Online 生产化仍需独立服务端设计与实施。
