...
6. **Voxia 已切到唯一 Near/Far Patch-diff 架构**：`-VoxiaWorldGenPreview` 只启动 `AVoxiaUnifiedVoxelWorldActor` / `production_all_features` 根。Root 只拥有 `FVoxiaPatchTargetKey` 与派生 readiness；Near 固定 `4³ chunks` Patch，Far 固定 `8³ tiles` Patch；`UVoxiaVoxelPresentationSceneHost` ledger 是 live Patch、exact ownership、canonical boundary slot、seam、component 与 fence 的唯一事实。一个 Near Patch ready 不等待完整 Tile，一个 Far Patch ready 不等待完整 BuildFuture；Far 与固定 26-slot boundary shell 原子提交。相邻移动不按 outside depth、等待时长或队列长度阻塞，只有显式 Relocate loading 阻塞动作。旧 Tile handoff/chunk transaction、target latch、动态 atlas 扩容、full fallback、no-far sink、generation relabel、deterministic retry 与 legacy far runtime/probe 已从 production 删除。核心代码、Development build 与定向自动化已通过；完整 Automation、Null-RHI、Real-RHI 连续移动和长稳证据正在刷新，未刷新前不写成本轮最终 closeout。Far LOD VXP5 exact-surface 材质与合法 ownership MID 父链的既有完成事实不变；Online authority、阶段 3 prefab 与 B/C 尚未开始。
