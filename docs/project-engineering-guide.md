...
> - 当前“此刻为真”的状态见 [`docs/current_status/`](current_status/README.md)。
> - 本文件只提供长期相对稳定的项目概览、技术栈、结构、命令、编码约定，不记录易变的运行时状态。

## 项目概览

基于 Elixir/OTP 构建的 MMORPG 游戏服务集群。Mix umbrella 结构，在 `apps/` 下包含 12 个职责分离的微服务应用。系统使用分布式 Erlang 集群、`libcluster` 自动发现、PostgreSQL（通过 Ecto）作为主持久化路径、自定义二进制协议作为客户端通信格式（见 [`docs/original/2026-04-10-线协议规范.md`](original/2026-04-10-线协议规范.md)），并通过 Rust NIF（Rustler）承载性能敏感的物理与空间计算逻辑。
...
完整协议说明见 [`docs/original/2026-04-10-线协议规范.md`](original/2026-04-10-线协议规范.md)。真值源以 `apps/gate_server/lib/gate_server/codec.ex` 为准（规范文档部分已 stale）。
