...
当前 Voxia 产品化链路另有一个 live BEAM RPC stdio 脚本：

```powershell
elixir --sname voxia_server_cli --cookie mmo scripts/voxia_server_stdio_cli.exs --cmd "snapshot; connections"
```

它不启动第二套服务端，而是连接运行中的节点，跨 Gate / World / Scene / DataService 查询真实状态。LOD projection 相关命令：

- `lod_status <scene_id> [stride]`：读取持久化 projection 覆盖状态。
- `lod_sample <scene_id> <origin_x> <origin_z> <stride> <count_x> <count_z>`：走运行时 heightmap 读路径抽样。
- `lod_rebuild <scene_id> [stride_csv] [batch_size]`：显式 projection materialization/backfill；不得当作运行时 fallback。
