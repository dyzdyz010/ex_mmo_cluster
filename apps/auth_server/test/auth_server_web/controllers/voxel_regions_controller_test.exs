defmodule AuthServerWeb.VoxelRegionsControllerTest do
  ...
  setup do
    ...
    root =
      Path.join(System.tmp_dir!(), "voxim_regions_test_#{System.unique_integer([:positive])}")
    ...
    payload =
      RegionCodec.encode_payload(
        1,
        {0, 1, 0},
        0,
        @content_version,
        :binary.copy(<<7, 0>>, 66 * 66 * 66)
      )

    File.write!(Path.join([world, "L1", "r_0_1_0.vxr"]), payload)
    # 头对不上路径的文件：按 missing 处理，不吐脏数据。
    File.write!(
      Path.join([world, "L0", "r_5_5_5.vxr"]),
      RegionCodec.encode_payload(1, {0, 1, 0}, 0, @content_version, "x")
    )
    ...
    Application.put_env(:world_server, :movement_routes, %{
      7 => %{
        ...
        world_ref: {:voxim_http_regions_test, node()},
        scene_epoch: 1
      }
    })
  ...
  defp request(items, content_version) do
    ...
    IO.iodata_to_binary([
      <<"VXRQ", 1::32-little, content_version::64-little, length(items)::32-little>> | body
    ])
  end
  ...
  test "first fetch returns the payload file byte for byte, unknown regions are missing", %{
    conn: conn,
    payload: payload
  } do
    conn =
      post_regions(
        conn,
        request([{1, {0, 1, 0}, 0, 0}, {1, {9, 9, 9}, 0, 0}, {0, {5, 5, 5}, 0, 0}], 0)
      )
    ...
    assert [
             {:payload, 1, {0, 1, 0}, ^payload},
             {:missing, 1, {9, 9, 9}},
             {:missing, 0, {5, 5, 5}}
           ] = items
  end

  test "matching (seq, hash) under the same content_version is unchanged; anything else is a payload",
       %{conn: conn, payload: payload} do
    {:ok, header} = RegionCodec.decode_payload_header(payload)

    conn1 =
      post_regions(conn, request([{1, {0, 1, 0}, header.seq, header.hash}], @content_version))

    assert {:ok, _, [{:unchanged, 1, {0, 1, 0}}]} = RegionCodec.decode_reply(conn1.resp_body)

    conn2 =
      post_regions(conn, request([{1, {0, 1, 0}, header.seq, header.hash + 1}], @content_version))

    assert {:ok, _, [{:payload, 1, {0, 1, 0}, ^payload}]} =
             RegionCodec.decode_reply(conn2.resp_body)

    # 客户端拿着别的 content_version 的副本：hash 再对也要重发。
    conn3 = post_regions(conn, request([{1, {0, 1, 0}, header.seq, header.hash}], 42))

    assert {:ok, _, [{:payload, 1, {0, 1, 0}, ^payload}]} =
             RegionCodec.decode_reply(conn3.resp_body)
  end
