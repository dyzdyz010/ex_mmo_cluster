...
也不再成倍加快后台定时提交（`transform_world_test` 的循环上限原靠这些额外提交才够，按最慢场景约 2800 s 提到 8000 次）。
根因二：蔓延中的火每次提交 5–7 轮内核，每轮都从零重建热域（全部种子展开、整张节点表与深比较、逐节点默认记录）
并全表扫描燃烧行。提交内种子只增时按新增种子增量扩域；节点默认记录按几何节点与目录/环境标签缓存（相态宏格每次重算）；
燃烧行首轮扫描后按每轮变更行维护；辐射项复用内核下标。节点集合与遍历次序不变，NIF 输入逐位相同，NIF 未改动。
等价性：`thermal_domain_equivalence_test.exs` 用改前 2c4038e7 逐提交记录的摘要核对燃烧木屋（辐射目录 ε 0.9 与
旧目录 ε 0 各一），`thermal_fire_benchmark_test.exs` 的 BENCH_DIGEST 模式核对真实地形林火 300 s 共 60 个摘要，全部相同。
基准（本机 i9-14900KF、真实 GeneratedStore 世界 4ecf1f7864707144、Db 日志、订阅者齐全、自然节拍 375 s）：
`MMO_DB_PORT=5433 mix test --no-start --only benchmark test/thermal_fire_benchmark_test.exs`（BENCH_SCENE=forest|house|qinglan）。
改前→改后：林火（≤2100 节点）实时比 0.887→0.997、计算余量 9.6×→12.9×、回调 p50 52→41 ms；林火+三座木屋同时
（≤2340 节点）0.82→0.998、5.2×→9.4×、p50 88→46 ms、p95 267→167 ms；单座木屋 0.968→1.000。内核每 50 ms 段
1 个子步（每提交 10 个），NIF 3–6 ms/提交。余下成本与待定：全表扫描 `state.damage`（hot/燃尽/相态/转化/电路）
仍按提交 O(全部行)，10 万行约百毫秒级（R8 §8 已定增量维护，需要统一的写入点）；烧毁格的几何事务在
`select_transaction` 为比较大小整块编码 L1–L3 区域（50–200 ms/笔）；60 s 检查点 0.4–0.9 s 与同步 Db 追加尖峰
仍会阻塞 World；温度行按 1 K 阈值复制未做（World 侧广播 ≤1 ms，属线上语义决定）。
...
F08 本次增量已将相变广延量结算归回已有 `Phase`：只消费当前格的材质/焓/完整度、数量、
本次运输量与材料参数，返回格/库存广延量、有限工具能量账及材质/属性结果。
World 仍读取当前占用和属性、接纳工具与余额、盖事务序号，并通过 `apply_batch` 原子提交。
依据 [Elixir 1.18 GenServer 的适用边界](https://hexdocs.pm/elixir/1.18.4/GenServer.html#module-when-not-to-use-a-genserver)：
进程用于运行时状态与并发，代码组织使用模块/函数；这里复用 Phase 的能量模型，不新增状态 owner。
验证范围为纯结算的守恒/端点/不修复性质，以及实际 World 工具、流动、日志失败回滚和冷恢复接缝；
不因职责迁移重跑未受影响的客户端构建或已批准的性能验收。
已实跑：`MMO_DB_PORT=1` 下，`phase_settlement_test.exs` 五项通过，
`phase_world_test.exs` 与 `liquid_world_test.exs` 共 37 项通过（排除既有 realtime 冷板）。
随后定向修复抽取时提前读取环境温度的问题：新增空相变域不配置热环境的真实启动用例，
先复现失败再单独通过；没有重复整套回归。命令和原始日志见 Voxim 工程审计的接续增量。
...
依据本模块既有几何失效实现和[Elixir Map操作契约](https://hexdocs.pm/elixir/Map.html)，
静态派生值随编辑、附件变化、目录发布及冷恢复重建；不缓存权威演化的温度、HP、燃料或焓，不改变NIF事件边界。
缺少稀疏属性行时的默认记录仅从目录、环境配置、实占用几何派生，读取时补当前seq；
与材质的可燃性一并复用，避免每个事件段重复构造默认值。不可燃节点不进入点燃属性读取。
...
B6新增属性后缀17字节及Hello9由客户端同步升级；完整属性快照和增量广播的行 `request_id=0`，输入请求号只用于意图回复／显式查询。燃烧中的余量、温度、HP、库存与能源账随现有日志和检查点恢复，不补算停机时间。B5旧目录只新增三项燃烧参数可兼容发布，已有燃烧参数原地修改仍拒绝；旧建设不重置。早期B6存档没有 `fuel_initialized_j`／`discarded_fuel_j`，这两个累计计数只覆盖字段加入后的行为，不能据其零值推断历史未供能或虚构历史初始化；验收中的原90000J须结合已保留原记录核对。旧版已耗尽但遗留占用的记录在加载后下一热提交结算删除，原剩余燃料不改写。

Test-only回归：`python scripts/b6_authority_tests.py --out .demo/b6-recovery-06`，独立Docker内VM，不加载运行世界、不重启服务。该次83项中38项排除、45项实际执行、0失败，包含真实World订阅→PropertyBatch编码、点燃／熄灭、接触传播、隔离、宏格／微格／木面耗尽、部分回收、冷恢复、旧目录兼容及受影响B3/B4/B5回归；不是45项全部B6，也不替代真实双端／重启／成本验收。`.demo/b6-recovery-04-red/run.log` 保留B3加热至322.7K却未点燃的失败证据，06已修复；实跑入口和最终阶段状态由 [B6验收记录](../../../Voxim/Docs/R7/B6-acceptance.md) 维护。
