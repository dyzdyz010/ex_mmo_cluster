defmodule VoxelRegion.Magic.Catalog do
  ...
  `max_steps`、`max_semblances`、`draw_max_j` 属于共享目录的完整结构，这里只校验存在且为正；
  增量 1 的执行器只接受单步程序、没有拟态，取能上限由 `energy.draw` 的槽上限约束，三者暂无服务端调用方。
  符号只接受服务端已实现的动词（增量 1：`energy.draw`、`act.heat`），目录里出现未实现动词即拒绝启动，
  运行时不再为未知动词设分支。`reading`（Sevara 读法占位）与 `presets` 名称只服务客户端，不参与判定；
  预设程序在这里按同一 `Program.validate/2` 校验，保证发布的预设都可施放。
  """

  alias VoxelRegion.Magic.Program

  # 增量 1 服务端已实现的动词；新动词随其实现一起加入。
  @implemented ~w(energy.draw act.heat)
  ...
  def decode(bytes) do
    ...
    symbols =
      Map.new(data["symbols"], fn s ->
        true = s["id"] in @implemented and s["category"] == "act" and positive?(s["weight"])
        true = s["slots"] != [] and length(Enum.uniq_by(s["slots"], & &1["name"])) == length(s["slots"])
        ...
        {s["id"], %{weight: s["weight"], target: s["target"], slots: slots}}
      end)
