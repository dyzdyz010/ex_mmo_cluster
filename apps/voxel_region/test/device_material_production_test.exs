defmodule VoxelRegion.DeviceMaterialProductionTest do
  @moduledoc """
  只测试：R8-04 增量 1（器件材料化第一步）在 UE 发布的目录字节 `0b2bb0b6…`（DA_MaterialCoverageV1）与生产热环境（ε 0.9，
  数值与 `environment-radiation.json` 相同）上核对。电阻合金 40：σ 4 S/m、λ 0.2、C 40 kJ/(m³K)、k 25、耐热 1900 K；
  铁矿石 17 在 1373.15 K 接触煤时单向转化成 40。

  场景经作者编辑入口 / place_prefab 一次建立；面、设备、投料、开关都走正式附件与工具入口；时间只经 :thermal_commit 推进。
  ...
  @digest "0b2bb0b6e47e5fe52aa4393f9e8eaae7f68aac7cd42af01f8269df89a725f172"
  ...
  defp start(c, name) do
    ...
    w = start_supervised!({World, [source: VoxelRegion.TestSupport.Source, log: NullLog, root: root, observer: self(),
      ...
      production_materials: [@stone, @coal, @iron_ore, @wood, @copper, @alloy]]}, id: name)
  ...
  # 在块的西面（轴 0）与北面（轴 2）各铺一块 8×8 铜面，装源与开关；两者共用西北下角，源的另一端在西北上角、开关的另一端在东北下角。
  defp devices(w, a, corner, source_tool, seq0) do
    ...
    [source, switch] =
      for {axis, i} <- Enum.with_index([0, 2]) do
        {:ok, id} = World.attachment_intent(w, a, %{face | axis: axis, request_id: seq0 + i, client_intent_seq: seq0 + i})
        {id, axis, corner}
      end
    ...
    use.(switch, 4, seq0 + 11)
    use.(switch, 7, seq0 + 12)      # 打开
    use.(source, 8, seq0 + 13)      # 投料两次
    use.(source, 8, seq0 + 14)
    %{toggle: fn seq -> use.(switch, 7, seq) end}
  end
  ...
  # 目录手算的串联电阻：源内阻 + 开关 + 两个端口落在铜微格的宿主半格 + 铜—铜/铜—合金/合金—合金接触。
  ...
  test "灯：石块上一条铜微格线中间一格电阻合金，24 V 源 + 开关；电流按欧姆定律，灯丝得到自身 I²R，λ 份额记为光，断开即熄", c do
    ...
    place_micro(w, prefabs, {16, 8, 16}, top ++ down)
    ...
    lit = commits(w, 4, box)
    t = c.tools
    # 14 个铜—铜接触（顶线 5、拐角两侧 2、下行 7），2 个铜—合金接触，2 个宿主半格。
    r = t[3]["circuit_resistance_ohm"] + t[4]["circuit_resistance_ohm"] + 2 * copper_host(c) +
      14 * contact(c, @copper, @copper) + 2 * contact(c, @copper, @alloy)
  ...
  test "发热板：木块顶面 8×6 电阻合金微格夹在两根铜排之间（1.5 Ω），480 V 源装在木块上；付费电能耗尽前点燃木块", c do
    ...
    down = [{{8, 8, 0}, @copper}] ++ for(y <- 0..7, do: {{8, y, 0}, @copper})
    place_micro(w, prefabs, {24, 8, 24}, slab ++ down)
    ...
    current = t[19]["circuit_voltage_v"] / (t[19]["circuit_resistance_ohm"] + t[4]["circuit_resistance_ohm"] + row_r / 8)
