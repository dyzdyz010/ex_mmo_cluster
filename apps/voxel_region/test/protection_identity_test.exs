defmodule VoxelRegion.ProtectionIdentityTest do
  @moduledoc """
  只测试：没有受保护区域时，热（含辐射与燃烧）、液体、电路与冶炼的提交与引入受保护区域前的主线逐位相同。
  ...
  # 电路回路：源（24 V）装在 19 号木头顶面，21 号木头顶面是一块普通铜面（R8-04 增量 2 撤下加热器设备后不再装设备），
  # 铜块与回程铜条闭合。这一场景的记录在 1b2a10c8 上用同一改动后的场景重新录得（见 @recorded 注释）。
  defp circuit(c, name, _far?) do
    w = start(c, name)
    row = [{{18, 1, 0}, 24}, {{19, 1, 0}, 19}, {{20, 1, 0}, 24}, {{21, 1, 0}, 19}, {{22, 1, 0}, 24}]
    back = [{{18, 1, 1}, 24}, {{22, 1, 1}, 24}] ++ for(x <- 18..22, do: {{x, 1, 2}, 24})
    {:ok, _} = World.apply_edits(w, row ++ back)
    a = actor({19.5, 3.5, 0.5})
    faces = for x <- [19, 21] do
      seq = next()
      :ok = GenServer.call(a.player, {:eye, {x + 0.5, 3.5, 0.5}})
      {:ok, id} = World.attachment_intent(w, %{a | eye: {x + 0.5, 3.5, 0.5}}, %{request_id: seq, client_intent_seq: seq,
        logical_scene_id: 1, action: 0, kind: 0, axis: 1, size: 8, anchor: {x * 8, 16, 0}, id: 0, material: 24, tool_id: 1})
      {x, id}
    end
    fed = for {x, tool} <- [{19, 3}, {19, 8}] do
      seq = next()
      id = faces |> Map.new() |> Map.fetch!(x)
      :ok = GenServer.call(a.player, {:eye, {x + 0.5, 3.5, 0.5}})
      {:ok, _} = done = World.tool_intent(w, stamp(%{a | eye: {x + 0.5, 3.5, 0.5}}, seq), %{request_id: seq, client_intent_seq: seq,
        logical_scene_id: 1, action: 1, tool_id: tool, direction: {0.0, -1.0, 0.0}, micro: {x * 8, 16, 0}, granularity: 3,
        incarnation: id, owner: {id, 1}, material: 24})
      elem(done, 1)
    end
    trace(w, elapsed_at(w, List.last(fed)), 5.0)
  end
  ...
  # circuit 一项在 R8-04 增量 2（撤下加热器设备）时按改后的场景在 1b2a10c8 的临时 worktree 上重新录两次，结果相同。
  @recorded %{
    ...
    circuit: %{0.5 => "45eafc768a16412b", 1.0 => "2b15b7d77a2fa21e", 1.5 => "1735cc19bc9e4ace", 2.0 => "24749d0f5bfb46d7", 2.5 => "e8f9435fd81b7da2", 3.0 => "79748750df8dcba3", 3.5 => "487ac1c158a67af4", 4.0 => "630e43152840ccbe", 4.5 => "c86efd8adaf26d68", 5.0 => "35bb0e2a330dd0a6"},
  ...
  for {scenario, far?} <- [fire: false, circuit: false, liquid: false, ore: false, fire: true, liquid: true, ore: true] do
    @tag :identity
  ...
  def run(:circuit, c, name, far?), do: circuit(c, name, far?)
