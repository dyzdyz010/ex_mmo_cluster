defmodule VoxelRegion.MagicWorldTest do
  ...
  魔法 `8f418a41…`（Test-only 手写，结构与数值同契约初版）；热环境 = 生产 ε 0.9。
  ...
  defp spell(c, a, micro, program, opts \\ []) do
    ...
    World.spell_intent(c.w, Map.merge(a, %{received_us: Keyword.get(opts, :at, seq * 1_000_000), clock_node: node()}), request)
  end

  @stone_micro {20, 12, 4}
  @leaf_micro {4, 20, 28}

  test "取能：10 MJ 石取 1 MJ、η 0.9 → 施法者得 0.9 MJ（付 2000 J 控制开销后余 898 000 J）、石格热源 0.1 MJ、石剩 9 MJ；热提交后两处有限热源全部进世界", c do
    before = observe(c.w)
    assert {:ok, %{outcome: nil, seq: seq, caster: caster}} = spell(c, c.a, @stone_micro, draw_program(1_000_000))
    assert seq == before.seq + 1
    s = observe(c.w)
    # ΔE = min(1 MJ, 10 MJ, 5 MJ − 0) = 1 MJ；得 0.9 × 1 MJ = 900 000 J；E_ctl = 2000 × (1 + 0/1e6)^1.5 = 2000 J。
    assert_in_delta caster.energy_j, 898_000.0, 1.0e-6
    assert {caster.spent_j, caster.quote_j, caster.quote_s, caster.capacity_j, caster.coherence} == {2000.0, 2000.0, 1.0, 5.0e6, 4.0}
    assert_in_delta energy(c), 898_000.0, 1.0e-6
    ...
    assert ledger(s, :cast_waste_j) == 2000.0
    # 验收式：石减少 1 MJ = 施法者取得 + 取能损耗；施法支出 2000 J = spell_heat_j (0) + cast_waste_j。
    assert_in_delta ledger(s, :caster_drawn_j) + ledger(s, :draw_loss_j), 1.0e7 - battery(s).stored_j, 1.0e-6
    # 有限热源：石格 0.1 MJ、脚下 2000 J，都在一个 500 ms 提交内放完（功率 = 能量 / 0.5 s）。
    ...
    assert s.thermal.sources[{0, 0, 0}] == %{remaining_j: 2000.0, power_w: 4000.0}
    after_commit = commit(c.w)
    assert after_commit.thermal.sources == %{}
    assert_in_delta after_commit.thermal.supplied_j - s.thermal.supplied_j, 102_000.0, 1.0e-3
  end
  ...
  test "远程点火：孤立叶宏格 0.4 MJ / 200 kW → 着火；投入全部经热源进世界，账与余额一致", c do
    ...
    # 总支出 = 0.4 MJ + 2000 × 1.4^1.5 (= 3313.0047 J) = 403 313.0047 J；余额 898 000 − 403 313.0047 = 494 686.9953 J。
    assert_in_delta caster.spent_j, 403_313.0047, 1.0e-4
    assert_in_delta caster.energy_j, 494_686.9953, 1.0e-4
    s = observe(c.w)
    assert s.thermal.sources[{0, 2, 3}] == %{remaining_j: 400_000.0, power_w: 200_000.0}
    assert ledger(s, :spell_heat_j) == 400_000.0
    assert_in_delta ledger(s, :cast_waste_j), 2000.0 + 3313.0047, 1.0e-4
    # 同一目标已有热源：第二次点火拒绝，不扣能量。
    assert {:error, :heat_source_busy} = spell(c, c.a, @leaf_micro, heat_program(400_000), at: 3_000_000)
    assert_in_delta energy(c), 494_686.9953, 1.0e-4
    ...
    # 热源放完后：本次施法进入世界的热 = 0.4 MJ（目标）+ 3313.0047 J（脚下）。内核供热账 supplied_j 含燃烧放热，
    ...
    assert_in_delta fed.(done) - fed.(s0), 403_313.0047, 1.0e-2
  ...
  test "走火：余额 100 kJ、成本 302 964.4 J → 余额 0、脚下热源 100 kJ、目标无热源；结果为已提交事务", c do
    # 余额 100 kJ：取能 R 使 0.9 R − 2000 = 100 000 → R = 113 333.33… J。
    {:ok, _} = spell(c, c.a, @stone_micro, draw_program(113_333.33333333333), at: 1_000_000)
    assert_in_delta energy(c), 100_000.0, 1.0e-6
    s0 = commit(c.w)
    waste0 = ledger(s0, :cast_waste_j)
    # 0.3 MJ 加热：总支出 = 300 000 + 2000 × 1.3^1.5 (= 2964.4 J) = 302 964.4 J > 100 000 J → misfire_energy。
    ...
    assert_in_delta caster.quote_j, 302_964.4, 0.1
    s = observe(c.w)
  ...
  test "拒绝不扣能量、不改世界：他人地块、超出施法域、射线未命中、施法过快、目录过期、程序非法、目标不是蓄能石；报价不改世界", c do
    ...
    assert {:error, :out_of_domain} = World.spell_intent(c.w, Map.merge(c.a, %{received_us: 3_000_000, clock_node: node()}), sky)
    # 取能只作用于蓄能石。
    ...
    assert_in_delta q.quote_j, 403_313.0047, 1.0e-4
