defmodule MmoContracts.LiquidWireTest do
  ...
  test "B7 Hello11 and explicit scoop/pour retain the production envelope" do
    ...
    for action <- [2,3] do
      ...
      assert {:ok,{:voxel_production_intent,%{action: ^action,coord: {-1,542,3},material: 21}}}=Voxel.Codec.decode(wire)
    end
    assert {:error,:invalid_message}=Voxel.Codec.decode(<<0x7F,19::64,7::32,1::64,4,0::96,11::16,21::16>>)
  end
  ...
  test "VXR9 quantity uses same region seq/hash and sorted little-endian suffix" do
    ...
    assert binary_part(raw,byte_size(raw)-12,12)==<<1::little-32,index::little-32,128::little-32>>
    assert {:ok,%{liquid_units: %{^index=>128},region: {-1,8,0},seq: 47}}=Payload.decode(packet)
  ...
  test "legacy water retains implicit full occupancy and quantity records reject impossible shapes" do
    ...
    for invalid <- [%{index=>0},%{index+1=>128}] do
      assert {:error,:invalid_payload}=Payload.decode(Payload.encode(%{payload|liquid_units: invalid},%{},47,9))
    end
