defmodule MmoContracts.VoximM1ContractTest do
    ...
    for {name, domain, value} <- VoximM1Vectors.vectors(), producer <- ["elixir", "ue"] do
      ...
      golden =
        if name == "hello" do
          ...
          replace(golden, 9, <<Codec.protocol_version()::16>>)
        else
          golden
        end
  ...
  test "reasons have exact authenticated and pre-auth representation and reserve unknown values" do
    ...
    assert {:error, :invalid_m1_message} = Codec.decode(replace(hello, 9, <<12::16>>))
    version = Codec.protocol_version()
    assert {:ok, %Session.Hello{protocol_version: ^version}} = Codec.decode(hello)
