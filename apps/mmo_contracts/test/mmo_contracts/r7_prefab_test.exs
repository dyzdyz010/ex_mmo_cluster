defmodule MmoContracts.R7PrefabTest do
  ...
  test "packed CSR records and value overrides produce identical complete bytes" do
    ...
    bare = %Payload{
      ...
      region: {0, 0, 0},
      map_extent: 2,
      cells: :binary.copy(<<19::16-little>>, 66 * 66 * 66)
    }
    ...
    for mask <- 0..63, uniform <- [false, true] do
      maps =
        for face <- 0..5,
            into: <<>>,
            do: if(uniform, do: :binary.copy(<<face + 1>>, 4), else: <<1, 2, 3, 4>>)

      fmi = for face <- 0..5, (mask &&& 1 <<< face) != 0, into: <<>>, do: <<face::16-little>>

      p = %{
        bare
        | records: %{{2, 3, 4} => {ids, mask, 0}, {4, 3, 4} => {ids, mask, 0}},
          fmi: fmi,
          maps: maps
      }

      edits = %{
        {4, 3, 4} => {19, Skins.uniform(19)},
        {1, 3, 4} => {19, Payload.skins(p, {2, 3, 4}, 19)}
      }
  ...
  test "packed skins preserve every mask and canonicalize uniform maps" do
    ...
    for mask <- 0..63, uniform <- [false, true] do
      maps =
        for face <- 0..5,
            into: <<>>,
            do: if(uniform, do: :binary.copy(<<face + 1>>, 4), else: <<face + 1, 9, 8, 7>>)

      indices = for face <- 0..5, (mask &&& 1 <<< face) != 0, into: <<>>, do: <<face::16-little>>

      p = %Payload{
        records: %{{1, 2, 3} => {ids, mask, 1}},
        map_extent: 2,
        fmi: <<5::16-little, indices::binary>>,
        maps: maps
      }
      ...
      assert Payload.skins(p, {1, 2, 3}, 19) ==
               MmoContracts.Voxel.Skins.canonical({2, List.to_tuple(faces)})
