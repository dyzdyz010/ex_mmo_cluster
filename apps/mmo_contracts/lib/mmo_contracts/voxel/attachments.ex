defmodule MmoContracts.Voxel.Attachments do
  ...
  defp slots(<<kind, axis, x::signed-little-64, y::signed-little-64, z::signed-little-64,
               id::little-64, material::little-16, rest::binary>>, previous, acc)
       when kind in [0,1] and axis in 0..2 and id > 0 do
    key = {kind,axis,{x,y,z}}
    if (previous == nil or key > previous) and material?(material),
      do: slots(rest,key,Map.put(acc,key,{id,material})), else: {:error,:invalid_attachments}
  end
