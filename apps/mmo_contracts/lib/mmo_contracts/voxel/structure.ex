defmodule MmoContracts.Voxel.Structure do
  ...
  def encode(grids),
    do:
      IO.iodata_to_binary([
        <<2::32-little, map_size(grids)::32-little>>,
        for(
          {i, g} <- Enum.sort(grids),
          do: [<<i::32-little, div(byte_size(g), 2)::32-little>>, g]
        )
      ])

  @doc "协议边界一次校验版本、排序、材质位和实际结构占用。"
  def decode(<<1::32-little, n::32-little, rest::binary>>) when n > 0,
    do: records(rest, n, -1, %{})

  def decode(<<2::32-little, n::32-little, rest::binary>>) when n > 0,
    do: records_v2(rest, n, -1, %{})

  def decode(_), do: {:error, :invalid_structure}
  defp records(<<>>, 0, _, grids), do: {:ok, grids}

  defp records(<<i::32-little, g::binary-size(@samples * 2), rest::binary>>, n, previous, grids)
       when n > 0 and i > previous and i < 66 * 66 * 66 do
    values = for <<v::16-little <- g>>, do: v

    if Enum.all?(values, &(&1 <= 511 and &1 != @flag)) and
         Enum.any?(values, &((&1 &&& @flag) != 0)),
       do: records(rest, n - 1, i, Map.put(grids, i, g)),
  ...
  defp records_v2(<<i::32-little, count::32-little, rest::binary>>, n, previous, grids)
       when n > 0 and i > previous and i < 66 * 66 * 66 and count in [@samples, @samples * 7] and
              byte_size(rest) >= count * 2 do
    <<g::binary-size(count * 2), tail::binary>> = rest

    if valid?(g),
      do: records_v2(tail, n - 1, i, Map.put(grids, i, g)),
      else: {:error, :invalid_structure}
  end
