...
defmodule SceneServer.Movement.PrefabCollisionTest do
  ...
  test "canonical delta installs micro collision atomically and retains historical revision", %{
    id: id,
    opts: opts
  } do
    ...
    assert_receive {:canonical_delta,
                    %CanonicalDelta{transaction_seq: 1, chunks: chunks} = placed}

    assert Enum.map(chunks, &{&1.coord, &1.n, &1.scale_m}) == [
             {{0, 0, 0}, 128, 0.125},
             {{1, 0, 0}, 128, 0.125}
           ]

    {updates, events} =
      updates |> CollisionUpdates.enqueue(placed, 0) |> CollisionUpdates.consume(0)
    ...
    {updates, events} =
      updates |> CollisionUpdates.enqueue(removed, 1) |> CollisionUpdates.consume(1)
