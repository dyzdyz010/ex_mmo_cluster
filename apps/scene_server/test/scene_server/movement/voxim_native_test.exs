defmodule SceneServer.Movement.VoximNativeTest do
  ...
  defp step(world, p, state),
    do: Native.step_characters(world, p, [{1, state, {0.0, 0.0, 0}}]) |> hd() |> elem(1)
  ...
  test "actual NIF shares world, restores states, and removes all-air/BVH support" do
    ...
    assert [{1, {_, {_, vy, _}, _}}, {2, {_, _, 1}}] =
             result = Native.step_characters(world, p, chars)

    assert vy > 0
    assert result == Native.step_characters(world, p, chars)

    world =
      Native.set_chunks(world, [
        {:set, {0, 0, 0}, 2, 1.0, {0.0, 0.0, 0.0}, <<0::64>>},
        {:remove, {1, 0, 0}}
      ])
    ...
    for start <- [a, b] do
      assert {{_, y, _}, {_, vy, _}, 0} =
               Enum.reduce(1..30, start, fn _, s -> step(world, p, s) end)
  ...
  test "boundary rejects a whole malformed batch without partial mutation" do
    ...
    for ops <- [
          [floor_op(1), floor_op(0)],
          [floor_op(0), floor_op(0)],
          [floor_op(0), {:set, {1, 0, 0}, 2, 1.0, {2.0, 0.0, 0.0}, <<1>>}]
        ] do
    ...
    assert_raise ArgumentError, fn ->
      Native.step_characters(world, p, [{1, s, {0.0, 0.0, 2}}])
    end

    assert_raise ArgumentError, fn ->
      Native.step_characters(world, p, [{2, s, {0.0, 0.0, 0}}, {1, s, {0.0, 0.0, 0}}])
    end
  ...
  test "two-character dirty CPU call cost is measured online" do
    ...
    operations =
      for x <- 0..7,
          y <- 0..7,
          z <- 0..7,
          do: {:set, {x, y, z}, 16, 1.0, {x * 16.0, y * 16.0, z * 16.0}, cells}
    ...
    {us, states} =
      :timer.tc(fn ->
        Enum.reduce(1..600, [{1, a}, {2, b}], fn _, states ->
          Native.step_characters(
            world,
            p,
            Enum.map(states, fn {id, s} -> {id, s, {0.0, 0.0, 0}} end)
          )
        end)
      end)

    IO.puts(
      "P1_NIF synthetic_chunks=512 cells_bytes=#{512 * byte_size(cells)} build_us=#{build_us} two_characters=2 steps=600 total_us=#{us} us_per_call=#{us / 600} final=#{inspect(states)}"
    )
  ...
  test "online input and edit replay exports bit-exact two-character states" do
    ...
    {_, _, trace} =
      Enum.reduce(1..240, {world, [{1, a}, {2, b}], []}, fn tick, {world, states, trace} ->
        world =
          if tick == 90,
            do: Native.set_chunks(world, [{:remove, {0, 0, 0}}, {:remove, {1, 0, 0}}]),
            else: world

        world =
          if tick == 150, do: Native.set_chunks(world, [floor_op(0), floor_op(1)]), else: world

        characters =
          Enum.map(states, fn {id, state} ->
        ...
        {world, next,
         [
           Enum.map(next, fn {id, {{x, y, z}, {vx, vy, vz}, grounded}} ->
             <<tick::32, id::64, x::float-64, y::float-64, z::float-64, vx::float-64,
               vy::float-64, vz::float-64, grounded::32>>
           end)
           | trace
         ]}
      end)
    ...
    IO.puts(
      "P1_REPLAY ticks=240 characters=2 bytes=#{byte_size(bytes)} sha256=#{Base.encode16(:crypto.hash(:sha256, bytes), case: :lower)}"
    )
