  ...
  test "remote Scene derives collision from the same canonical World without moving native source state" do
    ...
    nodes =
      Enum.reduce(1..2, [], fn id, previous ->
        {:ok, peer, host} =
          :peer.start_link(%{
            name: String.to_atom("m4a_shared_#{id}_#{System.unique_integer([:positive])}"),
        ...
        info =
          :peer.call(peer, SceneServer.Movement.VoximNeighbourNodesProbe, :boot, [base, id, world])
  ...
  test "two BEAM Scene/World nodes move through P1 and publish neighbouring authority to their own observers" do
    ...
    peers =
      for id <- 1..2 do
        {:ok, peer, node} =
          :peer.start_link(%{
            name: String.to_atom("m4a_scene_#{id}_#{System.unique_integer([:positive])}"),
            connection: :standard_io,
            args: [~c"+S", ~c"2:2"]
          })
    ...
    routes =
      Map.new(Enum.with_index(peers, 1), fn {p, id} ->
    ...
    players =
      for {p, id} <- Enum.with_index(peers, 1) do
        ...
        assert_receive {:mmo_reliable, ^identity, 2, %MmoContracts.Voxel.CanonicalBootstrap{}},
                       15_000
    ...
    samples =
      for batch <- 0..19 do
        for p <- players do
          ...
          Player.input(p.pid, p.identity, %Movement.InputBatch{
            identity: p.identity,
            frames: frames
          })
        end
        ...
        IO.puts(
          "M4A_SAMPLE " <>
            inspect(Enum.map(values, &Map.take(&1, [:entity_id, :state, :processed_input_seq])))
        )
    ...
    assert Enum.all?(0..1, fn i ->
             Enum.any?(samples, fn pair ->
               elem(Enum.at(pair, i).state.position, 1) >
                 elem(Enum.at(players, i).initial.position, 1) + 0.5
             end)
           end)
    ...
    assert Enum.all?(0..1, fn i ->
             Enum.any?(samples, fn pair ->
               abs(
                 elem(Enum.at(pair, i).state.position, 2) -
                   elem(Enum.at(players, i).initial.position, 2)
               ) > 1
             end)
           end)

    for {p, remote_id} <- Enum.zip(peers, [2, 1]) do
      assert [%{visible: [%{entity_id: ^remote_id}]}] =
               Replication.observe(Scene.observe(p.scene).replication_pid)
    end

    events = drain([])

    snapshots =
      for {:mmo_datagram, identity, %Movement.Snapshot{} = s} <- events, do: {identity, s}

    for {local, remote, offset} <- [
          {hd(players), List.last(final), be.origin_us - ae.origin_us},
          {List.last(players), hd(final), ae.origin_us - be.origin_us}
        ] do
      matching =
        for {identity, s} <- snapshots,
            identity == local.identity,
            Enum.any?(s.records, &(&1.entity_id == remote.entity_id)),
            do: s
    ...
    summary = %{
      nodes: Enum.map(peers, &%{node: Atom.to_string(&1.node), os_pid: &1.os_pid}),
      origin_offset_us: be.origin_us - ae.origin_us,
      snapshots: length(snapshots),
      processed: Enum.map(final, & &1.processed_input_seq),
      steps: Enum.map(final, & &1.physics_steps),
      sources: "GeneratedStore + independent file overlay; production Scene/Player/P1",
      boundary_x_m: 41,
      transport: "distributed BEAM + production Sink; no UE/QUIC",
      snapshot_body_bytes:
        Enum.sum(
          Enum.map(snapshots, fn {_, s} ->
