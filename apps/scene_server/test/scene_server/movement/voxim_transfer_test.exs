    ...
    test "a boundary cut transfers the unprocessed prefix and activates exactly one writer edits=#{edit_during_transfer}",
      ...
      world =
        start_supervised!(
          {VoxelRegion.World,
           [
             name: :m4a_transfer_world,
             source: VoxelRegion.GeneratedStore,
             root: base <> "/transfer-world",
      ...
      if edit_during_transfer do
        ...
        for level <- 1..5 do
          ...
          :ok =
            VoxelRegion.GeneratedStore.bake_region(
              store,
              level,
          ...
          replica =
            start_supervised!(
              Supervisor.child_spec(
                {VoxelRegion.Replica, [authority_ref: world, l0_box: box, name: nil]},
                id: {:replica, id}
              )
            )
      ...
      frames =
        for seq <- 1..120,
            do: %Movement.InputFrame{
              input_seq: seq,
              axis_x: if(seq <= 45, do: 32767, else: 0),
              axis_z: 0,
              yaw: 0,
              jump_pressed: 0
            }
      ...
      if edit_during_transfer do
        ...
        assert [
                 %{transaction_seq: 2, collision_revision: 3, apply_tick: second_tick},
                 %{transaction_seq: 3, collision_revision: 4, apply_tick: third_tick}
               ] = applied
  ...
  defp through_fence(identity, seq, acc \\ []) do
    receive do
      {:mmo_reliable, ^identity, 2, event} ->
        case event do
          %MmoContracts.Voxel.TimelineFence{transaction_seq: n} when n >= seq ->
            Enum.reverse([event | acc])

          _ ->
            through_fence(identity, seq, [event | acc])
        end
    after
