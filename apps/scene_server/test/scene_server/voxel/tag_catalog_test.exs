defmodule SceneServer.Voxel.TagCatalogTest do
  ...
  describe "seed loading" do
    test "loads all 8 tags from priv/catalogs/tag_catalog_v1.exs", %{server: server} do
      snapshot = TagCatalog.current_snapshot(server)
      assert %TagCatalogSnapshot{} = snapshot
      assert snapshot.catalog_version == 1
      assert length(snapshot.definitions) == 8
    end

    test "catalog_version returns 1", %{server: server} do
      assert TagCatalog.catalog_version(server) == 1
    end

    test "definitions are sorted by id ascending and cover 1..8", %{server: server} do
      snapshot = TagCatalog.current_snapshot(server)
  ...
  describe "current_snapshot byte-stable encoding" do
    test "matches TagCatalogSnapshot.normalize! invariants", %{server: server} do
      ...
      assert length(decoded.definitions) == 8
