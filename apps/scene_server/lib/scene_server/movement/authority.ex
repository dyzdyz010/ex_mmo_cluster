defmodule SceneServer.Movement.Authority do
  ...
  def partition({lo, hi}, {other_lo, other_hi}) do
    axis = Enum.find(0..2, &(elem(hi, &1) == elem(other_lo, &1) or elem(lo, &1) == elem(other_hi, &1)))
    if elem(hi, axis) == elem(other_lo, axis),
      do: {{:halfspace, axis, elem(hi, axis), :below}, {:halfspace, axis, elem(hi, axis), :above}},
      else: {{:halfspace, axis, elem(lo, axis), :above}, {:halfspace, axis, elem(lo, axis), :below}}
  end
end
