...
- **C-1** 顺序数字 id：attribute 1..19 / tag 1..8
...
| 11 | `electric_conductivity` | `MS/m` | material_default | false | 0.0 |
...
3 = AttributeCatalog.catalog_version()
```
...
- Phase 7.D3 起，`FieldTickWorker` 不再静默丢弃 non-observe kernel effects：
  ...
  ash 或清空格子。燃烧产生的持续热源以 `source_kind: :combustion` 写回同一个 FieldRegion
  的 temperature source_points，下一 tick 继续参与温度传播。
  ...
  `voxel_field_effect_rejected` 明确拒绝。燃烧把 `structural_integrity` 跌破材料阈值时
  产生的 `voxel_structural_collapse_candidate` 作为后续 object / prefab 坍塌结算入口，
  当前不直接修改 object truth。
