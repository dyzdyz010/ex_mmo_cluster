...
- `mix scene_server.natural_phenomenon_observe --scenario spread --coord 0,0,0`
  ...
  变成可燃物。
- `node scripts/run_browser_combustion_field_smoke_supervised.js` 提供真实浏览器验收面：
  脚本会启动 DB / umbrella / Vite / Chromium，通过浏览器 CLI 订阅 chunk、放置
  wood / dry_grass / cloth / stone，对 wood 提交 1150°C `voxel_temp`，再用
  `field_overlay` 和 `voxel_combustion` 证明浏览器确实从服务端 0x73 场域快照看到了
  `smoke_density`、oxygen deficit，以及 stone 仍是不可燃控制组。结构化结果写入
  `.demo/observe/browser-combustion-field-smoke-summary.json`。
- 腐蚀也有同类只读真值入口：
