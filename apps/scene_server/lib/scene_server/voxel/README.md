...
`prepare_transaction/3` 接收一份**意图清单 (intents 列表)**，全部归一化 + 通过 batch
...
`:chunk_already_fenced`。**fence 不持久化**（Phase 3 D4）：节点重启或 fence 已被
`abort_transaction/2` 释放后，再次 `commit_transaction/2` 会返回
`:transaction_not_prepared`，让上层把这次跨 chunk 事务视为部分失败并整体回滚。这一对
API 只面向上层 `BuildTransactionApplier`，不暴露给 Gate/玩家路径。
...
后续切片会在同一子树下补充紧凑区块增量、ChunkProcess fence 持久化（让节点重启后
in-flight 事务可被恢复），以及更完整的迁移回滚。
