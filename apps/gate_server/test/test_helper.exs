...
# :live_llm 调用真实模型接口（NPC_LLM_URL / NPC_LLM_KEY / NPC_LLM_MODEL / 可选 NPC_LLM_EFFORT），只在显式 --include 时运行。
# :live_jev 调用真实 Jev 接口（TYPESAFE_API_URL / TYPESAFE_API_KEY / TYPESAFE_MODEL），只在显式 --include 时运行。
# :npc_scale 是多 NPC 成本测量（打印数字，约 1 分钟），只在显式 --include 时运行。
ExUnit.start(assert_receive_timeout: 1_000, exclude: [:live_llm, :live_jev, :npc_scale])
