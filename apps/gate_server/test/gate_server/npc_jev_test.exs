defmodule GateServer.NpcJevTest do
  ...
  test "request: one state, typed questions; harm is asked only when an action is planned" do
    body = Jev.body(%{model: "jev-latest"}, "The last block was placed.")
    assert "jev-latest" == body.model
    assert "The last block was placed." == body.state
    assert [:activity] == body.questions |> Map.keys() |> Enum.sort()
    assert Enum.sort(Jev.activities()) == body.questions.activity.criteria |> Map.keys() |> Enum.sort()

    body = Jev.body(%{model: "m"}, "Out of stone.", "mine the wall next door")
  ...
  test "verdict: confident routine choices act; doubt or surprise goes to the planner; possible harm to others blocks" do
    assert {:act, :continue_building} == Jev.verdict(response("continue_building", 0.97))
    assert {:act, :fetch_material} == Jev.verdict(response("fetch_material", 0.85))
    # 差一点不到阈值：不硬选。
    assert {:escalate, :low_confidence} == Jev.verdict(response("fetch_material", 0.84))
    # 选了 replan：交给规划者。
    assert {:escalate, :unexpected} == Jev.verdict(response("replan", 0.99))
    # harm：只有明确判否（≤ 0.15）才放行；0.16 就拦下，且优先于其他一切。
    assert {:act, :continue_building} == Jev.verdict(response("continue_building", 0.99, 0.15))
    assert {:blocked, :harms_others} == Jev.verdict(response("continue_building", 0.99, 0.16))
    assert {:blocked, :harms_others} == Jev.verdict(response("replan", 0.3, 0.9))
  end
  ...
  test "ask goes through the injected sender and returns the verdict with the raw response" do
    ...
    assert {:ok, {:act, :move_to_safety}, %{"answers" => _}} = Jev.ask(%{model: "m"}, "s", nil, sender)
    assert {:error, {429, "slow down"}} == Jev.ask(%{model: "m"}, "s", nil, fn _, _ -> {:error, {429, "slow down"}} end)
  end
  ...
  test "the real model on the hand-labelled scheduling cases: no wrong automatic decision, at least 80 % automatic" do
    ...
    results =
      for c <- cases do
        {:ok, verdict, _} = Jev.ask(endpoint, c["state"])
        expected = if c["expected"] == "replan", do: {:escalate, :unexpected}, else: {:act, String.to_atom(c["expected"])}
  ...
  test "the real model: routine, danger, shortage, surprise and harm to others" do
    ...
    for {situation, action, expected} <- [
          ...
          {"You looked at the site: the entire east wall that was finished earlier is gone.", nil, {:escalate, :unexpected}},
      ...
      {:ok, verdict, %{"answers" => answers}} = Jev.ask(endpoint, base <> situation, action)
