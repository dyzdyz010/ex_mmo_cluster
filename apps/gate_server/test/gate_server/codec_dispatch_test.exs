defmodule GateServer.CodecDispatchTest do
  ...
  describe "protocol routing by first byte" do
    test "movement input message (0x01) is in codec range" do
      ...
      assert {:ok, {:movement_input, %{seq: 9, client_tick: 100}}} = Dispatch.decode(msg)
    end
    ...
    test "enter_scene message (0x02) is in codec range" do
      ...
      <<type::8, _::binary>> = msg
      assert type >= 0x01 and type <= 0x7F
      assert {:ok, {:enter_scene, 42, 7}} = Dispatch.decode(msg)
    end
    ...
    test "time_sync message (0x03) is in codec range" do
      ...
      <<type::8, _::binary>> = msg
      assert type >= 0x01 and type <= 0x7F
      assert {:ok, {:time_sync, 8, 999}} = Dispatch.decode(msg)
    end
    ...
    test "heartbeat message (0x04) is in codec range" do
      ...
      <<type::8, _::binary>> = msg
      assert type >= 0x01 and type <= 0x7F
      assert {:ok, {:heartbeat, 999}} = Dispatch.decode(msg)
    end
    ...
    test "auth_request with request_id is in codec range" do
      ...
      <<type::8, _::binary>> = msg
      assert type >= 0x01 and type <= 0x7F
      assert {:ok, {:auth_request, "test", "token", 7}} = Dispatch.decode(msg)
    end
    ...
    test "fast-lane request is in codec range" do
      ...
      <<type::8, _::binary>> = msg
      assert type >= 0x01 and type <= 0x7F
      assert {:ok, {:fast_lane_request, 7}} = Dispatch.decode(msg)
    end
    ...
    test "chat_say is in codec range" do
      ...
      <<type::8, _::binary>> = msg
      assert type >= 0x01 and type <= 0x7F
      assert {:ok, {:chat_say, "hi", 11}} = Dispatch.decode(msg)
    end
  ...
  describe "server response encoding for codec dispatch" do
    ...
    test "enter_scene_result success encodes with location and expected_seq" do
      {:ok, bin} = Sink.encode({:enter_scene_result, :ok, 7, {100.0, 200.0, 90.0}, 42})
  ...
  describe "end-to-end codec flow" do
    test "movement input and ack preserve sequencing metadata" do
      ...
      assert {:ok, {:movement_input, %{seq: 73, client_tick: 1000}}} = Dispatch.decode(client_msg)
    ...
    test "auth flow echoes request_id in result" do
      ...
      {:ok, {:auth_request, "user", "token", request_id}} = Dispatch.decode(client_msg)
      assert request_id == 99

      {:ok, response} = Sink.encode({:result, :ok, request_id})
