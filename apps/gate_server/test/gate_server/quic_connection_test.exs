...
defmodule T1Route do
  def route(1),
    do: {:ok, %{scene_ref: Process.whereis(T1Scene), world_ref: :unused, scene_epoch: 7}}
end
...
defmodule T1TransportTest do
    ...
    {:ok, conn} =
      :quicer.connect(
        ...
        [
          alpn: [~c"voxim-m1"],
          verify: :verify_peer,
          cacertfile: String.to_charlist(certs <> "ca.pem"),
  ...
  defp control(conn, hello, cid \\ 101) do
    ...
    {:ok, join_bytes} =
      Session.Codec.encode(%Session.Join{
        request_id: 1,
        username: "one",
        token: "valid",
        cid: cid,
    ...
    {:ok, _} =
      :quicer.async_send(
        stream,
        <<1, byte_size(hello_bytes)::32, hello_bytes::binary, byte_size(join_bytes)::32,
          join_bytes::binary>>,
        0
      )
  ...
  test "authorized join has Gate identity; wrong cid never reaches Scene", %{
    conn: conn,
    hello: hello
  } do
  ...
  test "voxel sink order survives messages queued before second stream opens", %{
    conn: conn,
    hello: hello
  } do
    control(conn, hello)
  ...
  test "movement datagrams preserve identity and stale epochs never reach Scene", %{
    conn: conn,
    hello: hello
  } do
    control(conn, hello)
    ...
    frame = %MmoContracts.Movement.InputFrame{
      input_seq: 1,
      axis_x: 0,
      axis_z: 0,
      yaw: 0,
    ...
    ack = %MmoContracts.Movement.OwnerAck{
      identity: identity,
      server_tick: 1,
  ...
  test "duplicate cid closes old epoch reliably while new join remains isolated", %{
    ...
    expected =
      <<1, byte_size(hello_bytes)::32, hello_bytes::binary, byte_size(end_bytes)::32,
        end_bytes::binary>>
  ...
  defp receive_bytes(stream, expected, bytes) do
    receive do
      {:quic, more, ^stream, _} when is_binary(more) ->
        receive_bytes(stream, expected, bytes <> more)
    after
  ...
  test "paused receiver holds reliable data while snapshots replace before native send", %{
    conn: conn,
    hello: hello
  } do
    ...
    control_bytes =
      <<1, byte_size(hello_bytes)::32, hello_bytes::binary, byte_size(reply_bytes)::32,
        reply_bytes::binary>>
    ...
    for seq <- 1..100 do
      ...
      record = %MmoContracts.Movement.SnapshotRecord{
        entity_id: 202,
        entity_epoch: 1,
        interest_generation: 1,
        collision_revision: 1,
        state: %{player | yaw: seq}
      }

      snapshot = %MmoContracts.Movement.Snapshot{
        identity: identity,
        server_tick: seq,
        records: [record]
      }
    ...
    assert {:ok, %MmoContracts.Movement.OwnerAck{processed_input_seq: 100}} =
             MmoContracts.Movement.Codec.decode(ack_bytes)

    assert_receive {:quic, snapshot_bytes, ^conn, _} when is_binary(snapshot_bytes), 5000

    assert {:ok,
            %MmoContracts.Movement.Snapshot{
              server_tick: 100,
              records: [%{entity_id: 202, state: %{yaw: 100}}]
            }} =
  ...
  @tag :batching
  test "replacements batch by real tick and preserve generations while ACK progresses first", %{
    conn: conn,
    hello: hello
  } do
    control(conn, hello)
    ...
    ack = %Movement.OwnerAck{
      identity: identity,
      server_tick: 52,
      processed_input_seq: 10,
      collision_revision: 7,
      state: a.state,
      substituted_through_seq: 0,
      simulation_tick: 51
    }
  ...
  @tag :batching
  test "FIFO service order is sorted only inside a compatible encoded packet", %{
    conn: conn,
    hello: hello
  } do
    control(conn, hello)
  ...
  @tag :batching
  test "replacement preserves each entity's send turn while newer ticks keep arriving", %{
    conn: conn,
    hello: hello
  } do
    control(conn, hello)
      ...
      delivered =
        for round <- 0..2 do
          ...
          ack = %Movement.OwnerAck{
            identity: identity,
            server_tick: tick,
            processed_input_seq: tick,
            collision_revision: round + 1,
            state: hd(records).state,
            substituted_through_seq: 0
          }
          ...
          assert Enum.all?(
                   received.records,
                   &(&1.collision_revision == round + 1 and &1.state.yaw == tick)
                 )
  ...
  test "negotiated datagram size is not capped at 1200 bytes", %{conn: conn, hello: hello} do
    ...
    send(
      sink,
      {:quic, :dgram_state_changed, server_conn, %{dgram_send_enabled: true, dgram_max_len: 1380}}
    )
  ...
  test "real negotiated datagrams carry all records and expose sent byte counters", %{
    ...
    assert stats.snapshot_bytes_sent ==
             Enum.sum(Enum.map(packets, fn {_, bytes} -> byte_size(bytes) end))

    IO.puts(
      "M3_GATE_BATCH negotiated_before=#{limit} negotiated_after=#{stats.max_datagram} records=32 datagrams=#{length(packets)} bytes=#{stats.snapshot_bytes_sent}"
    )
  end

  @tag :batching
  test "reliable leave and reenter retain order while the queued generation is replaced", %{
    conn: conn,
    hello: hello
  } do
    ...
    reenter = %{
      enter
      | entity_epoch: fresh.entity_epoch,
        interest_generation: fresh.interest_generation,
        server_tick: 71
    }
    ...
    send(
      sink,
      {:mmo_datagram, identity,
       %{snapshot | server_tick: 71, records: [fresh, snapshot_record(202)]}}
    )
    ...
    expected =
      IO.iodata_to_binary([
        <<1>>
        | Enum.map([hello, enter, leave, reenter], fn message ->
  ...
  defp send_bounded_snapshot(sink, snapshot, limit) do
    ...
    :sys.replace_state(sink, fn state ->
      ...
      {:noreply, state} =
        QuicConnection.handle_info({:mmo_datagram, snapshot.identity, snapshot}, state)
  ...
  defp controlled_native_send(sink, messages, limit) do
    :sys.replace_state(sink, fn state ->
      ...
      state =
        Enum.reduce(messages, %{state | max_datagram: limit}, fn message, acc ->
          {:noreply, queued} =
            QuicConnection.handle_info({:mmo_datagram, acc.identity, message}, acc)
  ...
  test "split control purpose and length preserve Hello while wrong identity closes", %{
    conn: conn,
    hello: hello
  } do
    {:ok, stream} = :quicer.start_stream(conn, [{:active, true}])
...
defmodule M4aGateTransferTest do
  ...
  defp edit_gate(state, owner) do
    receive do
      {:inspect_state, owner} ->
        send(owner, {:gate_state, state})
        edit_gate(state, owner)

      event ->
        {:noreply, state} = QuicConnection.handle_info(event, state)

        if match?({:mmo_edit_finished, _, _, _}, event),
          do: send(owner, {:gate_edit_completed, state.edit_completed, state})
  ...
  defp batch_event(scene, request, x) do
    bytes =
      <<0x78, request::64, request::32, scene::64, 1::32, x::signed-32, 1::signed-32,
        1::signed-32, 11::16>>
  ...
  test "cold edits preserve Gate input and transfer progress, FIFO and connection result ownership" do
    ...
    initial = %{
      ...
      | listener: listener,
        ...
        route: %{scene_ref: :source_scene, world_ref: world},
        streams: %{
          voxel_stream: %{purpose: 2, buffer: <<>>, started: true},
    ...
    results =
      for {bytes, _, _} <- :queue.to_list(completed.reliable[2]) do
    ...
    {:noreply, ^other} =
      QuicConnection.handle_info({:mmo_voxel_bytes, completed.edit_ref, <<1>>}, other)

    closing = %{completed | closing: true}

    {:noreply, ^closing} =
      QuicConnection.handle_info({:mmo_voxel_bytes, completed.edit_ref, <<1>>}, closing)

    {:noreply, ^completed} =
      QuicConnection.handle_info({:mmo_voxel_bytes, initial.identity, <<1>>}, completed)
  ...
  test "voxel stream accepts pending and previous Scene tags while rejecting unrelated or out of bounds edits" do
    ...
    initial = %{
      ...
      | voxim_overlay: true,
        ...
        pending_transfer: %{
          initial.pending_transfer
          | route: %{scene_ref: :target_scene, world_ref: :shared_world}
        },
    ...
    assert_receive {{:voxel_batch_edit_intent, %{logical_scene_id: 2}},
                    %{world_ref: :shared_world}, _}
    ...
    assert_receive {{:voxel_batch_edit_intent, %{logical_scene_id: 1}},
                    %{world_ref: :shared_world}, _}
  ...
  test "normal build beyond the initial collision box reaches authority without closing Gate" do
    ...
    bytes =
      <<0x7F, 1::64, 1::32, 1::64, 1::8, 292::signed-32, 534::signed-32, 31::signed-32, 1::16,
        19::16>>
  ...
  defp pending_state do
    ...
    pending = %{
      identity: fresh,
      route: %{scene_ref: :target_scene},
      player: self(),
      cid: 101,
      transaction_seq: 20,
      collision_revision: 3,
      cut_tick: 60,
      processed_input_seq: 30,
    ...
    %{
      state
      | identity: old,
        route: %{scene_ref: :source_scene},
        player: :sealed_source,
  ...
  defp ready(state, identity, seq \\ 20, revision \\ 3) do
    ...
    {:noreply, state} =
      QuicConnection.handle_info(
        {:quic, <<byte_size(bytes)::32, bytes::binary>>, :control_stream, %{}},
        state
      )
  ...
  test "pending old and new inputs reach only passive target on both transports" do
    ...
    for transport <- [:datagram, :control],
        identity <- [initial.identity, initial.pending_transfer.identity] do
  ...
  test "Gate seals the source before preparing and keeps old fence output until Ready" do
    ...
    {:noreply, state} =
      QuicConnection.handle_info({:mmo_transfer_request, old, source, 2}, initial)
  ...
  test "a duplicate claim makes old Gate prepare and commit fail the owner CAS" do
    ...
    state = %{
      characters: %{101 => %{identity: replacement, pid: self(), scene_ref: :replacement}},
      next_epoch: 13,
      opts: [route_module: Router]
    }
  ...
  test "matching identity cannot prepare or commit from another Gate PID" do
    ...
    state = %{
      characters: %{101 => %{identity: old, pid: self(), scene_ref: :source_scene}},
      next_epoch: 11,
      opts: [route_module: Router]
    }
