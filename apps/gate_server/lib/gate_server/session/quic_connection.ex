defmodule GateServer.Session.QuicConnection do
  ...
  def init(opts) do
    {:ok,
     %{
       ...
       transfer_pending_inputs: 0,
       transfer_last_us: 0,
       edit_worker: nil,
       edit_ref: make_ref(),
       edit_pending: 0,
       edit_completed: 0,
       edit_queue_wait_us: 0,
       edit_worker_us: 0,
       ...
       bytes_in: 0,
       bytes_out: 0,
       reliable: %{1 => :queue.new(), 2 => :queue.new()},
       busy: MapSet.new(),
       queue_age_us: 0,
       reliable_completed: 0,
       pending_datagrams: :gb_trees.empty(),
       snapshot_keys: %{},
       snapshot_order: 0,
       datagram_busy: false,
       datagrams_replaced: 0,
       datagrams_sent: 0,
       snapshot_records_sent: 0,
       snapshot_datagrams_sent: 0,
  ...
  def format_status(status) do
    ...
    |> Map.put(:state, Map.take(status.state, [:identity, :closing]))
  end
  ...
  def handle_call(:stats, _from, state) do
    stats =
      Map.take(state, [
        ...
        :datagrams_replaced,
        :datagrams_sent,
        :snapshot_records_sent,
        :snapshot_datagrams_sent,
        :snapshot_bytes_sent,
        :transfer_prepared,
        :transfer_committed,
        :transfer_pending_inputs,
        :transfer_last_us,
        :edit_pending,
        :edit_completed,
        :edit_queue_wait_us,
    ...
    pending =
      case state.pending_transfer do
        nil ->
          nil
    ...
    stats =
      Map.put(
        stats,
        :reliable_queued,
        Enum.sum(Enum.map(state.reliable, fn {_, q} -> :queue.len(q) end))
      )
  ...
  def handle_info(
        {:mmo_transfer_request, identity, player, target_scene_id},
        %{identity: identity, player: player, pending_transfer: nil, closing: false} = state
      ) do
    started = System.monotonic_time(:microsecond)

    # seal 与此前输入由同一个 Gate 发送，源处理完该输入前缀后才能交出 artifact。
    with {:ok, artifact} <- SceneServer.Movement.Player.seal(player, identity),
         {:ok, fresh, route, target} <-
           GenServer.call(
             state.listener,
             {:prepare_transfer, identity, target_scene_id, artifact}
           ) do
      pending = %{
        identity: fresh,
        route: route,
        player: target,
        cid: artifact.id,
        transaction_seq: artifact.transaction_seq,
        collision_revision: artifact.simulation_revision,
        cut_tick: artifact.simulation_tick,
        processed_input_seq: artifact.slots.processed_input_seq,
        started_us: started,
        input_batches: 0
      }
      ...
      message = %Session.Transfer{
        identity: identity,
        next_identity: fresh,
        cut_tick: pending.cut_tick,
        processed_input_seq: pending.processed_input_seq,
        transaction_seq: pending.transaction_seq,
        collision_revision: pending.collision_revision,
      ...
      {:error, reason} ->
        Logger.warning(
          "mmo_transfer_prepare_failed old_scene=#{identity.scene_id} target_scene=#{target_scene_id} reason=#{inspect(reason)}"
        )
  ...
  def handle_info(
        {:mmo_reliable, identity, purpose, message},
        %{identity: identity, closing: false} = state
      )
  ...
  def handle_info(
        {:mmo_voxel_bytes, identity, bytes},
        %{identity: identity, closing: false} = state
      ),
      do: {:noreply, send_bytes(state, 2, bytes, 0)}
  ...
  def handle_info(
        {:mmo_edit_finished, ref, queue_us, elapsed_us},
        %{edit_ref: ref, closing: false} = state
      ) do
    {:noreply,
     %{
       state
       | edit_pending: state.edit_pending - 1,
         edit_completed: state.edit_completed + 1,
         edit_queue_wait_us: max(state.edit_queue_wait_us, queue_us),
         edit_worker_us: max(state.edit_worker_us, elapsed_us)
     }}
  end
  ...
  def handle_info({:quic, :send_complete, stream, false}, state) do
    ...
    state = %{
      state
      | busy: MapSet.delete(state.busy, purpose),
        reliable_completed: state.reliable_completed + 1
    }
        ...
        if byte_size(bytes) <= state.max_datagram do
          ...
          {:noreply,
           %{
             state
             | datagram_busy: true,
               datagrams_sent: state.datagrams_sent + 1,
               bytes_out: state.bytes_out + count,
               snapshot_records_sent: state.snapshot_records_sent + records,
               snapshot_datagrams_sent:
                 state.snapshot_datagrams_sent + if(records > 0, do: 1, else: 0),
               snapshot_bytes_sent:
                 state.snapshot_bytes_sent + if(records > 0, do: count, else: 0),
               queue_age_us:
                 max(state.queue_age_us, System.monotonic_time(:microsecond) - queued_at)
           }}
        else
          {:noreply, close(state, 8)}
        end
  ...
  def handle_info(
        {:quic, :dgram_send_state, conn, %{state: :dgram_send_canceled}},
        %{conn: conn} = state
      ),
  ...
  def handle_info({:quic, event, conn, _}, %{conn: conn} = state)
      when event in [:shutdown, :transport_shutdown],
      do: {:noreply, %{state | closing: true}}

  def handle_info({:quic, :peer_send_shutdown, _stream, _}, %{closing: false} = state),
    do: {:noreply, close(state, 3)}

  def handle_info({:quic, event, _stream, _}, state)
      when event in [:peer_send_aborted, :peer_receive_aborted],
      do: {:noreply, close(state, 3)}
  ...
  defp queue_datagram(state, %Movement.OwnerAck{} = message, queued_at) do
    ...
    %{
      state
      | pending_datagrams: :gb_trees.enter(:owner, {message, queued_at}, state.pending_datagrams),
  ...
  defp queue_datagram(state, %Movement.Snapshot{records: records} = message, queued_at) do
    Enum.reduce(records, state, fn record, acc ->
      ...
      {key, at, order, replaced} =
        case Map.fetch(acc.snapshot_keys, record.entity_id) do
          {:ok, key} ->
            {_, at} = :gb_trees.get(key, acc.pending_datagrams)
            {key, at, acc.snapshot_order, 1}

          :error ->
            {{acc.snapshot_order, record.entity_id}, queued_at, acc.snapshot_order + 1, 0}
        end
      ...
      %{
        acc
        | pending_datagrams: :gb_trees.enter(key, value, acc.pending_datagrams),
          snapshot_keys: Map.put(acc.snapshot_keys, record.entity_id, key),
  ...
  defp delete_datagram(state, {_, entity_id} = key) do
    %{
      state
      | pending_datagrams: :gb_trees.delete(key, state.pending_datagrams),
  ...
  defp pack_datagram(state, %Movement.Snapshot{} = message, bytes, queued_at, iterator) do
    case :gb_trees.next(iterator) do
      {key, {%Movement.Snapshot{identity: identity, server_tick: tick, records: [record]}, at},
       next}
  ...
  defp frame(%{identity: identity} = state, 1, <<255, 1::16, 2, _::binary>> = bytes)
       when identity != nil do
    case Movement.Codec.decode(bytes) do
  ...
  defp enqueue_edit(state, message) do
    ...
    Logger.info(
      "mmo_edit_ingress kind=#{kind} request_id=#{request.request_id} client_seq=#{Map.get(request, :client_intent_seq, 0)} node=#{node()} received_us=#{queued_at}"
    )

    ctx = %{
      status: :in_scene,
      voxim_overlay: true,
      world_ref: state.route.world_ref,
      player: state.player,
  ...
  defp edit_loop(gate, ref) do
    receive do
      {{kind, request} = message, ctx, queued_at} ->
        ...
        Logger.info(
          "mmo_edit_completed kind=#{kind} request_id=#{request.request_id} scene_id=#{request.logical_scene_id} queue_wait_us=#{started - queued_at} worker_us=#{elapsed}"
        )
      ...
      case state.router.route(join.scene_id) do
        {:ok, route} ->
          {identity, result} =
            GenServer.call(
              state.listener,
              {:claim, state.scene, Map.put(route, :scene_id, join.scene_id), character}
            )
  ...
  defp control(%{pending_transfer: pending} = state, %Session.Ready{} = ready)
       when pending != nil do
    if ready.identity == pending.identity and
         ready.baseline_transaction_seq == pending.transaction_seq and
         ready.collision_revision == pending.collision_revision do
      case GenServer.call(
             state.listener,
             {:commit_transfer, state.identity, pending.identity, pending.cid}
           ) do
        :ok ->
          ...
          %{
            ...
            | identity: pending.identity,
              ...
              pending_transfer: nil,
              transfer_committed: state.transfer_committed + 1,
        ...
        {:error, reason} ->
          Logger.warning(
            "mmo_transfer_commit_failed old_scene=#{state.identity.scene_id} target_scene=#{pending.identity.scene_id} reason=#{inspect(reason)}"
          )
  ...
  defp control(%{identity: identity} = state, %Session.Ready{identity: identity} = ready)
       when identity != nil do
    SceneServer.Movement.Player.ready(
      state.player,
      identity,
      ready.baseline_transaction_seq,
      ready.collision_revision
    )

    state
  end

  defp control(%{identity: identity} = state, %Session.TimeProbe{} = probe)
       when identity != nil do
  ...
  defp forward_input(
         %{pending_transfer: pending} = state,
         %Movement.InputBatch{identity: identity} = batch
       )
       when pending != nil do
    if identity == state.identity or identity == pending.identity do
      SceneServer.Movement.Player.input(pending.player, pending.identity, %{
        batch
        | identity: pending.identity
      })
      ...
      %{
        state
        | pending_transfer: %{pending | input_batches: pending.input_batches + 1},
  ...
  defp transfer_log(event, old, pending, now) do
    Logger.info(
      "mmo_#{event} old_scene=#{old.scene_id} new_scene=#{pending.identity.scene_id} " <>
  ...
  defp send_bytes(state, purpose, bytes, flags) do
    queue =
      :queue.in({bytes, flags, System.monotonic_time(:microsecond)}, state.reliable[purpose])
  ...
  defp submit_reliable(state, purpose, bytes, flags, queued_at) do
    ...
    case :quicer.async_send(
           stream,
           [prefix, <<byte_size(bytes)::32>>, bytes],
           Bitwise.bor(flags, 0x1000)
         ) do
      {:ok, count} ->
        ...
        %{
          state
          | bytes_out: state.bytes_out + count,
            busy: MapSet.put(state.busy, purpose),
  ...
  defp close(state, reason) do
    {:ok, bytes} =
      Session.Codec.encode(%Session.SessionEnd{identity: state.identity, reason: reason})
