// Phase 1c-5 / 1.6b adapter: server-authoritative `RefinedCellWireData`
// (occupancy + parallel layer attribute pools,
// `apps/scene_server/lib/scene_server/voxel/codec.ex`) → browser-side
// `FRefinedCellData` (single 512-bit occupancy bigint + flat per-slot
// arrays, `clients/web_client/src/voxel/storage/types.ts`).
//
...
//
// Phase 1.6b expansion (G-3 recommended): `attributeSetRef`, `tagSetRef`
// and `ownerObjectId` are no longer dropped — they are projected onto
// per-slot Uint32Array / BigUint64Array companions on the result. Downstream
// renderer / mesher / collision / audit consumers can read provenance
// without re-parsing wire bytes. `objectRefs` and the full u64 of
// `boundaryCache` are still narrowed (the wire form remains the truth source
// and `RefinedCellWireData` exposes them directly when callers need them).
//
// When two layers cover the same slot (the wire format guarantees they do
// not, but the adapter must be defensive) the layer that appears later in
// `wire.layers` wins for every per-slot field. The result is *not* written
// back to the wire — authoritative state is re-derived from each subsequent
...
export function wireToRefinedCell(wire: RefinedCellWireData): FRefinedCellData {
  const attributeSetRefsBySlot = new Uint32Array(SLOT_COUNT);
  const tagSetRefsBySlot = new Uint32Array(SLOT_COUNT);
  const ownerObjectIdsBySlot = new BigUint64Array(SLOT_COUNT);

  for (const layer of wire.layers) {
    const layerMask = combineMaskWords(layer.maskWords);
    if (layerMask === 0n) continue;
    let remaining = layerMask;
    while (remaining !== 0n) {
      const slot = trailingZeros(remaining);
      attributeSetRefsBySlot[slot] = layer.attributeSetRef;
      tagSetRefsBySlot[slot] = layer.tagSetRef;
      ownerObjectIdsBySlot[slot] = layer.ownerObjectId;
  ...
  return {
    ...
    attributeSetRefsBySlot,
    tagSetRefsBySlot,
    ownerObjectIdsBySlot,
  };
}
