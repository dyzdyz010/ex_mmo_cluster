...
**详见 [`docs/2026-06-27-架构设计指导思想-系统正交.md`](docs/2026-06-27-架构设计指导思想-系统正交.md)。任何设计稿/动手前先读，并按其「开工前自查清单」过一遍。**

三条核心：

1. **系统正交**：每个系统只负责一件清晰的事；系统间只走**稳定契约**，不共享可变的隐式假设。改一个系统不应「牵一发动全身」——若改 A 莫名弄坏概念上无关的 B，就是隐藏耦合，是设计缺陷。
2. **自维护不变量**：一个系统对外承诺的**所有**不变量（**含活性/续租/超时/重连等时间性不变量**）必须由它**自己持续维护**；绝不让别的系统的正确性悄悄依赖一个**没人维护**的假设。警惕「一次性建立、之后没人管」的资源（订阅/lease/连接/缓存）会静默失效。
3. **显式契约 > 隐式假设**：跨系统依赖要么**承诺方强制维护契约**，要么**依赖方对破坏鲁棒（自愈）**；最糟是「依赖方默默假设 + 承诺方没维护」。

> 血泪案例（已记入上文文档）：挖放体素「被接受却不显示」，最初误判为「移动导致订阅没更新」（暗示移动与编辑耦合）——**实为订阅缺少活性维护，lease 静默过期，站着不动也会坏，移动是红鲱鱼**。教训：订阅有「覆盖（位置驱动）」与「活性（时间驱动）」两个正交职责，原代码漏了活性；edit 的可见性隐式挂在「订阅活着」这个未维护不变量上。诊断 bug 先排除红鲱鱼（问「这故障跟我以为的原因真有因果吗」）。
