...
后续新增功能、行为修改、缺陷修复及测试维护，必须先读取并严格遵守
[`../Voxim/Docs/Testing.md`](../Voxim/Docs/Testing.md)。该文档是客户端与配套服务端共用的测试设计规则唯一正文，
适用于设计、实现、审查和验收；阶段记录、技能和临时计划不得以旧流程绕过规则。
与本节最小充分测试共同执行；正文只在该文件维护，不复制成另一套服务端标准。
